...
struct NativeLayerList: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTableViewDataSource, NSTableViewDelegate {
        ...
        /// Programmatic equivalent of dropping a row between rows. Kept as a shared,
        /// testable route rather than removing the upstream reorder regression tests.
        func moveLayer(_ id: UUID, to row: Int) -> Bool {
            guard session.canEditLayers, let document = session.document,
                  document.layers.contains(where: { $0.id == id }), (0...rows.count).contains(row) else { return false }
            let above = row < rows.count ? rows[row].id : nil
            let parent = row < rows.count ? rows[row].parentID : nil
            session.beginEdit("Reorder Layer")
            let result = session.placeLayer(id, in: parent, above: above, atBottom: row == rows.count)
            session.endEdit()
            return result
        }
