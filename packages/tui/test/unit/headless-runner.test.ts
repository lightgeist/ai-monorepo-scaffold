...
describe('runTuiExec', () => {
  ...
  it('renders validated findings as comments and keeps exit code zero', async () => {
    const root = await mkdtemp(join(tmpdir(), 'atlascode-exec-review-'));
    try {
      const stdout = vi.fn();
      const runtime = runtimeWith([
        {
  ...
  it('retains invalid JSON and its selected message without polluting stdout', async () => {
    const root = await mkdtemp(join(tmpdir(), 'atlascode-runner-diagnostics-'));
    try {
      const stdout = vi.fn();
      const runtime = runtimeWith();
  ...
  it('fails when Runtime marks the Review result invalid', async () => {
    ...
    expect(stderr).toHaveBeenCalledWith(expect.stringContaining('atlascode exec failed'));
  });

  it('writes the raw final message and reports durable per-Turn model and usage facts', async () => {
    const root = await mkdtemp(join(tmpdir(), 'atlascode-exec-result-'));
    try {
      const stdout = vi.fn();
      const runtime = runtimeWith();
  ...
  it('publishes shutdown failure as the terminal result before writing the final message', async () => {
    const root = await mkdtemp(join(tmpdir(), 'atlascode-exec-shutdown-'));
    try {
      const stdout = vi.fn();
      const lastMessage = join(root, 'last-message.txt');
  ...
  it('fails authentication before creating a Session or sending a message', async () => {
    ...
    expect(stderr).toHaveBeenCalledWith(
      'atlascode exec failed: Sign in to your managed provider account to use Agent features. Run `atlascode login`, then retry.\n',
    );
  });
