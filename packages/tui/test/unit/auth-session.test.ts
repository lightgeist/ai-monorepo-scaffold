...
import { createAuthNamespace, type OAuthClient } from '@atlascode/oauth-core';
import { afterEach, describe, expect, it, vi } from 'vitest';

import { createAtlasCodeSharedAuthSession } from '../../src/runtime/auth-session.js';
...
describe('createAtlasCodeSharedAuthSession', () => {
  it('persists and reuses a plaintext credential without a passphrase provider', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'atlascode-tui-auth-session-'));
    roots.push(dataDir);
    ...
    const session = createAtlasCodeSharedAuthSession({
      dataDir,
      region: 'en',
      buildEnv: 'test',
      oauthClient,
    ...
    const restored = createAtlasCodeSharedAuthSession({
      dataDir,
      region: 'en',
      buildEnv: 'test',
      oauthClient: {
