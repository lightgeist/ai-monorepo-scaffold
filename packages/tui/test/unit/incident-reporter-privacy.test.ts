...
function decrypt(event: Record<string, any>) {
  ...
  const key = Buffer.from(
    hkdfSync(
      'sha256',
      Buffer.from(token),
      Buffer.from('atlascode-desktop-event-log-v1'),
      Buffer.from(userId),
      32,
    ),
  );
...
describe('TUI automatic incident HTTP privacy boundary', () => {
  ...
  it.each(['config', 'ATLASCODE_DISABLE_TELEMETRY', 'DO_NOT_TRACK'])('does not upload when %s revokes consent during authentication', async (source) => {
    let enabled = true;
