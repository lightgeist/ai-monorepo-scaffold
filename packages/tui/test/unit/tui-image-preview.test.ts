...
describe('preview file loader', () => {
  ...
  beforeAll(async () => {
    ...
    bundleDir = await mkdtemp(join(tmpdir(), 'atlascode-preview-bundle-'));
  ...
  it('skips oversized files and excessive decoded dimensions before launching a decoder', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'atlascode-image-limit-'));
    try {
  ...
  it('reads a real PNG without modifying it and rejects cancelled reads', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'atlascode-image-preview-'));
    try {
