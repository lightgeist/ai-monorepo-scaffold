...
describe("TuiInputFlow steer Draft handling", () => {
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const workspaceDir = await mkdtemp(join(tmpdir(), "atlascode-paste-"));
  ...
  it("creates a recovery token for literal slash content with hidden transport", async () => {
    ...
    Object.assign(harness.submissionSeed, {
      transportContent:
        "<atlascode-chat-context>hidden</atlascode-chat-context>\n\n/help should remain message text",
    });
    ...
    expect(harness.onSubmissionStarted).toHaveBeenCalledWith(
      "/help should remain message text",
      expect.objectContaining({
        transportContent: expect.stringContaining("atlascode-chat-context"),
      }),
    );
  });
  ...
  it("closes the side conversation on empty Ctrl+D instead of quitting AtlasCode", () => {
    const harness = createHarness(
      "",
      { attachments: [] },
      false,
