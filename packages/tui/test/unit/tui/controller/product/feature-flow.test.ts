...
import type { AtlasCodeProviderTemplate } from "../../../../../src/provider/contract.js";
...
function createHarness(
  options: {
    ...
    readonly loadProviderTemplates?: () => Promise<
      readonly AtlasCodeProviderTemplate[]
    >;
...
describe("TuiFeatureFlow", () => {
  ...
  it("opens /plugins as a Runtime-backed feature screen and installs from the catalog", async () => {
    ...
    await vi.waitFor(() =>
      expect(harness.runtime.listSkills).toHaveBeenCalledWith("atlascode"),
    );
  ...
  it("adds a known API Key provider from /model and selects its model for the Session", async () => {
    const template: AtlasCodeProviderTemplate = {
      providerId: "deepseek",
      name: "DeepSeek",
  ...
  it("keeps local status available when account and optional details fail", async () => {
    ...
    expect(output).toContain("AtlasCode status");
    expect(output).toContain("/workspace");
    expect(output).toContain("Unavailable · Unavailable");
    expect(output).toContain("Managed provider account unavailable");
