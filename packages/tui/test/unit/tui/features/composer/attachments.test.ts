...
describe('AtlasCode TUI attachments', () => {
  it('resolves relative image paths against the workspace with safe metadata', async () => {
    const workspaceDir = await mkdtemp(join(tmpdir(), 'atlascode-attachment-'));
  ...
  it.each([
    ...
    const workspaceDir = await mkdtemp(join(tmpdir(), 'atlascode-video-attachment-'));
  ...
  it('expands home paths and rejects directories', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'atlascode-home-'));
    const workspaceDir = await mkdtemp(join(tmpdir(), 'atlascode-workspace-'));
