...
describe('Editor behavior', () => {
  ...
  it('overrides placeholder per frame without changing draft, history or submitted text', () => {
    const { editor } = createEditor({ paddingX: 1, placeholder: 'Ask AtlasCode to do anything' });
    editor.focused = true;
    ...
    expect(stripTerminalSequences(rendered)).not.toContain('Ask AtlasCode to do anything');
  ...
  it('renders a display-only placeholder until the user starts typing', () => {
    const placeholder = 'Ask AtlasCode to do anything';
  ...
  it('truncates the placeholder inside narrow editor bounds', () => {
    const { editor } = createEditor({ placeholder: 'Ask AtlasCode to do anything' });
