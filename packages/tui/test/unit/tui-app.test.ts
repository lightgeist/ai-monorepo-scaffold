...
import type { SendMessageReq } from "@atlascode/local-runtime-v2/cli-service";
import type { TuiObservability } from "../../src/observability/local-observability.js";
import type { AtlasCodeAuthProgress } from "../../src/auth/application.js";
...
function createObservability(): TuiObservability {
  return {
    ...
    observeRender: vi.fn(),
    snapshot: vi.fn(() => ({
      filePath: "/tmp/atlascode-observability.jsonl",
      eventCounts: {
        startup: 0,
        runtimeAccess: 0,
        eventStream: 0,
...
function createRuntime(): TuiRuntime {
  ...
  const runtime: TuiRuntime = {
    ...
    getRuntimeDiagnostics: vi.fn(async () => ({
      ...
      runtimeOwnerId: "atlascode",
      dataDir: "/home/dev/.atlascode",
      configPath: "/home/dev/.atlascode/config.yaml",
      configPresent: true,
    ...
    prepareFeedback: vi.fn(async ({ description, sessionId }) => ({
      ...
      description,
      diagnostics: [
        { label: "Client", value: "atlascode 0.1.0" },
...
describe("createTuiApp", () => {
  ...
  it("shows the AtlasCode prompt in an empty Composer and hides it after input", async () => {
    const app = createTuiApp({
      runtime: createRuntime(),
      terminal: new FakeTerminal(),
      version: "0.1.0",
  ...
  it("wires complete chat and autocomplete business events into the TUI surface", async () => {
    const events: AtlasCodeBusinessEvent[] = [];
    const businessTelemetry: AtlasCodeBusinessTelemetry = {
      track: (event, properties) =>
        events.push({ event, properties } as AtlasCodeBusinessEvent),
  ...
  it("shows an available update as a dedicated startup notice", async () => {
    ...
    await vi.waitFor(() => {
      expect(terminal.writes.join("")).toContain(
        "AtlasCode 1.2.4 is available",
      );
    });

    const rendered = terminal.writes.join("");
    expect(checkForUpdate).toHaveBeenCalledTimes(1);
    expect(rendered).toContain("Run '/update' to install AtlasCode 1.2.4");
  ...
  it("routes /update through the in-process update confirmation flow", async () => {
    ...
    const applyUpdate = vi.fn(async () => ({
      applied: true,
      message:
        "AtlasCode 1.2.4 is installed. Restart running AtlasCode sessions to use it.",
    }));
    const app = createTuiApp({
      runtime: createRuntime(),
      terminal: new FakeTerminal(),
    ...
    await vi.waitFor(() =>
      expect(
        app.transcript
          .snapshot()
          .some((cell) => cell.content.includes("AtlasCode update completed")),
      ).toBe(true),
    );
    await app.stop();
  });
  ...
  it("leaves the current TUI after /update stages an npm-prefix replacement", async () => {
    ...
    const app = createTuiApp({
      ...
      applyUpdate: async () => ({
        ...
        message: "AtlasCode 1.2.4 is staged safely.",
      }),
      requestRestart,
    });
        ...
        app.transcript
          ...
          .some((cell) =>
            cell.content.includes(
              "AtlasCode update failed: Finish or stop the active response before updating.",
            ),
          ),
      ).toBe(true),
    );
  ...
  it("does not expose the TUI before base hydration has settled", async () => {
    ...
    expect(rendered).not.toContain("Starting AtlasCode");
    ...
    expect(app.transcript.snapshot()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "warning",
          content:
            "Sign in to your managed provider account to use Agent features. Run /login, then retry.",
        }),
      ]),
    );
    expect(runtime.createSession).not.toHaveBeenCalled();
  ...
  it("starts a terminal-native workbench and submits messages through the Agent runtime", async () => {
    ...
    expect(terminal.title).toBe("AtlasCode");
    expect(runtime.createSession).toHaveBeenCalledWith({
      workspaceDir: "/workspace",
    });
    expect(app.transcript.snapshot()).toEqual([
  ...
  it("syncs the generated Session title to the terminal without duplicate writes", async () => {
    ...
    expect(terminal.titleUpdates).toEqual(["AtlasCode", sessionTitle]);
  ...
  it("can disable queue UI and preserve the next draft while a turn is running", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(rendered).not.toContain("AtlasCode · Running");
  ...
  it("shows running Thinking in chat and opens the full Transcript with /transcript", async () => {
    ...
    try {
      await vi.waitFor(() => {
        ...
        expect(rendered).not.toContain("AtlasCode ·");
  ...
  it("submits the hidden Runtime engineering log command name as an ordinary prompt", async () => {
    ...
    const app = createTuiApp({
      ...
      version: "0.1.0",
      workspaceDir: "/workspace",
      runtimeLogDirectory: "/home/dev/.atlascode/v2/observability/logs",
    });
  ...
  it("exposes sanitized checks and submits the hidden /config name as ordinary text", async () => {
    ...
    expect(runtime.listSkills).toHaveBeenCalledWith("atlascode", "docs");
  ...
  it("previews /feedback without uploading until Enter confirms the Runtime draft", async () => {
    ...
    panel?.handleInput?.("d");
    expect(panel?.render(80).join("\n")).toContain("Client  atlascode 0.1.0");
  ...
  it("keeps the status report local and submits the hidden /config name as text", async () => {
    ...
    expect(statusOutput).toContain("AtlasCode status");
      ...
      login: vi.fn(
        async (
          ...
          return {
            ...
            message:
              _region === "en"
                ? "Signed in with MiniMax Global. Restart AtlasCode to use this account region."
    ...
    expect(app.transcript.snapshot().slice(-6)).toMatchObject([
      {
        kind: "final-summary",
        content:
          "Complete managed provider login in your browser:\nhttps://account.example.test/device?client_surface=tui&download_source=mcode-internal\nCode: ABCD-EFGH",
      },
      {
        kind: "warning",
        content:
  ...
  it("does not restore /quit after reopening the exited Session", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-quit-draft-"));
    try {
  ...
  it("preserves hidden context and attachments when Edit recovery requires a new message", async () => {
    ...
    expect(app.editor.getExpandedText()).not.toContain("atlascode-chat-context");
    ...
    expect(
      app.transcript
        ...
        .map((cell) => cell.content)
        .join("\n"),
    ).not.toContain("atlascode-chat-context");
    await app.stop();
  });
  ...
  it("sends the selected answer immediately and refreshes the Agent continuation", async () => {
    ...
    await vi.waitFor(() =>
      expect(app.transcript.get("question:question-review")).toMatchObject({
        status: "resolved",
        detail: "Answer sent · AtlasCode is continuing…",
      }),
    );
  ...
  it("sends pasted local attachments and restores failed sends to the Composer", async () => {
    ...
    const readClipboardImage = vi.fn(async () => ({
      attachment: {
        type: "image" as const,
        filePath: "/tmp/atlascode-clipboard/diagram.png",
    ...
    expect(runtime.sendMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        attachments: [
          expect.objectContaining({
            local: { filePath: "/tmp/atlascode-clipboard/diagram.png" },
  ...
  it("pastes a clipboard image without replacing the text draft and cleans it after send", async () => {
    ...
    const readClipboardImage = vi.fn(async () => ({
      attachment: {
        type: "image" as const,
        filePath: "/tmp/atlascode-clipboard/clipboard-1.png",
    ...
    expect(runtime.sendMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        attachments: [
          expect.objectContaining({
            local: { filePath: "/tmp/atlascode-clipboard/clipboard-1.png" },
            meta: expect.objectContaining({ mimeType: "image/png" }),
          }),
        ],
      }),
  ...
  it("retains existing draft state when clipboard read or send fails", async () => {
    ...
    const readClipboardImage = vi
      ...
      .mockResolvedValueOnce({
        attachment: {
          type: "image" as const,
          filePath: "/tmp/atlascode-clipboard/clipboard-2.png",
  ...
  it("keeps queued clipboard files alive until the Runtime reports a terminal queue state", async () => {
    ...
    const readClipboardImage = vi.fn(async () => ({
      attachment: {
        type: "image" as const,
        filePath: "/tmp/atlascode-clipboard/clipboard-queue.png",
  ...
  it("writes one structured result when an auto-drained queued turn finishes", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-queued-result-"));
  ...
  it("continues sending and switching sessions when draft cleanup fails", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-app-draft-warning-"));
    const runtime = createRuntime();
    const app = createTuiApp({
      runtime,
      terminal: new FakeTerminal(),
  ...
  it("does not read or restore a persisted Draft during TUI startup", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-app-draft-"));
    try {
      const first = createTuiApp({
        runtime: createRuntime(),
        terminal: new FakeTerminal(),
  ...
  it("does not resume a persisted Draft during startup when login recovery is enabled", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-login-restart-draft-"));
    try {
      const first = createTuiApp({
        runtime: createRuntime(),
        terminal: new FakeTerminal(),
  ...
  it("keeps the submitted Draft recoverable when shutdown wins before Runtime admission", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-app-pending-draft-"));
    try {
      const runtime = createRuntime();
  ...
  it("does not restore a submitted Draft after Runtime accepts the turn", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "atlascode-app-accepted-draft-"));
    try {
      const runtime = createRuntime();
      vi.mocked(runtime.sendMessage).mockImplementation(
        async function* sendMessage(
