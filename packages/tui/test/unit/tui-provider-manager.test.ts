...
import type { AtlasCodeProviderSnapshot } from "../../src/provider/contract.js";

const snapshot: AtlasCodeProviderSnapshot = {
...
const snapshotWithCodex: AtlasCodeProviderSnapshot = {
  ...snapshot,
  providers: [
    {
...
describe("TuiProviderManager", () => {
  ...
  it("resumes the Codex login panel while sign-in is pending", async () => {
    ...
    const pendingSnapshot: AtlasCodeProviderSnapshot = {
      ...snapshotWithCodex,
  ...
  it("never marks a custom provider as the in-use source", () => {
    ...
    const withSelectedCustomModel: AtlasCodeProviderSnapshot = {
  ...
  it("redacts credentials and URL parameters from a custom provider", () => {
    const credentialed: AtlasCodeProviderSnapshot = {
      ...snapshot,
      providers: snapshot.providers.map((provider) =>
        provider.providerId === "custom_provider:openai"
          ? {
  ...
  it("renders a disabled custom provider without the in-use marker", () => {
    ...
    const disabled: AtlasCodeProviderSnapshot = {
      ...snapshot,
      providers: snapshot.providers.map((provider) =>
        provider.providerId === "custom_provider:openai"
