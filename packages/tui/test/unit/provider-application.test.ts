...
import type { AtlasCodeProviderModel, AtlasCodeProviderView } from '../../src/provider/contract.js';
import { AtlasCodeProviderApplication } from '../../src/provider/application.js';

function createPort() {
  return {
    discoverUserModelsCandidate: vi.fn(async (): Promise<readonly AtlasCodeProviderModel[]> => []),
...
describe('AtlasCodeProviderApplication', () => {
  it('exposes a disconnected Codex OAuth row when Runtime makes it visible', async () => {
    const application = new AtlasCodeProviderApplication(createPort());
  ...
  it('omits the Codex OAuth row when Runtime marks it hidden', async () => {
    ...
    const application = new AtlasCodeProviderApplication(port);
...
describe('saved provider model refresh', () => {
  const provider: AtlasCodeProviderView = {
  ...
  it('discovers with saved credentials and adds only new IDs without switching models', async () => {
    ...
    await expect(new AtlasCodeProviderApplication(port).refreshModels(provider)).resolves.toBe(1);
  ...
  it.each([{ models: [] }, { models: [{ modelId: 'old-model' }] }])('does not save an empty or unchanged discovery result', async ({ models }) => {
    ...
    await expect(new AtlasCodeProviderApplication(port).refreshModels(provider)).resolves.toBe(0);
  ...
  it('keeps the saved configuration when discovery fails', async () => {
    ...
    await expect(new AtlasCodeProviderApplication(port).refreshModels(provider)).rejects.toThrow('Authentication failed');
    expect(port.saveUserModelProviderCandidate).not.toHaveBeenCalled();
  });
  ...
  it('surfaces a revision conflict without retrying or creating a new account', async () => {
    ...
    await expect(new AtlasCodeProviderApplication(port).refreshModels(provider)).rejects.toThrow('Configuration changed');
  ...
  it('rejects a missing revision before making any request', async () => {
    const port = createPort();
    await expect(new AtlasCodeProviderApplication(port).refreshModels({ ...provider, configRevision: undefined })).rejects.toThrow('Reopen /provider');
