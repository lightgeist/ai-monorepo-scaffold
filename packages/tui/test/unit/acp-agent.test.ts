...
describe('AtlasCode ACP agent', () => {
        ...
        authMethods: [
          {
            type: 'terminal',
            id: 'atlascode-login',
            name: 'Sign in to AtlasCode',
            args: ['login'],
          },
        ],
        agentInfo: { name: 'atlascode', title: 'AtlasCode', version: '1.2.3' },
        _meta: {
          'atlascode/extensions': {
            version: 1,
            methods: expect.arrayContaining([
              'session/activate',
              'atlascode/session/steer',
              'atlascode/session/goal/create',
              'atlascode/session/delegation/get',
            ]),
            notifications: [
              'atlascode/session/current_session_update',
              'atlascode/session/queue_update',
              'atlascode/session/goal_update',
              'atlascode/session/delegation_update',
            ],
          },
        },
      });
  ...
  it('consumes cancel while a Prompt is waiting for Session admission', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await expect(configuration).resolves.toEqual({
        _meta: { 'atlascode/transition': 'next_prompt' },
      });
      await expect(prompt).resolves.toEqual({ stopReason: 'cancelled' });
    });
  ...
  it('bridges Runtime permission requests to the ACP client and returns its decision', async () => {
    ...
    expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-1', 'allowOnce');
  });
  ...
  it('denies a forged allow-always decision when Runtime did not offer it', async () => {
    ...
    expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-forged-always', 'deny');
  });
  ...
  it('routes descendant interactions to their attached root and ignores unrelated Sessions', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      emitRuntimeEvent({
        ...
        request: {
          ...
          requester: { sessionId: 'session-unrelated', agentName: 'atlascode' },
        },
      });

      await vi.waitFor(() =>
        expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-child', 'allowOnce'),
      );
      await new Promise<void>((resolve) => setImmediate(resolve));
    });
    ...
    expect(dismissQuestionnaire).not.toHaveBeenCalledWith('atlascode', 'questionnaire-unrelated');
  });
  ...
  it('fails a pending permission closed with its ACP attachment instead of applying a late grant', async () => {
    ...
    expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-close-race', 'deny');
  });
  ...
  it('keeps ingesting another Session while one permission request is unanswered', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await vi.waitFor(() =>
        expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-b', 'deny'),
      );
      await vi.waitFor(() =>
        expect(updates).toContainEqual({
          sessionId: 'session-2',
  ...
  it('bounds concurrent fail-closed replies before closing an overflowing interaction stream', async () => {
    ...
    expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-72', 'deny');
  });
  ...
  it('keeps watching interactions after one Runtime permission reply fails', async () => {
    ...
    expect(replyPermission).toHaveBeenLastCalledWith('atlascode', 'permission-2', 'deny');
  });
  ...
  it('bridges Runtime questionnaires through ACP form elicitation when the client supports it', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      emitRuntimeEvent({
        ...
        request: {
          ...
          requester: { sessionId: 'session-1', agentName: 'atlascode' },
          presentation: {
    ...
    expect(replyQuestionnaire).toHaveBeenCalledWith('atlascode', 'questionnaire-1', [
  ...
  it('preserves option identities and Other fields across normalized questionnaire shapes', async () => {
    ...
    expect(replyQuestionnaire).toHaveBeenCalledWith('atlascode', 'questionnaire-identities', [
  ...
  it('dismisses a pending questionnaire when its ACP attachment closes', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await vi.waitFor(() =>
        expect(dismissQuestionnaire).toHaveBeenCalledWith('atlascode', 'questionnaire-close-race'),
      );
    });
  ...
  it('continues a questionnaire-blocked prompt after the ACP client answers', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await vi.waitFor(() => expect(replyQuestionnaire).toHaveBeenCalledOnce());
      await expect(
        connection.request('atlascode/session/steer', {
          sessionId: session.sessionId,
  ...
  it('falls back to an ACP permission choice when form elicitation fails', async () => {
    ...
    expect(permissionRequests).toHaveLength(1);
    expect(replyQuestionnaire).toHaveBeenCalledWith('atlascode', 'questionnaire-form-fallback', [
      { stepId: 'approach', selectedOptionIds: ['safe'] },
    ]);
  });
  ...
  it('falls back to the first ACP permission choice for a multi-step questionnaire', async () => {
    ...
    expect(replyQuestionnaire).toHaveBeenCalledWith('atlascode', 'questionnaire-2', [
  ...
  it('rejects managed login when the account is warning but the token is missing', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      ).rejects.toThrow('atlascode login');
    });
  ...
  it('applies mode intent to the next prompt and exposes Runtime-backed configuration', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      ).resolves.toEqual({ _meta: { 'atlascode/transition': 'next_prompt' } });
      await expect(
  ...
  it('exposes steer, queue, Goal, and delegation control through ACP extensions', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      await connection.request(acp.methods.agent.initialize, {
        protocolVersion: acp.PROTOCOL_VERSION,
        clientCapabilities: {
          _meta: { 'atlascode/extensions': { version: 1, notifications: true } },
        },
      });
      const session = await connection.request(acp.methods.agent.session.new, {
        cwd: '/workspace',
      ...
      await expect(
        connection.request<{ sessionId: string }, { sessionId: string }>('atlascode/session/activate', {
          sessionId: session.sessionId,
        }),
      ).resolves.toEqual({ sessionId: 'session-1' });
      ...
      await expect(
        ...
        >('atlascode/session/queue/enqueue', {
          sessionId: session.sessionId,
          text: '\n  Run checks next  \n',
        }),
      ...
      await expect(
        ...
        >('atlascode/session/queue/update', {
          sessionId: session.sessionId,
          itemId: 'queue-1',
          text: '\n    Run focused checks  \n',
        }),
      ...
      await expect(
        connection.request<{ item: { status: string } }, { sessionId: string; itemId: string }>(
          'atlascode/session/queue/delete',
          {
            sessionId: session.sessionId,
            itemId: 'queue-1',
          },
      ...
      await expect(
        connection.request<{ receipt: { rootStopped: boolean } }, { sessionId: string }>(
          'atlascode/session/delegation/stop',
          { sessionId: session.sessionId },
        ),
  ...
  it('omits and rejects Goal extensions when the Runtime feature gate is disabled', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      const extensions = initialized._meta?.['atlascode/extensions'] as {
        methods: string[];
      ...
      expect(extensions.notifications).not.toContain('atlascode/session/goal_update');
      ...
      await expect(
        connection.request('atlascode/session/goal/create', {
          sessionId: session.sessionId,
    ...
    await client.connectWith(agent, async (connection) => {
      await connection.request(acp.methods.agent.initialize, {
        ...
        clientCapabilities: {
          ...
          elicitation: { form: {} },
          _meta: {
            'atlascode/extensions': { version: 1, notifications: true },
          },
        },
      });
      await connection.request(acp.methods.agent.session.new, {
  ...
  it('cancels a stale ACP permission prompt when Runtime resolves the request', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await vi.waitFor(() =>
        expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-next', 'allowOnce'),
      );
    });
  ...
  it('does not let an old attachment terminal event terminate a reused request id', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      await vi.waitFor(() =>
        expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-reused', 'allowOnce'),
      );
    });
  });
  ...
  it('consumes duplicate request terminals one interaction at a time', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      answers[1]?.({ outcome: { outcome: 'selected', optionId: 'allow-once' } });
      await vi.waitFor(() =>
        expect(replyPermission).toHaveBeenCalledWith('atlascode', 'permission-duplicate', 'allowOnce'),
      );
    });
  });
  ...
  it('settles a blocked Prompt when Runtime dismisses its Questionnaire', async () => {
    ...
    expect(dismissQuestionnaire).not.toHaveBeenCalledWith('atlascode', 'questionnaire-expired');
  });
  ...
  it('fails closed for unsupported additional directories', async () => {
    ...
    await client.connectWith(agent, async (connection) => {
      ...
      ).rejects.toThrow('Additional directories are not supported by AtlasCode ACP');
    });
...
function planReviewEvent(index: number): TuiRuntimeEvent {
  ...
  return {
    ...
    request: {
      schemaVersion: 1,
      id: `plan-${index}`,
      requester: { sessionId, agentName: 'atlascode' },
      presentation: {
        replaceComposer: false,
        showProgress: false,
        allowBackNavigation: false,
