...
import { ATLASCODE_DARK_THEME, ATLASCODE_LIGHT_THEME } from '../../src/tui/theme/palettes.js';
...
describe('TranscriptView', () => {
  ...
  it('keeps leaked caption text and absolute paths out of the compact receipt', () => {
    const path = '/Users/example/workspace/sample-project/output/atlascode-architecture.html';
    const view = new TranscriptView(() => [
      createTranscriptCell({
        ...
        content: [
          'Done.',
          '<deliver-assets>',
          `<media src="${path}" caption="AtlasCode architecture overview · ${path}RunTurnInput leaked text" />`,
          '</deliver-assets>',
        ].join('\n'),
        createdAtMs: 1,
      }),
    ...
    const lines = view.render(44).map((line) => stripVTControlCharacters(line).trimEnd());
    const receiptLine = lines.findIndex((line) => line.includes('atlascode-architecture.html ↗'));
    const normalized = lines.join(' ').replace(/\s+/gu, ' ');

    expect(receiptLine).toBeGreaterThan(-1);
    expect(normalized).toContain('AtlasCode architecture overview');
  ...
  it('makes the generated file label a highlighted OSC 8 file link when supported', () => {
    ...
    const palette =
      originalTheme.appearance === 'light' ? ATLASCODE_LIGHT_THEME : ATLASCODE_DARK_THEME;
  ...
  it('renders assistant dots in the same tone as the body text', () => {
    ...
    const originalPalette =
      original.appearance === 'light' ? ATLASCODE_LIGHT_THEME : ATLASCODE_DARK_THEME;
    applyTuiRenderTheme(ATLASCODE_DARK_THEME, 3);
    try {
      const view = new TranscriptView(() => [
        createTranscriptCell({
      ...
      expect(line).toContain(color.hex(ATLASCODE_DARK_THEME.colors.text)('●'));
      expect(line).not.toContain(color.hex(ATLASCODE_DARK_THEME.colors.signal)('●'));
    } finally {
  ...
  it('renders local inspection reports as a complete bordered table', () => {
    const view = new TranscriptView(() => [
      createTranscriptCell({
        ...
        kind: 'inspection',
        status: 'succeeded',
        content: 'AtlasCode status\nAccount: Managed provider connected',
        inspection: {
          title: 'AtlasCode status',
          badge: { label: 'READY', tone: 'success' },
          sections: [
            {
              title: 'Account',
              rows: [
                { label: 'Account', value: 'Managed provider connected', tone: 'success' },
    ...
    expect(rendered).toMatch(/│ AtlasCode status\s+● READY │/u);
    expect(rendered).toContain('READY');
    expect(rendered).toContain('├─ Account ');
    expect(rendered).toMatch(/│ Account\s+│ Managed provider connected\s+│/u);
  ...
  it('stacks inspection labels and values on narrow terminals', () => {
          ...
          sections: [
            {
              ...
              rows: [
                {
                  label: 'Source',
                  value: '/home/dev/.atlascode/config.yaml',
                },
              ],
            },
          ],
    ...
    expect(rendered).toContain('│   /home/dev/.atlasco');
    expect(rendered).toContain('│   de/config.yaml');
  ...
  it('renders user messages as a vertically padded full-width prompt band', () => {
    ...
    const originalPalette =
      original.appearance === 'light' ? ATLASCODE_LIGHT_THEME : ATLASCODE_DARK_THEME;
    applyTuiRenderTheme(ATLASCODE_LIGHT_THEME, 3);
    try {
      const view = new TranscriptView(() => [
        createTranscriptCell({
          id: 'user-band',
      ...
      expect(lines.join('\n')).not.toContain('YOU');
      expect(lines.every((line) => line.includes('\u001B[48;2;240;237;248m'))).toBe(true);
    ...
    const view = new TranscriptView(() => [
      createTranscriptCell({
        ...
        content: JSON.stringify({
          command: 'pnpm --filter @atlascode/ui exec vitest run packages/ui/test/unit/example.test.tsx',
        }),
        detail: 'ok',
        createdAtMs: 1,
      }),
  ...
  it('keeps unanswered prompts in the picker and shows compact answer progress and receipts', () => {
    const view = new TranscriptView(() => [
      ...
      createTranscriptCell({
        ...
        title: 'Answers sent',
        content: 'Release channel  Stable\nCollaboration  Mixed mode',
        detail: 'AtlasCode is continuing…',
        createdAtMs: 6,
      }),
    ]);
    ...
    expect(rendered).toContain('AtlasCode is continuing');
