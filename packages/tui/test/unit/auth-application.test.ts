...
import type { LogoutResult } from '@atlascode/oauth-core';

import { createLocalAuthFixture } from '../../../oauth-core/test/helpers/local-auth.js';

import type { AtlasCodeBusinessEvent } from '../../src/analytics/business-telemetry.js';
import { AtlasCodeAuthApplication } from '../../src/auth/application.js';
...
describe('AtlasCodeAuthApplication', () => {
  ...
  it('lets Core reuse an authenticated session without forcing a refresh', async () => {
    ...
    const application = new AtlasCodeAuthApplication({ dataDir: '/data', sharedAuthCore: core });
  ...
  it.each([
    ...
    async (region, buildEnv, origin) => {
      ...
      await expect(application.logout()).resolves.toEqual({
        state: 'signed-out',
        message: `Signed out of MiniMax ${region === 'cn' ? 'China' : 'Global'} on Desktop, CLI/TUI, and embedded atlascode-tools.`,
  ...
  it('reports OAuth telemetry and preserves login failures', async () => {
    ...
    const application = new AtlasCodeAuthApplication({
      ...
      sharedAuthCore: core,
      telemetry: {
        track: (event, properties) => events.push({ event, properties } as AtlasCodeBusinessEvent),
