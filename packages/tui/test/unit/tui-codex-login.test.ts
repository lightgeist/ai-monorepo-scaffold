...
import type { AtlasCodeCodexOAuthStatus } from "../../src/provider/contract.js";
...
const disconnected: AtlasCodeCodexOAuthStatus = {
  state: "disconnected",
  providerId: "openai-codex",
};
const device: AtlasCodeCodexOAuthStatus = {
  state: "pending",
  providerId: "openai-codex",
  method: "device_code",
  loginId: "attempt-1",
...
function harness(initial = disconnected) {
  const application = {
    getCodexOAuthStatus: vi.fn(
      async (): Promise<AtlasCodeCodexOAuthStatus> => initial,
    ),
    connectCodexOAuth: vi.fn(
      async (): Promise<AtlasCodeCodexOAuthStatus> => device,
    ),
    cancelCodexOAuthLogin: vi.fn(
      async (): Promise<AtlasCodeCodexOAuthStatus> => disconnected,
    ),
  };
...
describe("Codex sign-in panel", () => {
  ...
  it("cancels an authorization that arrives after the user closes the starting panel", async () => {
    const h = harness();
    let release!: (status: AtlasCodeCodexOAuthStatus) => void;
