...
import type { AtlasCodeProviderApiFormat, AtlasCodeProviderSnapshot } from '../provider/contract.js';
...
export type AtlasCodeProviderCliRequest =
...
interface AtlasCodeProviderCommandContext {
  readonly application: AtlasCodeProviderApplication;
  shutdown(): Promise<void>;
}

export interface RunAtlasCodeProviderCommandOptions {
  readonly version: string;
  readonly request: AtlasCodeProviderCliRequest;
  readonly environment?: Readonly<Record<string, string | undefined>>;
  readonly workspaceDir?: string;
  readonly lane?: string;
  readonly createContext?: (lane?: string) => Promise<AtlasCodeProviderCommandContext>;
}

export async function runAtlasCodeProviderCommand(
  options: RunAtlasCodeProviderCommandOptions,
): Promise<string> {
  ...
  try {
    ...
    if (request.action === 'add') {
      const envName = request.apiKeyEnv?.trim() || 'ATLASCODE_PROVIDER_API_KEY';
...
async function createProviderCommandContext(
  ...
  lane?: string,
): Promise<AtlasCodeProviderCommandContext> {
  ...
  return {
    application: new AtlasCodeProviderApplication(runtime.adapter),
    shutdown: async () => {
...
function formatSnapshot(snapshot: AtlasCodeProviderSnapshot, json: boolean): string {
