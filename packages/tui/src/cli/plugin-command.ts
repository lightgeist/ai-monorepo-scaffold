...
interface AtlasCodePluginCommandApplication {
  catalog(input: {
...
interface AtlasCodePluginCommandContext {
  readonly application: AtlasCodePluginCommandApplication;
  readonly dataDir: string;
  shutdown(): Promise<void>;
}

export interface RunAtlasCodePluginCommandOptions {
  readonly version: string;
  readonly request: AtlasCodePluginCliRequest;
  readonly lane?: string;
  readonly createContext?: (lane?: string) => Promise<AtlasCodePluginCommandContext>;
}

export async function runAtlasCodePluginCommand(
  options: RunAtlasCodePluginCommandOptions,
): Promise<string> {
...
function toCliPlugin(plugin: AtlasCodePluginView) {
  return {
...
function formatPluginRows(catalog: AtlasCodePluginCatalog): string {
...
function formatPlugin(plugin: AtlasCodePluginView): string {
...
function parseSelector(
  selector: string,
  explicitMarketplace?: AtlasCodePluginMarketplace,
): { readonly name: string; readonly marketplace?: AtlasCodePluginMarketplace } {
  const match = /^([^@]+?)(?:@(official|local))?$/.exec(selector.trim());
  if (!match?.[1]) throw new Error(`Invalid Plugin selector: ${selector}`);
  const marketplace = match[2] as AtlasCodePluginMarketplace | undefined;
...
function selectPlugin(
  plugins: readonly AtlasCodePluginView[],
  name: string,
  ...
  return matches[0] as AtlasCodePluginView;
}

async function createPluginCommandContext(
  version: string = 'unknown',
  lane?: string,
): Promise<AtlasCodePluginCommandContext> {
  ...
  return {
    application: new AtlasCodePluginApplication(runtime.adapter),
    dataDir,
    shutdown: async () => {
