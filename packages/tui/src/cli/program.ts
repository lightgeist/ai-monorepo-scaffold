...
import type { AtlasCodeProviderCliRequest } from './provider-command.js';
import {
  isModelProviderApiFormat,
  ATLASCODE_PROVIDER_API_FORMATS,
  type AtlasCodeProviderApiFormat,
} from '../provider/contract.js';
import type { AtlasCodePluginCliRequest, AtlasCodePluginMarketplace } from '../plugin/contract.js';
import { resolveTuiManagedBackendLane } from './environment.js';
import type { AtlasCodeTelemetryCliAction } from './telemetry-command.js';
...
export interface CreateTuiProgramOptions {
  ...
  runLogin: (region?: AtlasCodeRegion, openBrowser?: boolean, lane?: string) => Promise<void>;
  runLogout: (region?: AtlasCodeRegion) => Promise<void>;
  runUpdate: () => Promise<void>;
  runProvider?: (request: AtlasCodeProviderCliRequest, lane?: string) => Promise<void>;
  runPlugin?: (request: AtlasCodePluginCliRequest, lane?: string) => Promise<void>;
  runTelemetry?: (action: AtlasCodeTelemetryCliAction) => Promise<void>;
...
export function createTuiProgram(options: CreateTuiProgramOptions): Command {
  ...
  const runProvider = (request: AtlasCodeProviderCliRequest) =>
    activeLane
      ? requireProviderRunner(options)(request, activeLane)
      : requireProviderRunner(options)(request);
  const runPlugin = (request: AtlasCodePluginCliRequest) =>
    activeLane
  ...
  const acp = program
    .command('acp')
    .description('Run AtlasCode as an Agent Client Protocol server over stdio')
  ...
  acp
    .command('login')
    .description('Sign in to use AtlasCode Agent features')
    ...
    .allowExcessArguments(false)
    .action((commandOptions: { region?: AtlasCodeRegion; browser?: boolean }) =>
      activeLane
  ...
  program
    .command('logout')
    .description('Sign out of the managed provider account used by this CLI')
    .option('--region <region>', 'account region: cn or global', parseLoginRegion)
    .allowExcessArguments(false)
    .action((commandOptions: { region?: AtlasCodeRegion }) => options.runLogout(commandOptions.region));

  program
    .command('update')
    .description('Check for and install an AtlasCode update')
    .allowExcessArguments(false)
  ...
  provider
    ...
    .option(
      '--api-format <format>',
      ATLASCODE_PROVIDER_API_FORMATS.join(', '),
      parseApiFormat,
      'anthropic-messages',
    )
    ...
    .action(
      (commandOptions: {
        name: string;
        baseUrl: string;
        apiFormat: AtlasCodeProviderApiFormat;
        model: string[];
        contextLimit?: number;
        outputLimit?: number;
        apiKeyEnv?: string;
  ...
  const plugin = program
    .command('plugin')
    .description('Manage AtlasCode Plugins')
    .allowExcessArguments(false)
  ...
  plugin
    ...
    .action(
      (commandOptions: {
        marketplace?: AtlasCodePluginMarketplace;
        available?: boolean;
        json?: boolean;
      }) =>
        runPlugin({
  ...
  for (const action of ['add', 'remove', 'enable', 'disable'] as const) {
    plugin
      ...
      .action(
        (
          selector: string,
          commandOptions: { marketplace?: AtlasCodePluginMarketplace; json?: boolean },
        ) =>
          runPlugin({
            action,
            selector,
  ...
  const marketplace = plugin
    .command('marketplace')
    .description('List or refresh AtlasCode Plugin sources');
...
function parseLoginRegion(value: string): AtlasCodeRegion {
  if (value === 'cn') return 'cn';
  if (value === 'global') return 'en';
  throw new InvalidArgumentError('expected "cn" or "global"');
}

function parseApiFormat(value: string): AtlasCodeProviderApiFormat {
  if (isModelProviderApiFormat(value)) return value;
  throw new InvalidArgumentError(`expected one of: ${ATLASCODE_PROVIDER_API_FORMATS.join(', ')}`);
}
...
function parsePluginMarketplace(value: string): AtlasCodePluginMarketplace {
