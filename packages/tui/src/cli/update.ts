...
import { AtlasCodeUpdateApplication, type AtlasCodeUpdatePlan } from '../update/application.js';
import {
  atlascodePrefixActivationScheduledMessage,
  atlascodePrefixJournalScheduleFailedMessage,
} from '../update/messages.js';
import { schedulePendingAtlasCodePrefixUpdate } from '../update/prefix-update.js';
import type { AtlasCodeUpdatePhase } from '../update/progress.js';
...
export interface RunAtlasCodeUpdateOptions {
  readonly application?: AtlasCodeUpdateApplication;
...
export async function runAtlasCodeUpdate(
  currentVersion: string,
  options: RunAtlasCodeUpdateOptions = {},
): Promise<void> {
  const application = options.application ?? new AtlasCodeUpdateApplication({ currentVersion });
  const interactive = options.interactive ?? Boolean(process.stdin.isTTY && process.stdout.isTTY);
  const write = options.write ?? ((value: string) => process.stdout.write(value));
  const schedulePendingPrefixUpdate =
    options.schedulePendingPrefixUpdate ?? (() => schedulePendingAtlasCodePrefixUpdate());
  if (await schedulePendingPrefixUpdate()) {
    write(`${atlascodePrefixActivationScheduledMessage()}\n`);
    return;
  }
  ...
  if (plan.kind === 'current') {
    write(
      plan.source === 'managed-installer'
        ? `AtlasCode ${plan.currentVersion} is current on ${plan.channel}.\n`
        : `AtlasCode ${plan.currentVersion} is current on @${plan.packageTag}.\n`,
    );
    return;
  }
  if (plan.kind === 'ahead') {
    write(
      `AtlasCode ${plan.currentVersion} is newer than ${
  ...
  if (plan.kind === 'manual') {
    write(
      'AtlasCode could not identify the owner of this installation. ' +
  ...
  const progress = new AtlasCodeUpdateCliProgress(write, updateProcessLabel(plan));
  progress.start();
  try {
    ...
    if (outcome.restartRequired && !(await schedulePendingPrefixUpdate())) {
      throw new Error(atlascodePrefixJournalScheduleFailedMessage());
    }
  } catch (error) {
    progress.stop();
    throw error;
...
class AtlasCodeUpdateCliProgress {
  private frameIndex = 0;
  private latestOutput = '';
  private latestOutputAtMs: number | undefined;
  private phase: AtlasCodeUpdatePhase | undefined;
  ...
  acceptPhase(phase: AtlasCodeUpdatePhase): void {
    this.phase = phase;
    if (phase === 'completed') {
      this.render();
      return;
  ...
  private reportActivity(): void {
    ...
    const outputActivity = this.latestOutputAtMs
      ...
      : '; waiting for output';
    const phase = this.phase ? phaseLabel(this.phase) : 'Updating AtlasCode';
  ...
  private render(): void {
    ...
    this.write(`\r\u001B[2K${frame} Updating AtlasCode${phase}${elapsed}${output}`);
  }
}

function phaseLabel(phase: AtlasCodeUpdatePhase): string {
...
function updateProcessLabel(
  plan: Extract<AtlasCodeUpdatePlan, { kind: 'available' | 'package-manager' }>,
): string {
...
function renderAvailableUpdate(
  ...
  if (plan.kind === 'available') {
    return (
      `AtlasCode ${plan.latestVersion} is available on ${plan.channel} ` +
      `(current ${plan.currentVersion}).\n`
    );
  }
  return (
    `AtlasCode ${plan.latestVersion} is available on @${plan.packageTag} ` +
...
function renderNonInteractiveInstruction(
  ...
  return plan.kind === 'package-manager'
    ...
    : 'No interactive confirmation is available. Run `atlascode update` from a terminal to install it.';
}
