...
import { createAuthNamespace, type AuthStatus } from '@atlascode/oauth-core';
import {
...
import {
  resolveAtlasCodeAuthEnvironment,
  setAtlasCodeStartupBuildEnvironment,
  type AtlasCodeAuthEnvironment,
  type ResolveAtlasCodeAuthEnvironmentOptions,
...
export interface ConfigureTuiRuntimeEnvironmentOptions extends ResolveAtlasCodeAuthEnvironmentOptions {
...
export function configureTuiRuntimeEnvironment(
  ...
  if (startupBuildEnvironment) setAtlasCodeStartupBuildEnvironment(startupBuildEnvironment);
  const buildEnvironment = resolveAtlasCodeAuthEnvironment(environmentOptions);
  ...
  const sharedAuthScope = dataDir
    ...
    : undefined;
  const inheritedRegion = readRuntimeRegion(target.ATLASCODE_RUNTIME_REGION);
  ...
  if (dataDir) {
    target.ATLASCODE_DATA_DIR = dataDir;
    target.ATLASCODE_RUNTIME_DATA_DIR = dataDir;
  }
  target.ATLASCODE_RUNTIME_REGION = environment.region;
  target.ATLASCODE_BUILD_ENV = environment.buildEnv;
  target.__ATLASCODE_RUNTIME_MANAGED = '1';
  return environment;
}
...
export function resolveTuiStartupEnvironmentOption(
  ...
  if (!internalPackage) {
    throw new Error('--env is only available in the internal AtlasCode package.');
  }
...
export function resolveTuiManagedBackendLane(
  ...
  buildEnv: AtlasCodeAuthEnvironment['buildEnv'] = resolveAtlasCodeAuthEnvironment().buildEnv,
): string | undefined {
...
function readSharedAuthScope(
  dataDir: string,
  buildEnv: AtlasCodeAuthEnvironment['buildEnv'],
): AtlasCodeAuthEnvironment | undefined {
...
function readRuntimeRegion(value: string | undefined): AtlasCodeAuthEnvironment['region'] | undefined {
