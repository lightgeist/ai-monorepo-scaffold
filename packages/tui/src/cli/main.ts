import {
  ATLASCODE_SUPPORTED_NODE_VERSIONS,
  ATLASCODE_VERSION,
...
import type { AtlasCodePluginCliRequest } from '../plugin/contract.js';
...
const ATLASCODE_PROCESS_TITLE = 'atlascode';
...
export interface RunTuiCliDependencies {
  ...
  readonly runLogout?: (region?: AtlasCodeRegion) => Promise<string>;
  readonly runUpdate?: (version: string) => Promise<void>;
  readonly runProvider?: (
    request: AtlasCodeProviderCliRequest,
    version: string,
    lane?: string,
  ) => Promise<string>;
  readonly runPlugin?: (
    request: AtlasCodePluginCliRequest,
    version: string,
    lane?: string,
  ) => Promise<string>;
...
export async function runTuiCli(dependencies: RunTuiCliDependencies = {}): Promise<void> {
  ...
  processRef.title = ATLASCODE_PROCESS_TITLE;
  ...
  if (!supportsNodeVersion(processRef.versions.node)) {
    processRef.stderr.write(
      `AtlasCode supports Node.js ${ATLASCODE_SUPPORTED_NODE_VERSIONS}; current version is ${processRef.versions.node}.\n`,
    );
  ...
  try {
    ...
    await createProgram({
      ...
      runAcp: async (lane) => {
        ...
        await runAcp(ATLASCODE_VERSION, lane);
        completedCommandExitMode = 'natural';
      },
...
async function formatTuiCliError(error: unknown): Promise<string> {
  ...
  const { buildAtlasCodePackageManagerCommand } = await import('../update/install-source.js');
  const command = buildAtlasCodePackageManagerCommand('npm-global', ATLASCODE_VERSION);
  return [
    'AtlasCode could not load its native SQLite dependency.',
...
async function defaultRunLogout(region?: AtlasCodeRegion): Promise<string> {
...
async function defaultRunUpdate(version: string): Promise<void> {
  const { runAtlasCodeUpdate } = await import('./update.js');
  await runAtlasCodeUpdate(version);
}
...
async function defaultRunProvider(
  ...
  const { runAtlasCodeProviderCommand } = await import('./provider-command.js');
  return runAtlasCodeProviderCommand({ request, version, lane });
}
...
async function defaultRunPlugin(
  ...
  const { runAtlasCodePluginCommand } = await import('./plugin-command.js');
  return runAtlasCodePluginCommand({ request, version, lane });
}
...
async function defaultRunTelemetry(
  ...
  const { runAtlasCodeTelemetryCommand } = await import('./telemetry-command.js');
  return runAtlasCodeTelemetryCommand(action, version, { environment });
}
