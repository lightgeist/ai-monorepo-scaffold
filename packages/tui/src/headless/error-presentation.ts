...
export function formatTuiExecFailure(error: TuiExecError): string {
  ...
  return `AtlasCode encountered an internal error: ${tuiErrorDiagnostic(error)}. Retry; if it keeps happening, report it through an available support channel.`;
}
...
export function formatTuiExecResultFailure(result: ExecResult): string {
  ...
  if (result.error?.category === 'config') {
    ...
    return `The run could not start with the current configuration${reason}. Open AtlasCode and run /doctor.`;
  }
  if (result.error?.category === 'internal') {
    const reason = result.error.message ? `: ${tuiErrorDiagnostic(result.error.message)}` : '';
    return `AtlasCode encountered an internal error${reason}. Retry; if it keeps happening, report it through an available support channel.`;
  }
  const reason = result.error?.message ? `: ${tuiErrorDiagnostic(result.error.message)}` : '';
