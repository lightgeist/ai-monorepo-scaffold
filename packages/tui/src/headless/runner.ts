...
export async function runTuiExec(
  ...
  let exitCode: number = ATLASCODE_EXEC_EXIT_CODES.internal;
  ...
  try {
    ...
    if (exitCode !== ATLASCODE_EXEC_EXIT_CODES.brokenPipe) {
      diagnostics.push(
        `atlascode exec failed: ${formatTuiExecFailure(normalized)}\n`,
      );
    }
  } finally {
    ...
    try {
      ...
      shutdownFailure = formatTuiActionFailure(error, {
        summary: "atlascode exec shutdown failed.",
    ...
    if (terminal) {
      ...
      try {
        ...
        if (terminal.result.status !== "succeeded") {
          diagnostics.push(
            `atlascode exec ${terminal.result.status}: ${formatTuiExecResultFailure(terminal.result)}\n`,
          );
        }
      } catch (error) {
        const normalized = normalizeRunError(error);
      ...
      shutdownFailure &&
      exitCode !== ATLASCODE_EXEC_EXIT_CODES.brokenPipe
    ) {
      exitCode = ATLASCODE_EXEC_EXIT_CODES.internal;
    }
...
async function assertSessionHasNoPendingInteraction(
  ...
  const agentName = session.agentName ?? ATLASCODE_DEFAULT_AGENT_NAME;
...
async function findLatestWorkspaceSession(
  ...
  do {
    const page = await runtime.listSessionPage({
      agentName: ATLASCODE_DEFAULT_AGENT_NAME,
      cursor,
      limit: 50,
