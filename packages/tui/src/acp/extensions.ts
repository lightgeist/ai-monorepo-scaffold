  ...
  return {
    ...
    notifications: goalEnabled
      ...
      : TUI_ACP_EXTENSION_NOTIFICATIONS.filter(
          (notification) => notification !== 'atlascode/session/goal_update',
        ),
  };
}
...
export function registerTuiAcpExtensions(options: RegisterTuiAcpExtensionsOptions): void {
  ...
  const activateSession = async (sessionId: string, client: acp.AgentContext) => {
    ...
    if (options.extensionNotificationsEnabled()) {
      await client
        .notify('atlascode/session/current_session_update', { sessionId })
        .catch(() => undefined);
    }
    return { sessionId };
  };
  ...
  options.app.onRequest('atlascode/session/activate', parseSessionRequest, ({ params, client }) =>
    activateSession(params.sessionId, client),
  );

  options.app.onRequest('atlascode/session/steer', parseTextRequest, async ({ params }) => {
  ...
  options.app.onRequest('atlascode/session/queue/list', parseSessionRequest, async ({ params }) => {
    resolve(params.sessionId);
    return { items: await options.runtime.listQueuedMessages(params.sessionId) };
  });

  options.app.onRequest('atlascode/session/queue/enqueue', parseTextRequest, async ({ params }) => {
    resolve(params.sessionId);
    return options.runtime.enqueueMessage(params.sessionId, params.text);
  });

  options.app.onRequest('atlascode/session/queue/update', parseQueueTextRequest, async ({ params }) => {
    resolve(params.sessionId);
    return {
      item:
  ...
  options.app.onRequest('atlascode/session/queue/delete', parseQueueItemRequest, async ({ params }) => {
    resolve(params.sessionId);
    return {
  ...
  options.app.onRequest('atlascode/session/queue/steer', parseQueueItemRequest, async ({ params }) => {
    resolve(params.sessionId);
    return options.runtime.steerQueuedMessage(params.sessionId, params.itemId);
  });

  options.app.onRequest('atlascode/session/goal/get', parseSessionRequest, async ({ params }) => {
    requireGoalEnabled();
    resolve(params.sessionId);
    return { goal: (await options.runtime.getGoal(params.sessionId)) ?? null };
  });

  options.app.onRequest('atlascode/session/goal/create', parseGoalCreateRequest, async ({ params }) => {
    requireGoalEnabled();
    resolve(params.sessionId);
    return {
  ...
  options.app.onRequest('atlascode/session/goal/patch', parseGoalPatchRequest, async ({ params }) => {
    requireGoalEnabled();
    resolve(params.sessionId);
    return {
  ...
  options.app.onRequest('atlascode/session/goal/clear', parseSessionRequest, async ({ params }) => {
    requireGoalEnabled();
    resolve(params.sessionId);
    return { cleared: await options.runtime.clearGoal(params.sessionId) };
  });

  options.app.onRequest('atlascode/session/delegation/get', parseSessionRequest, async ({ params }) => {
...
export function supportsTuiAcpExtensionNotifications(
  ...
  const extension = capabilities._meta?.['atlascode/extensions'];
  return (
    extension === true ||
    (isRecord(extension) &&
