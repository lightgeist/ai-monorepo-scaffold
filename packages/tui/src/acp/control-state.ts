...
function permissionModeOption(
  ...
  const current = ATLASCODE_PERMISSION_MODES.find((mode) => mode === permissionMode);
  if (!current) return undefined;
  return {
    type: 'select',
    id: ACP_CONFIG_PERMISSION_MODE,
    name: 'Permission mode',
    description: 'Controls how AtlasCode handles tool permission requests in this process.',
    category: '_permission',
    currentValue: current,
    options: ATLASCODE_PERMISSION_MODES.map((mode) => ({
      value: mode,
      name: formatTuiPermissionMode(mode),
    })),
    _meta: { 'atlascode/scope': 'process' },
  };
}
