...
const AUTH_METHOD_ID = 'atlascode-login';
...
export function createTuiAcpAgent(options: CreateTuiAcpAgentOptions): acp.AgentApp {
  ...
  const app = acp.agent({ name: 'atlascode' });
  ...
  const clearDetachedClientState = (
    ...
    if (supportsTuiAcpExtensionNotifications(clientCapabilities)) {
      void client
        .notify('atlascode/session/current_session_update', { sessionId: null })
        .catch(() => undefined);
    }
  };
  ...
  app.onConnect((connection) => {
    if (hasConnected) {
      connection.close(new Error('AtlasCode ACP supports exactly one Client connection per process.'));
      return;
    }
    hasConnected = true;
  ...
  app.onRequest(acp.methods.agent.session.setMode, async ({ params }) => {
    return runSessionLifecycle(params.sessionId, async (signal) => {
      ...
      return {
        _meta: {
          'atlascode/transition': active.pendingModeIntent ? 'next_prompt' : 'settled',
        },
      };
    });
  });
  ...
  app.onRequest(acp.methods.agent.session.setConfigOption, async (context) => {
    return runSessionLifecycle(context.params.sessionId, async (signal) => {
      ...
      if (typeof params.value !== 'string') {
        throw acp.RequestError.invalidParams(
          undefined,
          'AtlasCode ACP configuration options are select controls.',
        );
      }
  ...
  app.onRequest(acp.methods.agent.session.prompt, async (context) => {
    ...
    try {
      ...
      throw acp.RequestError.internalError(
        undefined,
        result.error
          ? `AtlasCode Runtime failed: ${result.error}`
          : 'AtlasCode Runtime failed.',
      );
    } finally {
...
function createRuntimeControlProjections(options: {
  ...
  if (resolved && options.event.type === 'session.queue.updated') {
    ...
    projections.push({
      ...
      run: async (signal) => {
        ...
        if (!isCurrent()) return;
        await options.client.notify('atlascode/session/queue_update', {
          sessionId: acpSessionId,
          items,
        });
      },
  ...
  if (goalEnabled && resolved && options.event.type === 'thread_goal.updated') {
    ...
    projections.push({
      ...
      run: async (signal) => {
        if (!isCurrentAttachment(options.sessions, acpSessionId, active, signal)) return;
        await options.client.notify('atlascode/session/goal_update', {
          sessionId: acpSessionId,
          goal,
        });
      },
...
async function notifyDelegationUpdates(
  ...
  for (const [acpSessionId, active] of attachments) {
    ...
    if (!isCurrent()) continue;
    await client.notify('atlascode/session/delegation_update', {
      sessionId: acpSessionId,
      snapshot,
    });
  }
...
function assertNoAdditionalDirectories(directories: readonly string[] | undefined): void {
  ...
  throw acp.RequestError.invalidParams(
    undefined,
    'Additional directories are not supported by AtlasCode ACP.',
  );
}
...
async function followQuestionnaireContinuations(options: {
  ...
  try {
    while (!options.signal.aborted) {
      ...
      if (transition.kind === 'failed') {
        throw acp.RequestError.internalError(
          undefined,
          transition.message
            ? `AtlasCode Runtime continuation failed: ${transition.message}`
            : 'AtlasCode Runtime continuation failed.',
        );
      }
    ...
    throw acp.RequestError.internalError(
      undefined,
      `AtlasCode Runtime continuation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
async function assertAuthenticated(runtime: TuiAcpRuntime): Promise<void> {
  try {
    ...
    if (error instanceof TuiLoginRequiredError) {
      throw acp.RequestError.authRequired(undefined, 'Run `atlascode login` and try again.');
    }
    throw error;
  }
}
