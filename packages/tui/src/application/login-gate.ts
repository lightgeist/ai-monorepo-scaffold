...
export const ATLASCODE_TUI_LOGIN_REQUIRED_MESSAGE =
  'Sign in to your managed provider account to use Agent features. Run /login, then retry.';

export const ATLASCODE_HEADLESS_LOGIN_REQUIRED_MESSAGE =
  'Sign in to your managed provider account to use Agent features. Run `atlascode login`, then retry.';
...
export class TuiLoginRequiredError extends TuiFailure {
  constructor(
    ...
    message = ATLASCODE_HEADLESS_LOGIN_REQUIRED_MESSAGE,
  ) {
...
export async function requireTuiInteractiveAgentAccess(
  ...
  await requireTuiAgentAccess(
    ...
    sessionId,
    ATLASCODE_TUI_LOGIN_REQUIRED_MESSAGE,
    onAccount,
  );
}
