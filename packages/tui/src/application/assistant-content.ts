...
const RICH_CONTENT_RE =
  /<(?:deliver[-_]assets|media|publish[_-]artifact|preview_card|genui-|atlascode-widget|atlascode-thinking|atlascode-progress|think|final|permission-(?:ask|response)|questionnaire-(?:ask|response))\b|^\s*preview_cards?:\s*$/imu;
...
export function projectAssistantContentForTerminal(content: string): TerminalAssistantContent {
  ...
  let markerPrefix = '\uE000atlascode-protected';
...
function simplifyUnprotectedContent(content: string, assets: DeliverAssetItem[]): string {
  let output = content;
  output = stripPairedBlocks(output, 'atlascode-thinking');
  output = stripPairedBlocks(output, 'think');
  output = unwrapPairedBlocks(output, 'atlascode-progress');
...
function simplifyWidgets(content: string): string {
  const completeWidgetRe = /<atlascode-widget\b([^>]*)>([\s\S]*?)<\/atlascode-widget\s*>/giu;
  let output = content.replace(completeWidgetRe, (_match, rawAttributes: string, body: string) => {
    const fallback = /<atlascode-fallback\b[^>]*>([\s\S]*?)<\/atlascode-fallback\s*>/iu.exec(body)?.[1];
    if (fallback?.trim()) return fallback.trim();
    return formatWidgetFallback(parseAttributes(rawAttributes));
  });

  output = output.replace(/<atlascode-widget\b([^>]*)>[\s\S]*$/iu, (_match, rawAttributes: string) =>
    formatWidgetFallback(parseAttributes(rawAttributes)),
  );
  return output.replace(/<atlascode-widget\b[^>]*$/iu, 'Interactive content is loading…');
}
