...
import type { AtlasCodeNpmPackageName } from './install-source.js';
import {
...
const UPDATE_LOCK_DIRECTORY = '.atlascode-update.lock';
const LAYOUT_VERSION = 2;

type AtlasCodePlatformPath = typeof path.posix | typeof path.win32;

export interface AtlasCodeVersionedPrefixActivation {
...
export function createAtlasCodeVersionedPrefixStagingPrefix(
  activePrefix: string,
  version: string,
...
export function prepareAtlasCodeVersionedPrefixStaging(stagingPrefix: string): void {
  mkdirSync(stagingPrefix, { recursive: true, mode: 0o700 });
}

export function acquireAtlasCodeVersionedPrefixUpdateLock(activePrefix: string): () => void {
  ...
  try {
    ...
    if (blocked) throw new Error('Another AtlasCode update is already running.');
  } catch (error) {
...
export function activateAtlasCodeVersionedPrefixInstall(
  activation: AtlasCodeVersionedPrefixActivation,
  platform: NodeJS.Platform = process.platform,
): string {
  const platformPath = platformPathFor(platform);
...
function readExpectedMetadata(
  ...
  const metadata = readAtlasCodePrefixPackageMetadata(prefix, activation.packageName, platform);
  if (metadata.version !== activation.expectedVersion) {
    throw new Error(
      `AtlasCode release contains ${metadata.version}; expected ${activation.expectedVersion}.`,
    );
  }
  return metadata;
}
...
function assertVersionedRelease(
  ...
  for (const [file, contents] of expected) {
    if (readFileSync(file, 'utf8') !== contents) {
      throw new Error(`AtlasCode release launcher is invalid: ${file}`);
    }
  }
}
...
function versionedReleaseLauncherContents(
  ...
  if (!existsSync(runtimeExecutable)) {
    throw new Error('AtlasCode versioned launcher runtime is missing.');
  }
  for (const command of versionedCommands(metadata)) {
    if (!command.binEntry) {
      throw new Error(`AtlasCode versioned ${command.name} package entry is missing.`);
    }
    ...
    if (
      ...
      throw new Error(`AtlasCode versioned ${command.name} launcher path is invalid.`);
    }
...
function versionedRootLauncherContents(
  ...
  if (platform === 'win32') {
    for (const command of versionedCommands()) {
      launchers.set(
        ...
        `@ECHO off\r\nSETLOCAL\r\nSET /P ATLASCODE_RELEASE=<"%~dp0current"\r\nECHO(%ATLASCODE_RELEASE%| %SystemRoot%\\System32\\findstr.exe /R /X "[0-9A-Za-z][0-9A-Za-z._-]*" >NUL || EXIT /B 1\r\nCALL "%~dp0releases\\%ATLASCODE_RELEASE%\\${command.releaseLauncher}.cmd" %*\r\nEXIT /B %ERRORLEVEL%\r\n`,
      );
      launchers.set(
        platformPath.join(activePrefix, `${command.name}.ps1`),
        `#!/usr/bin/env pwsh\r\n$release = (Get-Content -LiteralPath (Join-Path $PSScriptRoot 'current') -Raw).Trim()\r\nif ($release -notmatch '^[0-9A-Za-z][0-9A-Za-z._-]*$') { throw 'Invalid AtlasCode current release pointer.' }\r\n& (Join-Path $PSScriptRoot "releases\\$release\\${command.releaseLauncher}.ps1") @args\r\nexit $LASTEXITCODE\r\n`,
      );
    }
    return launchers;
  }
  for (const command of versionedCommands()) {
    launchers.set(
      ...
      `#!/bin/sh\nset -eu\nroot=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)\nrelease=$(tr -d "\\r\\n" < "$root/current")\ncase "$release" in ""|*[!0-9A-Za-z._-]*) echo "Invalid AtlasCode current release pointer." >&2; exit 1;; esac\nexec "$root/releases/$release/${command.releaseLauncher}" "$@"\n`,
    );
  }
  return launchers;
}

interface AtlasCodeVersionedCommand {
  readonly name: 'atlascode' | 'atlascode-tools';
  readonly releaseLauncher: '.atlascode-launcher' | '.atlascode-tools-launcher';
  readonly binEntry?: string;
}

function versionedCommands(
  metadata?: AtlasCodePrefixPackageMetadata,
): readonly AtlasCodeVersionedCommand[] {
  return [
    {
      name: 'atlascode',
      releaseLauncher: '.atlascode-launcher',
      ...(metadata ? { binEntry: metadata.binEntry } : {}),
    },
    {
      name: 'atlascode-tools',
      releaseLauncher: '.atlascode-tools-launcher',
      ...(metadata ? { binEntry: metadata.atlascodeToolsBinEntry } : {}),
    },
  ];
}
