import type {
  ...
  LocalRuntimeConfig,
} from '@atlascode/local-runtime-v2/process-local';
import { getDefaultLocalRuntimeConfig } from '@atlascode/local-runtime-v2';
import {
  getRuntimeBuildEnv,
  getRuntimeRegion,
...
import type { AtlasCodeToolsHostAuthSession } from '@atlascode/atlascode-tools-host';
import {
...
import { resolveAtlasCodeDataEnvironment } from '../auth/environment.js';
...
import { createAtlasCodeSharedAuthSession } from './auth-session.js';
import { resolveTuiManagedBackendLane } from '../cli/environment.js';
import {
  prepareTuiAtlasCodeToolsIntegration,
  type AtlasCodeToolsReadiness,
} from './atlascode-tools-integration.js';
...
export interface CreateTuiRuntimeDependencies {
  ...
  getDataEnvironment?: typeof resolveAtlasCodeDataEnvironment;
  fetchImpl?: typeof fetch;
  sharedAuthCore?: Pick<
    AtlasCodeOAuthCore,
    'getStatus' | 'getAccessToken' | 'handleUnauthorized' | 'watch'
  >;
  prepareAtlasCodeToolsIntegration?: typeof prepareTuiAtlasCodeToolsIntegration;
}
...
export async function createTuiRuntime(
  ...
  if (useSharedOAuth) {
    const activeSharedAuthCore =
      ...
      createAtlasCodeSharedAuthSession({
        dataDir: options.dataDir,
        region: authScope.region,
        buildEnv: authScope.buildEnv,
        oauthEndpoints: resolveAtlasCodeOAuthEndpointConfig(process.env, authScope),
      });
  ...
  const requestedAtlasCodeTools = getConfig().beta?.atlascodeTools === true;
  let atlascodeToolsReadiness: AtlasCodeToolsReadiness = fallbackAtlasCodeToolsReadiness(
    requestedAtlasCodeTools,
    authScope.buildEnv,
  );
  if (useSharedOAuth && sharedAuthCore) {
    atlascodeToolsReadiness = await (
      dependencies.prepareAtlasCodeToolsIntegration ?? prepareTuiAtlasCodeToolsIntegration
    )({
      requested: requestedAtlasCodeTools,
      dataDir: options.dataDir,
      buildEnv: authScope.buildEnv,
      region: authScope.region,
      session: createAtlasCodeToolsBrokerAuthSession(sharedAuthCore),
      entryUrl: import.meta.url,
      ...(bedrockLane ? { bedrockLane } : {}),
    });
  }
  const effectiveAtlasCodeTools = atlascodeToolsReadiness.ready;
  let authShutdown = false;
  const shutdownAuth = async (): Promise<void> => {
    if (authShutdown) return;
    authShutdown = true;
    stopSharedAuthWatch();
    await atlascodeToolsReadiness.dispose();
  };
  ...
  > = {
    ...
    configGetter: () => {
      ...
      return {
        ...
        beta: {
          ...config.beta,
          atlascodeTools: effectiveAtlasCodeTools,
        },
      };
    },
    ...
    options.surface === 'acp' ||
    (dependencies.getDataEnvironment ?? resolveAtlasCodeDataEnvironment)() === 'test'
  ...
  try {
      ...
      createEmbeddedRuntimeHost(
        {
          ...
          productCapabilities: { atlascodeTools: effectiveAtlasCodeTools },
        },
        dependencies.factory,
      ),
    );
    atlascodeToolsReadiness.ensureCommandPath();
    if (ownsObservability) {
      observability.recordStartup({
        phase: 'runtime.initialize',
        outcome: 'succeeded',
    ...
    if (cleanupFailures.length > 0) {
      throw new AggregateError(
        ...
        'AtlasCode Runtime startup cleanup failed.',
      );
    }
    throw error;
  }
...
function createAtlasCodeToolsBrokerAuthSession(
  core: NonNullable<CreateTuiRuntimeDependencies['sharedAuthCore']>,
...
function fallbackAtlasCodeToolsReadiness(
  requested: boolean,
...
export async function shutdownTuiRuntime(
  ...
  const reportFailure =
    ...
    ((step: string, error: unknown) => {
      ...
      process.stderr.write(`[atlascode] ${step} cleanup failed: ${message}\n`);
    });
