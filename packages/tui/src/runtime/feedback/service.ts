...
import { getRuntimeRegion, type AtlasCodeBuildEnv, type AtlasCodeRegion } from '@atlascode/config';
...
export class TuiFeedbackService {
  ...
  prepare(input: {
    ...
    const preview: TuiFeedbackPreview = {
      ...
      description,
      diagnostics: [
        { label: 'Client', value: `atlascode ${cleanFeedbackText(this.options.appVersion)}` },
  ...
  async submit(
    ...
    if (!token)
      throw feedbackError(
        'AtlasCode sign-in is required. Run /login, then retry.',
    ...
    try {
      ...
      if (!realUserID)
        throw feedbackError(
          'AtlasCode account identity is not ready. Check the connection, then retry.',
          'feedback_upload_failed',
          503,
          true,
        );
