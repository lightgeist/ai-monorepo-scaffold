...
export class TuiPluginAccess implements AtlasCodePluginRuntimeAccess {
  ...
  async listInstalledPlugins(
    ...
    return plugins
      ...
      .filter(
        (plugin): plugin is AtlasCodePluginView =>
          plugin !== undefined &&
  ...
  async mutatePlugin(
    input: Parameters<AtlasCodePluginRuntimeAccess['mutatePlugin']>[0],
...
function mutationMethod(action: Parameters<AtlasCodePluginRuntimeAccess['mutatePlugin']>[0]['action']) {
  return {
...
function toInstalledPlugin(plugin: InstalledPluginSummary): AtlasCodePluginView | undefined {
...
function toGeneratedSource(marketplace: AtlasCodePluginMarketplace) {
  return marketplace === 'official' ? OFFICIAL_PLUGIN_SOURCE : LOCAL_PLUGIN_SOURCE;
}

function fromGeneratedSource(source: number): AtlasCodePluginMarketplace | undefined {
