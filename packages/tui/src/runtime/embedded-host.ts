import { createLocalRuntimeHostV2, getDefaultLocalRuntimeConfig } from '@atlascode/local-runtime-v2';
import type {
...
import type { ProductBuildIdentity } from '@atlascode/shared/product-build-identity';
import { resolveAtlasCodeBuildIdentity } from '../auth/environment.js';
...
export function projectEmbeddedRuntimeConfig(
  ...
  buildIdentity: Pick<ProductBuildIdentity, 'isInternalBuild'> = resolveAtlasCodeBuildIdentity(),
  atlascodeToolsEnabled = false,
  ...
  return {
    ...
    beta: {
      ...
      atlascodeTools: atlascodeToolsEnabled && config.beta?.atlascodeTools === true,
    },
    memory: {
      ...config.memory,
      enabled: false,
...
export async function createEmbeddedRuntimeHost(
  ...
  if (process.env.ATLASCODE_LOCAL_RUNTIME_V2_FORCE_LEGACY === '1') {
    throw new Error(
      'AtlasCode embedded Runtime requires the local-runtime-v2 front door; legacy fallback is disabled.',
    );
  }
  ...
  const host = await factory({
    ...
    configGetter: () =>
      projectEmbeddedRuntimeConfig(
        configGetter(),
        resolveAtlasCodeBuildIdentity(),
        productCapabilities?.atlascodeTools === true,
      ),
  ...
  try {
    await host.ready;
    if (!host.cliService) {
      throw new Error('AtlasCode embedded Runtime does not expose CliService.');
    }
