...
export class TuiRuntimeAdapter implements TuiRuntime {
  ...
  constructor(cliService: CliService, options: TuiRuntimeAdapterOptions = {}) {
    ...
    const defaultAgentName =
      options.defaultAgentName ?? ATLASCODE_DEFAULT_AGENT_NAME;
  ...
  async prepareFeedback(input: {
    ...
    if (!this.feedback)
      throw new Error("AtlasCode feedback is unavailable.");
  ...
  submitFeedback(
    ...
    if (!this.feedback)
      return Promise.reject(new Error("AtlasCode feedback is unavailable."));
  ...
  setMiniMaxModelSource(
    source: import("../provider/contract.js").AtlasCodeMiniMaxModelSource,
  ) {
  ...
  createUserModelProvider(
    input: import("../provider/contract.js").AtlasCodeCreateProviderInput,
  ) {
  ...
  saveUserModelProviderCandidate(
    input: import("../provider/contract.js").AtlasCodeSaveProviderCandidateInput,
  ) {
    return this.productAccess.saveUserModelProviderCandidate(input);
  }
  updateUserModelProvider(
    input: import("../provider/contract.js").AtlasCodeUpdateProviderInput,
  ) {
...
export function createTuiQueueRequestId(): string {
  return `atlascode_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
}
