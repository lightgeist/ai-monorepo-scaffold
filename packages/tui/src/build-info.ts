...
export const ATLASCODE_MIN_NODE_VERSION = '22.19.0';
export const ATLASCODE_SUPPORTED_NODE_VERSIONS = '22.19+, 24, 25, or 26';
export const TUI_BUILD_PROFILE = 'tui';
export const ATLASCODE_PACKAGE_NAME = '@deepintuition/atlascode';
...
export function resolveTuiPackageVersion(moduleUrl: string | URL = import.meta.url): string {
  for (const relativePath of ['./package.json', '../package.json']) {
    try {
      ...
      if (
        (manifest.name === ATLASCODE_PACKAGE_NAME || manifest.name === '@atlascode/atlascode') &&
  ...
  throw new Error(`Cannot resolve ${ATLASCODE_PACKAGE_NAME} package version`);
}
...
export const ATLASCODE_VERSION = resolveTuiPackageVersion();
