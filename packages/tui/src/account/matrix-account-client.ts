...
import type { LocalRuntimeAuthContextSnapshot, AtlasCodeBuildEnv, AtlasCodeRegion } from '@atlascode/config';

import type { TuiTokenPlanQuota, TuiTokenPlanSummary } from '../runtime/port.js';

const MATRIX_ORIGINS: Readonly<Record<AtlasCodeRegion, Record<AtlasCodeBuildEnv, string>>> = {
  cn: {
...
const OPEN_PLATFORM_ORIGINS: Readonly<Record<AtlasCodeRegion, Record<AtlasCodeBuildEnv, string>>> = {
  cn: {
...
export class TuiMatrixAccountClient {
  ...
  private async fetchAccountIdentity(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AtlasCode account identity request failed with HTTP ${response.status}`);
      }
      const body = asRecord(await response.json());
      if (!body) throw new Error('AtlasCode account identity request returned an invalid response');
      ...
      if (!realUserID) {
        throw new Error('AtlasCode account identity request returned no account ID');
      }
      const identity = {
        realUserID,
  ...
  private async postMatrixJson(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AtlasCode ${operation} failed with HTTP ${response.status}`);
      }
      const responseBody = asRecord(await response.json());
      if (!responseBody) throw new Error(`AtlasCode ${operation} returned an invalid response`);
  ...
  async renewAccessToken(signal?: AbortSignal): Promise<string | undefined> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AtlasCode token renewal failed with HTTP ${response.status}`);
      }
      const body = asRecord(await response.json());
      if (!body) throw new Error('AtlasCode token renewal returned an invalid response');
      assertSuccessfulResponse(body, 'token renewal');
      const token = readString(asRecord(body.data), body, 'token');
      if (!token) throw new Error('AtlasCode token renewal returned no access token');
      return token;
    } finally {
      scopedSignal.dispose();
    }
...
export class TuiAuthenticationError extends Error {
  ...
  constructor(readonly status: 401 | 403) {
    super('Managed provider sign-in expired. Run /login, then retry.');
  }
}
...
function assertSuccessfulResponse(body: Record<string, unknown>, operation: string): void {
  ...
  if (typeof statusInfo?.code === 'number' && statusInfo.code !== 0) {
    ...
    throw new Error(`AtlasCode ${operation} failed with status ${statusInfo.code}`);
  }
  const baseResponse = asRecord(body.base_resp);
  if (typeof baseResponse?.status_code === 'number' && baseResponse.status_code !== 0) {
    throw new Error(`AtlasCode ${operation} failed with status ${baseResponse.status_code}`);
  }
}
