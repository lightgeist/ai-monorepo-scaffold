...
import { ATLASCODE_MAX_TOTAL_ATTACHMENT_BYTES } from '../application/attachment-policy.js';
import { inferTuiNativeVideoMimeType } from '../application/video-mime.js';

export const ATLASCODE_CLIPBOARD_IMAGE_MAX_BYTES = ATLASCODE_MAX_TOTAL_ATTACHMENT_BYTES;
...
export async function copyTuiAttachmentToTemporaryLease(
  ...
  const directory = await mkdtemp(join(options.tempRoot ?? tmpdir(), 'atlascode-clipboard-'));
...
async function persistClipboardImageBytes(
  ...
  const directory = await mkdtemp(
    join(options.tempRoot ?? tmpdir(), 'atlascode-clipboard-'),
  ).catch((error: unknown) => {
    throw classifyClipboardImageError(error, 'unavailable');
  });
...
async function readWslClipboardImageViaPowerShell(
  ...
  const directory = await mkdtemp(
    join(options.tempRoot ?? tmpdir(), 'atlascode-wsl-clipboard-'),
  );
...
function classifyClipboardImageError(
  ...
  if (
    ...
    return new TuiClipboardImageError(
      'permission-denied',
      'AtlasCode cannot read the system clipboard. Allow clipboard access, then try again.',
      { cause: error },
    );
  }
  return new TuiClipboardImageError(
...
function normalizeMaximumBytes(value: number | undefined): number {
  if (value === undefined || !Number.isFinite(value)) {
    return ATLASCODE_CLIPBOARD_IMAGE_MAX_BYTES;
  }
