...
export class TuiUpdateNotice implements Component {
  ...
  render(width: number): string[] {
    ...
    const headline = fitFirstStatusCandidate(
      [
        ` ${chalk.bold.hex(colors.signal)('✦ A new version of AtlasCode is available')} ${chalk.hex(
          colors.text,
        )('— update for the latest improvements')}`,
        ` ${chalk.bold.hex(colors.signal)(`✦ AtlasCode ${this.availableVersion} is available`)}`,
        ` ${chalk.bold.hex(colors.signal)('✦ AtlasCode update available')}`,
      ],
      safeWidth,
    );
    const action = fitFirstStatusCandidate(
      [
        ` ${chalk.hex(colors.muted)("Run '")}${chalk.bold.hex(colors.signal)(
          '/update',
        )}${chalk.hex(colors.muted)(`' to install AtlasCode ${this.availableVersion}`)}`,
        ` ${chalk.hex(colors.muted)('Run ')}${chalk.bold.hex(colors.signal)(
          '/update',
