...
The hero always uses the same ANSI Shadow glyphs: the full `ATLASCODE` at 91 columns or more, the six-line `AtlasCode` at 44–90 columns, and the same `M` glyph at 11–43 columns. Do not invent a separate pixel font or stack `MINI` / `MAX` / `CODE` on narrow screens. Compact Welcome must still render the hero and show the product name only once in the frame header.
