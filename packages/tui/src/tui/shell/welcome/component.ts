...
import {
  ATLASCODE_WELCOME_DESIGN,
  ATLASCODE_WELCOME_PASTE_IMAGE_SHORTCUT,
} from './design.js';
...
export class TuiWelcome implements Component {
  ...
  renderViewport(width: number, height: number): string[] {
    ...
    if (safeWidth < 4) return [fitLine('A', safeWidth)];
...
function renderWideWelcome(
  ...
  const tips = resolveWelcomeItems(content?.tips, ATLASCODE_WELCOME_DESIGN.wide.tips, 3);
  const news = resolveWelcomeItems(
    content?.changelogEntries,
    ATLASCODE_WELCOME_DESIGN.wide.news,
    3,
  );
  return [
    ...
    renderFrameRow(renderSectionTitle(ATLASCODE_WELCOME_DESIGN.sectionTitles.tips), width),
    ...tips.map((text) =>
      renderFrameRow(renderWelcomeBullet(resolveWelcomeCopy(text, keybindings)), width),
    ),
    renderFrameDivider(width),
    renderFrameRow(renderSectionTitle(ATLASCODE_WELCOME_DESIGN.sectionTitles.news), width),
    ...news.map((text) =>
      renderFrameRow(renderWelcomeBullet(resolveWelcomeCopy(text, keybindings)), width),
    ),
...
function renderStackedWelcome(
  ...
  const tips = resolveWelcomeItems(content?.tips, ATLASCODE_WELCOME_DESIGN.stacked.tips, 2);
  const news = resolveWelcomeItems(
    content?.changelogEntries,
    ATLASCODE_WELCOME_DESIGN.stacked.news,
    3,
  );
  return [
    ...renderTuiWelcomeHero(width),
...
function renderCompactWelcome(
  ...
  const tips = resolveWelcomeItems(content?.tips, ATLASCODE_WELCOME_DESIGN.compact.tips, 2);
  const news = resolveWelcomeItems(
    content?.changelogEntries,
    ATLASCODE_WELCOME_DESIGN.compact.news,
    3,
  );
  return [
    ...renderTuiWelcomeHero(width),
    '',
    renderFrameHeader(
      `${chalk.bold.hex(colors.brand)('AtlasCode')} ${chalk.hex(colors.muted)(`v${state.version}`)}`,
...
function renderWelcomeHeader(state: TuiShellState, width: number): string {
  return renderFrameHeader(
    chalk.hex(colors.muted)(`AtlasCode v${state.version} · by Deep Intuition`),
    renderActivity(state),
    width,
  );
}
