...
import type { AtlasCodeBusinessTelemetry } from '../analytics/business-telemetry.js';
...
import { AtlasCodeInteractiveRenderer } from './renderer/index.js';
...
import { ATLASCODE_DEFAULT_AGENT_NAME } from '../product-context.js';
import { createTuiAutomationResultWriter } from './automation/result-writer.js';
import { ATLASCODE_WELCOME_DESIGN } from './shell/welcome/design.js';
...
export function createTuiChatControllerComposition(options: CreateTuiAppOptions): {
  ...
  const defaultAgentName = options.defaultAgentName ?? ATLASCODE_DEFAULT_AGENT_NAME;
...
/**
 * Build the `ProcessTerminal` + `AtlasCodeInteractiveRenderer` + `TuiThemeController`
 * triad. Returned `themeController` is held by the caller so that
 * `AtlasCodeInteractiveRenderer.onRendererChanged` can rebind it after a
...
export function createTuiApplicationRenderer(options: CreateTuiAppOptions) {
  ...
  const renderer = new AtlasCodeInteractiveRenderer({
    terminal,
...
export function createTuiApplicationWidgets(options: {
  ...
  const welcome = new TuiWelcome(
    ...
    {
      tips: selectRandomTuiItems(ATLASCODE_WELCOME_DESIGN.tipPool, 3),
      ...(changelog
        ? {
