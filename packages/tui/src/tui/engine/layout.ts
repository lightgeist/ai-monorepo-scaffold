...
export interface TuiMouseEvent {
	...
	button: "primary" | "middle" | "secondary" | "none" | "unknown";
	x: number;
	y: number;
}
...
export function dispatchLayoutMouseEvent(frame: LayoutFrame, event: TuiMouseEvent): boolean {
	const targets: Array<{
		box: LayoutBox;
		component: MouseAwareComponent;
		depth: number;
		order: number;
	}> = [];
	let order = 0;
	const visit = (box: LayoutBox, depth: number): void => {
		if (!containsPoint(box.clip, event.x, event.y)) return;
		if (isMouseAwareComponent(box.component)) {
			targets.push({ box, component: box.component, depth, order: order++ });
		}
		for (const child of box.children) visit(child, depth + 1);
	};
	visit(frame.root, 0);
	targets.sort((left, right) =>
		right.box.layer - left.box.layer || right.depth - left.depth || right.order - left.order,
	);

	for (const { box, component } of targets) {
		if (
			component.handleMouse({
				...event,
				x: event.x - box.rect.x,
				y: event.y - box.rect.y + (box.lineOffset ?? 0),
			})
		) {
			return true;
		}
