...
import {
	extractOsc11BackgroundColorResponse,
	extractTerminalColorSchemeReport,
	type RgbColor,
...
export interface Component {
	...
	/** Focused component owns keyboard paging instead of the outer fullscreen viewport. */
	handlesViewportKeys?: boolean;
...
export abstract class TuiBase extends Container implements TUI {
	...
	private handleTerminalInput(data: string): void {
		const remaining = this.consumeTerminalColorSequences(data);
		...
		// Focus reports remain available to viewport listeners but are never editor text.
	...
	/**
	 * Terminals answer OSC 11 and push DEC 2031 reports asynchronously, so either sequence can
	 * arrive coalesced with unrelated input. Extract each one wherever it sits and return the
	 * input that still has to be dispatched, instead of discarding the whole chunk.
	 * @returns The remaining input after every recognized color sequence was removed.
	 */
	private consumeTerminalColorSequences(data: string): string {
		let remaining = data;
		while (this.pendingOsc11BackgroundReplies > 0) {
			const extracted = extractOsc11BackgroundColorResponse(remaining);
			if (!extracted) {
				break;
			}
			remaining = extracted.rest;
			this.settleOsc11BackgroundQuery(extracted.value);
		}

		const report = extractTerminalColorSchemeReport(remaining);
		if (report) {
			remaining = report.rest;
	...
	private settleOsc11BackgroundQuery(rgb: RgbColor | undefined): void {
