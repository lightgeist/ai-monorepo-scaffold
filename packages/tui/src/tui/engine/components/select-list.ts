...
const TRAILING_ANSI_RESET = /(?:\u001B\[0m)+$/u;
...
export class SelectList implements Component {
	...
	render(rawWidth: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			if (!item) continue;
			if (item.groupLabel && item.groupLabel !== previousGroupLabel) {
				lines.push(
					this.theme.groupLabel
						? this.theme.groupLabel(item.groupLabel)
						: this.theme.description(item.groupLabel),
				);
			}
			previousGroupLabel = item.groupLabel;
		...
		if (startIndex > 0 || endIndex < this.filteredItems.length) {
			...
			lines.push(this.theme.scrollInfo(truncatePlainToWidth(scrollText, Math.max(0, width - 2))));
		}
	...
	private renderItem(
		...
		if (descriptionSingleLine && width > 40) {
			...
			if (remainingWidth > MIN_DESCRIPTION_WIDTH) {
				const truncatedDescription = truncatePlainToWidth(descriptionSingleLine, remainingWidth);
				if (isSelected) {
					return this.theme.selectedText(`${prefix}${truncatedValue}${spacing}${truncatedDescription}`);
				}
	...
	private truncatePrimary(item: SelectItem, isSelected: boolean, maxWidth: number, columnWidth: number): string {
		...
		return truncatePlainToWidth(truncatedValue, maxWidth);
	}
...
function truncatePlainToWidth(text: string, maxWidth: number): string {
	return truncateToWidth(text, Math.max(0, maxWidth), "").replace(TRAILING_ANSI_RESET, "");
}
