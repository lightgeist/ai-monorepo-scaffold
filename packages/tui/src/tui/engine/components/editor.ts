...
export interface EditorStateSnapshot {
  ...
  readonly pastes: readonly { readonly id: number; readonly content: string }[];
...
export interface EditorOptions {
  ...
  /** Single-line display text shown while the editor is empty. */
  placeholder?: string;
...
export class Editor implements Component, Focusable {
  ...
  private transformPaste?: (text: string) => string;
  ...
  private reportedAutocompleteKind: '/' | '@' | undefined;
  ...
  public captureUndoExtensionState?: () => unknown;
  public restoreUndoExtensionState?: (state: unknown) => void;
  ...
  constructor(tui: TUI, theme: EditorTheme, options: EditorOptions = {}) {
    ...
    this.transformPaste = options.transformPaste;
  }
  ...
  private cursorOffset(): number {
    return (
      this.state.lines
        .slice(0, this.state.cursorLine)
        .reduce((total, line) => total + line.length + 1, 0) + this.state.cursorCol
    );
  }

  private setStateFromText(text: string, cursorOffset: number): void {
    const lines = text.split('\n');
    const cursor = Math.max(0, Math.min(text.length, Math.floor(cursorOffset)));
    let remaining = cursor;
    let cursorLine = 0;
    while (cursorLine < lines.length - 1 && remaining > (lines[cursorLine]?.length ?? 0)) {
      remaining -= (lines[cursorLine]?.length ?? 0) + 1;
      cursorLine += 1;
    }
    this.state = {
      lines: lines.length > 0 ? lines : [''],
      cursorLine,
      cursorCol: Math.min(remaining, lines[cursorLine]?.length ?? 0),
    };
    this.preferredVisualCol = null;
    this.snappedFromCursorCol = null;
  }

  private dropMissingPastes(): void {
    const activeIds = new Set(
      [...this.getText().matchAll(PASTE_MARKER_REGEX)].map((match) => Number(match[1])),
    );
    for (const id of this.pastes.keys()) {
      if (!activeIds.has(id)) this.pastes.delete(id);
    }
  }
  ...
  /** Override the display-only placeholder for this frame without changing Editor state. */
  render(width: number, placeholder = this.placeholder): string[] {
    ...
    for (const layoutLine of visibleLines) {
      const placeholderText =
        this.isEditorEmpty() && layoutLine.hasCursor && layoutLine.cursorPos === 0
          ? sliceByColumn(placeholder ?? '', 0, contentWidth, true)
          : '';
      let displayText = placeholderText || layoutLine.text;
      ...
      if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
        ...
        if (after.length > 0) {
          ...
          const formatText = placeholderText ? (this.theme.placeholder ?? identity) : identity;
          const cursor = `\x1b[7m${formatText(firstGrapheme)}\x1b[0m`;
          displayText = before + marker + cursor + formatText(restAfter);
  ...
  handleInput(inputData: string): void {
    ...
    if (this.autocompleteState && this.autocompleteList) {
      ...
      if (kb.matches(data, 'tui.input.tab')) {
        const selected: AutocompleteItem | null = this.autocompleteList.getSelectedItem();
        if (selected && this.autocompleteProvider) {
          if (this.autocompleteSuggestions) {
            this.onAutocompleteSelect?.(this.autocompleteSuggestions, selected);
          }
          this.pushUndoSnapshot();
          this.lastAction = null;
          ...
          this.cancelAutocomplete();
          if (this.onChange) this.onChange(this.getText());
          if (selected.continueCompletion) this.handleSlashCommandCompletion();
        }
        return;
      }
      ...
      if (
        ...
        this.autocompleteSuggestions?.applyOnEnter === false
      ) {
        this.cancelAutocomplete();
        ...
        if (selected && this.autocompleteProvider) {
          ...
          const isCommand = this.autocompleteSuggestions?.kind
            ? this.autocompleteSuggestions.kind === 'command'
            : this.autocompletePrefix.startsWith('/');
          if (isCommand && !selected.continueCompletion) {
            this.cancelAutocomplete();
  ...
  captureState(): EditorStateSnapshot {
    return {
      schemaVersion: 1,
      text: this.getText(),
      cursor: this.cursorOffset(),
      pastes: [...this.pastes.entries()]
  ...
  restoreState(snapshot: EditorStateSnapshot): boolean {
    if (!isValidEditorStateSnapshot(snapshot)) return false;
    this.cancelAutocomplete();
    this.lastAction = null;
    this.exitHistoryBrowsing();
    this.setStateFromText(snapshot.text, snapshot.cursor);
    this.pastes = new Map(snapshot.pastes.map(({ id, content }) => [id, content]));
    this.pasteCounter = snapshot.pasteCounter;
    this.dropMissingPastes();
    this.scrollOffset = 0;
  ...
  setCursorOffset(offset: number): void {
    this.setStateFromText(this.getText(), offset);
    this.tui.requestRender();
  }

  replaceRange(start: number, end: number, replacement: string, cursorOffset?: number): void {
    const current = this.getText();
    const rangeStart = Math.max(0, Math.min(current.length, Math.floor(start)));
    const rangeEnd = Math.max(rangeStart, Math.min(current.length, Math.floor(end)));
    const normalized = this.normalizeText(replacement);
    const next = `${current.slice(0, rangeStart)}${normalized}${current.slice(rangeEnd)}`;
    if (next === current) return;
    this.cancelAutocomplete();
    this.pushUndoSnapshot();
    this.lastAction = null;
    this.exitHistoryBrowsing();
    this.setStateFromText(next, cursorOffset ?? rangeStart + normalized.length);
    this.dropMissingPastes();
    if (this.onChange) this.onChange(this.getText());
    this.tui.requestRender();
  }
  ...
  dispose(): void {
    ...
    this.onAutocompleteView = undefined;
  ...
  private insertCharacter(char: string, skipUndoCoalescing?: boolean): void {
    ...
    if (!this.autocompleteState) {
      if (this.providerRequestsAutocomplete()) {
  ...
  private handlePaste(pastedText: string): void {
    ...
    const transformedText = this.transformPaste?.(decodedText) ?? decodedText;

    // Clean the pasted text: normalize line endings, expand tabs
    const cleanText = this.normalizeText(transformedText);
  ...
  private submitValue(): void {
    ...
    if (this.onSubmit) this.onSubmit(result, snapshot);
  }
  ...
  private handleBackspace(): void {
    ...
    if (this.state.cursorCol > 0) {
      ...
      const isPastedSegmented = lastGrapheme
        ? PASTE_MARKER_SINGLE.exec(lastGrapheme.segment)
        : null;
      ...
      if (isPastedSegmented) {
        ...
        // [paste #1] is removed).
        const higherIds = [...this.pastes.keys()]
        ...
        this.state.lines = this.state.lines.map((lineValue) =>
          lineValue.replace(PASTE_MARKER_REGEX, (fullMatch, idGroup, suffixGroup) => {
    ...
    if (this.autocompleteState) {
      ...
      const autocompleteLine = this.state.lines[this.state.cursorLine] || '';
      const textBeforeCursor = autocompleteLine.slice(0, this.state.cursorCol);
      // Slash command context
      if (this.providerRequestsAutocomplete() || this.isInSlashCommandContext(textBeforeCursor)) {
        this.tryTriggerAutocomplete();
      }
  ...
  private pushUndoSnapshot(): void {
    const extensionState = this.captureUndoExtensionState?.();
    this.undoStack.push({
      state: this.state,
      pastes: this.pastes,
      pasteCounter: this.pasteCounter,
  ...
  private undo(): void {
    ...
    this.pasteCounter = snapshot.pasteCounter;
    this.restoreUndoExtensionState?.(snapshot.extensionState);
    this.lastAction = null;
    this.preferredVisualCol = null;
    if (this.onChange) {
      this.onChange(this.getText());
  ...
  private providerRequestsAutocomplete(): boolean {
    return (
      this.autocompleteProvider?.shouldAutoTriggerCompletion?.(
        this.state.lines,
        this.state.cursorLine,
        this.state.cursorCol,
      ) === true
  ...
  private handleTabCompletion(): void {
    ...
    if (
      ...
      this.providerRequestsAutocomplete()
    ) {
  ...
  private applyAutocompleteSuggestions(
    ...
    const kind = suggestions.prefix.startsWith('/')
      ...
      : suggestions.prefix.startsWith('@')
        ? '@'
        : undefined;
    if (kind && kind !== this.reportedAutocompleteKind) {
      this.reportedAutocompleteKind = kind;
...
function isValidEditorStateSnapshot(snapshot: EditorStateSnapshot): boolean {
  if (
  ...
  return snapshot.pastes.every(({ id, content }) => {
    if (
      !Number.isInteger(id) ||
      id <= 0 ||
