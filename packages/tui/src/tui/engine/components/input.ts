...
export interface InputOptions {
	prompt?: string;
	transformPaste?: (value: string) => string;
	mask?: string;
}
...
export class Input implements Component, Focusable {
	...
	private handlePaste(pastedText: string): void {
		...
		const cleanText = this.options.transformPaste
			? this.options.transformPaste(pastedText)
	...
	render(width: number): string[] {
		...
		const renderedValue = this.options.mask ? [...this.value].map(() => this.options.mask).join("") : this.value;
		const renderedCursor = this.options.mask
			? [...this.value.slice(0, this.cursor)].length * visibleWidth(this.options.mask)
			: this.cursor;
		let visibleText = "";
		let cursorDisplay = renderedCursor;
		...
		if (totalWidth < availableWidth) {
			...
			const cursorCol = visibleWidth(renderedValue.slice(0, renderedCursor));
