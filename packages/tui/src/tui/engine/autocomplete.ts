...
export interface AutocompleteItem {
	...
	label: string;
	description?: string;
	/** Query the next completion stage after this item is accepted. */
...
export interface AutocompleteSuggestions {
	/** Distinguish command arguments from slash-prefixed paths when accepting with Enter. */
	kind?: "command" | "argument";
	items: AutocompleteItem[];
	prefix: string; // What we're matching against (e.g., "/" or "src/")
	/** Set false to submit the current input with Enter; Tab still applies the selection. */
	applyOnEnter?: boolean;
}

export interface AutocompleteProvider {
	/** Override automatic triggering; undefined keeps default triggers, false requires Tab. */
	shouldAutoTriggerCompletion?(lines: string[], cursorLine: number, cursorCol: number): boolean | undefined;
...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	shouldAutoTriggerCompletion(lines: string[], cursorLine: number, cursorCol: number): boolean | undefined {
		...
		return this.commands.some((command) =>
			"name" in command && command.name === match[1] && !!command.getArgumentCompletions,
		) ? true : undefined;
	}
			...
			if (spaceIndex === -1) {
				...
				const commandItems = this.commands.map((cmd) => {
					...
					return {
						...
						continueCompletion: "getArgumentCompletions" in cmd && !!cmd.getArgumentCompletions,
					};
				});
				...
				const filtered = fuzzyFilter(commandItems, prefix, (item) => item.name).map((item) => ({
					...
					...(item.continueCompletion && { continueCompletion: true }),
				}));
