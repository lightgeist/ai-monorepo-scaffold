...
import { AtlasCodeUpdateAdmissionError } from '../../../update/progress.js';
...
export interface TuiUpdateOptions {
  ...
  readonly inspectUpdate?: () => Promise<AtlasCodeUpdatePlan>;
...
export interface TuiUpdateFlowOptions {
  readonly inspect: () => Promise<AtlasCodeUpdatePlan>;
  readonly apply: (
    plan: AtlasCodeUpdatePlan,
    options?: AtlasCodeUpdateApplyOptions,
  ) => Promise<AtlasCodeUpdateOutcome>;
...
export class TuiUpdateFlow {
  ...
  async show(): Promise<void> {
    ...
    if (this.updateTask) {
      this.options.append(
        'An AtlasCode update is already running. Please wait for it to finish.',
        'warning',
      );
      return;
    }
    const requestSequence = ++this.requestSequence;
    let plan: AtlasCodeUpdatePlan;
    try {
  ...
  private startBackgroundUpdate(
    ...
    this.close(panel);
    this.options.append(
      `AtlasCode update started in the background (${plan.currentVersion} → ${plan.latestVersion}).`,
    );
    const task = this.requireAdmission()
      ...
      .then(
        (outcome) => {
          if (!this.stopped) {
            this.options.append(`AtlasCode update completed: ${outcome.message}`);
        ...
        (error: unknown) => {
          if (!this.stopped) {
            this.options.append(
              `AtlasCode update failed: ${error instanceof Error ? error.message : String(error)}`,
              'error',
            );
          }
        },
  ...
  private async requireAdmission(): Promise<void> {
    ...
    if (admission && !admission.allowed) {
      throw new AtlasCodeUpdateAdmissionError(
        admission.reason ?? 'Finish active AtlasCode work before updating.',
      );
    }
  }
}
