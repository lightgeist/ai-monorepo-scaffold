...
import { AtlasCodeProviderApplication } from '../../../provider/application.js';
import type { AtlasCodeCodexOAuthStatus, AtlasCodeProviderTemplate } from '../../../provider/contract.js';
import { AtlasCodePluginApplication } from '../../../plugin/application.js';
import type { AtlasCodePluginRuntimeAccess, AtlasCodePluginView } from '../../../plugin/contract.js';
...
import { ATLASCODE_VERSION } from '../../../build-info.js';
...
export interface TuiFeatureFlowOptions {
  ...
  readonly loadProviderTemplates?: () => Promise<readonly AtlasCodeProviderTemplate[]>;
...
export class TuiFeatureFlow {
  ...
  private readonly providerApplication: AtlasCodeProviderApplication;
  private readonly pluginApplication: AtlasCodePluginApplication;
  ...
  constructor(private readonly options: TuiFeatureFlowOptions) {
    ...
    this.providerApplication = new AtlasCodeProviderApplication(options.runtime);
    this.pluginApplication = new AtlasCodePluginApplication(options.runtime);
  ...
  private openTranscript(onForkFromUserMessage?: (messageId: string) => Promise<void>): void {
    ...
    const panel = new TuiTranscriptPanel({
      ...
      exportMetadata: () => {
        ...
        return {
          sessionId: session?.sessionId,
          title: session?.title || 'AtlasCode Transcript',
          exportedAtMs: Date.now(),
        };
      },
    ...
    try {
      ...
      this.options.append(
        formatTuiActionFailure(error, {
          ...
          nextStep: 'Reinstall or update AtlasCode, then retry /changelog.',
        }),
        'warning',
      );
    }
  ...
  async showModelPicker(query: string): Promise<void> {
    ...
    let codexOAuthStatus: AtlasCodeCodexOAuthStatus;
    try {
  ...
  private async showProviderOnboarding(): Promise<void> {
    ...
    this.options.onChanged();
    let templates: readonly AtlasCodeProviderTemplate[] = [];
  ...
  async showPlugins(initialQuery = ''): Promise<void> {
        ...
        if (!this.isStopped()) {
          this.options.append(
            formatTuiActionFailure(error, {
              ...
              nextStep: 'Restart AtlasCode or retry /plugins.',
            }),
            'warning',
          );
        }
    ...
    const refreshSkillsAfterMutation = async (
      operation: () => Promise<AtlasCodePluginView>,
  ...
  async showAccountStatus(): Promise<void> {
    await this.showReportInspection(
      ...
      async (publish) => {
        ...
        const presentation = {
          version: this.options.version ?? ATLASCODE_VERSION,
