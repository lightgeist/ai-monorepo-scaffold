...
export const ATLASCODE_TUI_FEEDBACK_LOGIN_REQUIRED_MESSAGE =
  'Sign in to your managed provider account to submit feedback. Run /login, then retry.';
...
export class FeedbackFlow {
  ...
  async show(description: string): Promise<void> {
    ...
    try {
      ...
      this.appendNotice(
        loginRequired
          ? ATLASCODE_TUI_FEEDBACK_LOGIN_REQUIRED_MESSAGE
