...
import type { AtlasCodeAuthPort } from '../../../auth/application.js';
...
export interface TuiCommandFlowOptions {
  ...
  readonly auth?: AtlasCodeAuthPort;
  ...
  readonly requestRestart?: (region?: AtlasCodeRegion, initialPrompt?: string) => void;
...
export class TuiCommandFlow {
  ...
  private showLoginRegionPicker(): void {
    if (!this.options.auth) {
      this.pendingLoginContinuation = undefined;
      this.options.append('Managed provider authentication is unavailable in this host.', 'warning');
      return;
    }
  ...
  private async runDailyCheckinCommand(): Promise<void> {
    try {
      ...
      this.options.append(
        "Couldn't verify managed provider sign-in. Check the connection, then retry /checkin.",
        'warning',
      );
      return;
    }
        ...
        try {
          ...
          this.options.append(
            formatTuiActionFailure(error, {
              ...
              nextStep: 'Check the AtlasCode data directory permissions, then retry /settings.',
  ...
  private async runAuthCommand(operation: 'login' | 'logout', region?: AtlasCodeRegion): Promise<void> {
    if (!this.options.auth) {
      this.options.append('Managed provider authentication is unavailable in this host.', 'warning');
      return;
    }
    this.options.setHint(operation === 'login' ? 'Starting managed provider sign-in…' : 'Signing out…');
    this.options.onChanged();
    try {
      const result =
        operation === 'login'
...
function looksLikeSessionReference(value: string): boolean {
  ...
  return /^\d+$/u.test(normalized) || /^(?:atl_|mvs_|ses_|session[-_:]|[0-9a-f]{8}-)/iu.test(normalized);
}
