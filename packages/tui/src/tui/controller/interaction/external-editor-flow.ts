...
export const ATLASCODE_EXTERNAL_EDITOR_SETUP_HINT = `Set ATLASCODE_EDITOR, VISUAL, or EDITOR to use ${formatTuiKeybinding('composer.external-editor')}; GUI editors need --wait.`;
  ...
  async open(): Promise<void> {
    ...
    if (!command) {
      this.options.setHint(
        `Set ATLASCODE_EDITOR, VISUAL, or EDITOR to use ${formatTuiKeybinding('composer.external-editor', this.options.keybindings)}; GUI editors need --wait.`,
      );
