...
const CATPPUCCIN_SYNTAX_COLORS: Readonly<
  Record<'dark' | 'light', Readonly<Record<SyntaxTone, string>>>
> = {
  dark: {
    blue: '#89B4FA',
    flamingo: '#F2CDCD',
...
} as const satisfies Record<'dark' | 'light', Record<SyntaxTone, keyof ChalkInstance>>;

export function createCatppuccinHighlightTheme(
  chalk: ChalkInstance,
  appearance: () => 'light' | 'dark',
): Theme {
  const color = (tone: SyntaxTone) => (text: string) => {
    const mode = appearance();
    if (chalk.level !== 1) return chalk.hex(CATPPUCCIN_SYNTAX_COLORS[mode][tone])(text);
  ...
  // Exact Catppuccin Highlight.js token mapping, adapted to ANSI output. As in Codex,
