import type { AtlasCodePluginView } from '../../../plugin/contract.js';
...
export interface TuiPluginManagerOptions {
  readonly plugins: readonly AtlasCodePluginView[];
  readonly initialQuery?: string;
  readonly onInstall: (plugin: AtlasCodePluginView) => Promise<AtlasCodePluginView>;
  readonly onRemove: (plugin: AtlasCodePluginView) => Promise<AtlasCodePluginView>;
  readonly onSetEnabled: (plugin: AtlasCodePluginView, enabled: boolean) => Promise<AtlasCodePluginView>;
  readonly onRefresh: () => Promise<readonly AtlasCodePluginView[]>;
...
export class TuiPluginManager implements TuiFeatureScreen, Component, Focusable {
  ...
  focused = false;
  private plugins: AtlasCodePluginView[];
  ...
  private visiblePlugins(): AtlasCodePluginView[] {
  ...
  private selected(): AtlasCodePluginView | undefined {
    ...
    try {
      ...
      this.status = formatTuiActionFailure(error, {
        ...
        nextStep: isPluginAuthRequired(error)
          ...
          : 'Retry or run atlascode plugin for details.',
      });
    } finally {
      if (!this.disposed) {
        this.busy = false;
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      this.status = formatTuiActionFailure(error, {
        ...
        nextStep: 'Retry or run atlascode plugin marketplace upgrade.',
      });
    } finally {
      if (!this.disposed) {
        this.busy = false;
...
function renderPluginRow(plugin: AtlasCodePluginView, selected: boolean, width: number): string {
...
function capabilitySummary(plugin: AtlasCodePluginView): string {
