...
const CUSTOM_FORMATS: readonly {
  readonly value: AtlasCodeProviderApiFormat;
...
export interface TuiProviderOnboardingOptions {
  readonly templates: readonly AtlasCodeProviderTemplate[];
...
export class TuiProviderOnboarding implements Component, Focusable {
  ...
  private template?: AtlasCodeProviderTemplate;
  private connection?: AtlasCodeProviderView;
  private alias = '';
  ...
  private customApiFormat: AtlasCodeProviderApiFormat = 'openai-completions';
  ...
  private filteredModels(): AtlasCodeProviderTemplate['models'] {
    const query = this.searchInput.getValue().trim().toLocaleLowerCase();
  ...
  private createFormatList(): SelectList {
    ...
    list.onSelect = (item) => {
      this.customApiFormat = item.value as AtlasCodeProviderApiFormat;
  ...
  private matchingConnections(): readonly AtlasCodeProviderView[] {
  ...
  private saveInput(apiKey: string): AtlasCodeSaveProviderCandidateInput | undefined {
    if (this.template) {
  ...
  private selectedModel(): AtlasCodeProviderTemplate['models'][number] | undefined {
