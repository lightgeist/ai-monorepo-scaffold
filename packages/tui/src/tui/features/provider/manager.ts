...
export interface TuiProviderManagerOptions {
  snapshot: AtlasCodeProviderSnapshot;
  onRefresh(): Promise<AtlasCodeProviderSnapshot>;
  onRefreshModels?(provider: AtlasCodeProviderView): Promise<number>;
  onTest(providerId: string, modelId?: string): Promise<AtlasCodeProviderTestResult>;
  onConnectCodex?(): void;
  onSaveCustom?(input: AtlasCodeSaveProviderCandidateInput): Promise<AtlasCodeSaveProviderCandidateResult>;
...
export class TuiProviderManager implements Component, Focusable {
  ...
  private snapshotValue: AtlasCodeProviderSnapshot;
  ...
  private providers(): readonly AtlasCodeProviderView[] {
    return this.snapshotValue.providers;
  }

  private selectedProvider(): AtlasCodeProviderView | undefined {
  ...
  private async connectCodex(provider: AtlasCodeProviderView): Promise<void> {
 ...
 * selected.
 */
function isSelectedSource(provider: AtlasCodeProviderView): boolean {
  return (
    (provider.kind === 'minimax-oauth' || provider.kind === 'minimax-api-key') && provider.active
  );
}

function markerFor(provider: AtlasCodeProviderView): string {
  if (provider.kind === 'codex-oauth') {
...
function providerModelList(provider: AtlasCodeProviderView): string | undefined {
...
function providerDetail(provider: AtlasCodeProviderView): string {
  if (provider.kind === 'codex-oauth') {
...
function providerSummary(provider: AtlasCodeProviderView): string {
  if (provider.kind === 'codex-oauth') {
