...
import {
  ATLASCODE_MAX_ATTACHMENT_COUNT,
  ATLASCODE_MAX_TOTAL_ATTACHMENT_BYTES,
...
export class TuiComposerDraft {
  ...
  private canAddAttachment(
    ...
    if (current.length >= ATLASCODE_MAX_ATTACHMENT_COUNT) {
      if (warn) {
        this.options.append(
    ...
    if (totalBytes + attachment.sizeBytes > ATLASCODE_MAX_TOTAL_ATTACHMENT_BYTES) {
      if (warn) {
        this.options.append(
