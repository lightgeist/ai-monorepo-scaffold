import type { AtlasCodeProviderApplication } from '../../../provider/application.js';
import type {
  AtlasCodeCodexOAuthLoginMethod,
...
export class TuiCodexLogin implements Component {
  ...
  private method: AtlasCodeCodexOAuthLoginMethod = 'browser';
  private status: AtlasCodeCodexOAuthStatus | undefined;
  ...
  constructor(private readonly options: CodexLoginOptions) {
    ...
    this.methods.onSelect = (item) => {
      void this.start(item.value as AtlasCodeCodexOAuthLoginMethod);
    };
  ...
  private async start(method: AtlasCodeCodexOAuthLoginMethod): Promise<void> {
    this.method = method;
    this.phase = 'starting';
  ...
  private update(status: AtlasCodeCodexOAuthStatus): void {
    if (this.cancelling) return;
    this.status = status;
    this.error = undefined;
