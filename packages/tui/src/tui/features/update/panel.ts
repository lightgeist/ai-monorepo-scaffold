...
import type {
  AtlasCodeUpdateApplyOptions,
  AtlasCodeUpdateOutcome,
  AtlasCodeUpdatePlan,
} from '../../../update/application.js';
import {
  isAtlasCodeUpdateAdmissionError,
  isAtlasCodeUpdateCancelledError,
  type AtlasCodeUpdatePhase,
...
type ActionableAtlasCodeUpdatePlan = Extract<
  AtlasCodeUpdatePlan,
  { kind: 'available' | 'package-manager' }
>;

type AtlasCodeUpdatePanelState =
  | { status: 'review' }
  | {
      status: 'applying';
      startedAtMs: number;
  ...
  | { status: 'succeeded'; outcome: AtlasCodeUpdateOutcome };

export interface TuiUpdatePanelOptions {
  readonly plan: ActionableAtlasCodeUpdatePlan;
  readonly maxRows: number | (() => number);
  readonly apply: (
    plan: ActionableAtlasCodeUpdatePlan,
...
export class TuiUpdatePanel implements Component, Focusable {
  private state: AtlasCodeUpdatePanelState = { status: 'review' };
  ...
  private async apply(): Promise<void> {
    ...
    try {
      ...
      if (this.disposed) return;
      if (isAtlasCodeUpdateCancelledError(error)) this.setState({ status: 'cancelled' });
      else if (isAtlasCodeUpdateAdmissionError(error)) {
        ...
        const presentation = presentTuiFailure(error, {
          ...
          preservation: 'The previous AtlasCode version is still active.',
        });
  ...
  private async restart(): Promise<void> {
    ...
    try {
      ...
      const presentation = presentTuiFailure(error, {
        summary: "AtlasCode couldn't restart automatically.",
        nextStep: 'Close this terminal and start AtlasCode again.',
  ...
  private renderPaused(width: number): string[] {
    const reason = this.state.status === 'paused' ? this.state.reason : 'AtlasCode is busy.';
    return [
  ...
  private renderCancelled(width: number): string[] {
    return [
      ...
      ...renderWrapped('The previous AtlasCode installation remains active.', width, colors.muted),
      '',
  ...
  private renderSuccess(width: number): string[] {
    ...
    return [
      renderDecisionHeading(
        `✓ AtlasCode updated to ${this.options.plan.latestVersion}`,
        undefined,
        width,
        'success',
      ),
      ...
      chalk.bold.hex(colors.signal)(
        this.restarting ? '⠋ Restarting AtlasCode…' : '› Restart AtlasCode now',
      ),
  ...
  private acceptPhase(phase: AtlasCodeUpdatePhase, cancellable: boolean): void {
    if (this.disposed || this.state.status !== 'applying') return;
    this.setState({ ...this.state, phase, cancellable });
  }

  private setState(state: AtlasCodeUpdatePanelState): void {
    this.state = state;
...
function sourceLabel(plan: ActionableAtlasCodeUpdatePlan): string {
