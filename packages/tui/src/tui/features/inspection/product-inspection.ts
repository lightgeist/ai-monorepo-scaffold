...
export function createTuiAccountStatusInspection(
  ...
  const accountLabel = options.accountLoading
    ...
    : needsLogin
      ...
      : identity
        ? safeInline(identity)
        : status === 'ready'
          ? 'Managed provider connected'
          : status === 'warning'
            ? 'Connected with warnings'
            : 'Managed provider account unavailable';
  ...
  return {
    title: `AtlasCode status${options.version ? ` · v${safeInline(options.version)}` : ''}`,
        ...
        ? [
            {
              ...
              rows: [
                {
                  label: 'Current run',
                  value: 'AtlasCode may use built-in defaults until the config is fixed and restarted.',
...
function configurationNextStep(diagnostics: TuiRuntimeDiagnostics): string {
  ...
  if (diagnostics.warnings.some((issue) => issue.includes('does not exist'))) {
    return `Create or restore ${path}, restart AtlasCode, then run /doctor again.`;
  }
  if (diagnostics.warnings.some((issue) => issue.includes('cannot be read'))) {
    return `Check access to ${path}, restart AtlasCode, then run /doctor again.`;
  }
  if (diagnostics.warnings.some((issue) => issue.includes('defaultModel'))) {
    return `Set defaultModel in ${path} to an available provider/model, restart AtlasCode, then run /doctor again.`;
  }
  return `Fix ${path}, restart AtlasCode, then run /doctor again.`;
}
