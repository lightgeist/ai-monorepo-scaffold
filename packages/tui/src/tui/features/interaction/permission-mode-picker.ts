...
type VisiblePermissionMode = (typeof ATLASCODE_PERMISSION_MODES)[number];
...
export class TuiPermissionModePicker implements Component {
  ...
  constructor(
    ...
    const currentIndex = ATLASCODE_PERMISSION_MODES.indexOf(
    ...
    if (matchesKey(data, 'up')) {
      this.selectedIndex =
        this.selectedIndex === 0
          ? ATLASCODE_PERMISSION_MODES.length - 1
          : this.selectedIndex - 1;
      this.requestRender();
      return;
    }
    if (matchesKey(data, 'down')) {
      this.selectedIndex = (this.selectedIndex + 1) % ATLASCODE_PERMISSION_MODES.length;
      this.requestRender();
      return;
    }
    if (/^[1-3]$/u.test(data)) {
      const mode = ATLASCODE_PERMISSION_MODES[Number(data) - 1];
      if (mode) this.onSelect(mode);
      return;
    }
    if (matchesKey(data, 'enter')) {
      const mode = ATLASCODE_PERMISSION_MODES[this.selectedIndex];
      if (mode) this.onSelect(mode);
      return;
    }
    ...
    return renderQuestionnaireFrame(
      {
        ...
        body: [
          chalk.hex(colors.muted)(
            compact ? 'Choose a tool access policy.' : 'Choose how AtlasCode handles tool access.',
          ),
          '',
          ...ATLASCODE_PERMISSION_MODES.flatMap((mode, index) =>
            renderMode(mode, {
