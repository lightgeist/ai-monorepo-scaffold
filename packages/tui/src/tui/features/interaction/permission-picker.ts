  ...
  constructor(
    ...
    const decisions: Array<{
          ...
          ]),
      {
        ...
        label:
          request.allowAlwaysSupported === false
            ? '2 Deny and guide AtlasCode'
            : '3 Deny and guide AtlasCode',
      },
    ];
  ...
  private buildFrameModel(width: number): PermissionFrameModel {
    ...
    if (this.denyFeedback) {
      ...
      return {
        ...
        trailing: [
          '',
          chalk.hex(colors.muted)('Add guidance for AtlasCode, or leave this blank.'),
          ...this.denyInput
            .render(Math.max(1, contentWidth))
...
function permissionDecisionDetail(decision: TuiPermissionDecision): string {
  ...
  return 'AtlasCode asks again in a new conversation';
}
  ...
  const candidates =
    decision === 'allowOnce'
      ...
      : decision === 'allowAlways'
        ...
        : [`${prefix} Deny and guide AtlasCode`, `${prefix} Deny and guide`, `${prefix} Deny`];
  return (
