...
import { resolveAtlasCodeAuthEnvironment } from '../auth/environment.js';
...
import {
  ...
  type CreateAtlasCodeBusinessTelemetryOptions,
  type AtlasCodeBusinessTelemetry,
...
import {
  ...
  type AtlasCodeOAuthCore,
} from '@atlascode/oauth-core';
import {
  resolveTuiManagedBackendLane,
  resolveTuiStartupEnvironmentOption,
} from '../cli/environment.js';
import { resolveAtlasCodeStartupUpdateNotice } from '../update/startup-notice.js';
import type { AtlasCodeUpdateApplication } from '../update/application.js';
import { tuiErrorDiagnostic } from '../user-facing-failure.js';
import { getConfig, resetConfig, writeTuiStatusLineSetting, type AtlasCodeRegion } from '@atlascode/config';
...
import { schedulePendingAtlasCodePrefixUpdate } from '../update/prefix-update.js';
import { ATLASCODE_TUI_RESULT_PATH_ENV } from './automation/result-writer.js';
import { startTuiStartupStatus, type TuiStartupStatus } from './startup-status.js';

const ATLASCODE_EXIT_SLOGAN = 'Intelligence with everyone, bye~';
...
export interface LaunchTuiDependencies {
  ...
  ) => Promise<Pick<AtlasCodeUpdateApplication, 'inspect' | 'apply'>>;
  ...
  createBusinessTelemetry?: typeof createAtlasCodeBusinessTelemetry;
  ...
  createSharedAuthSession?: typeof createAtlasCodeSharedAuthSession;
  createAuthApplication?: typeof createDefaultAtlasCodeAuthApplication;
}

export function createConfiguredTuiBusinessTelemetry(options: {
  readonly configEnabled: boolean;
  readonly environment: NodeJS.ProcessEnv;
  readonly telemetryOptions: CreateAtlasCodeBusinessTelemetryOptions;
  readonly createTelemetry?: typeof createAtlasCodeBusinessTelemetry;
}): AtlasCodeBusinessTelemetry | undefined {
  const policy = resolveAtlasCodeBusinessTelemetryPolicy({
    configEnabled: options.configEnabled,
    environment: options.environment,
  });
  if (!policy.enabled) return undefined;
  return (options.createTelemetry ?? createAtlasCodeBusinessTelemetry)(options.telemetryOptions);
}
...
export async function launchTui(
  ...
  if (!options.terminal && (!process.stdin.isTTY || !process.stdout.isTTY)) {
    throw new Error('AtlasCode interactive mode requires a TTY.');
  }
  ...
  const authEnvironment = resolveAtlasCodeAuthEnvironment({
  ...
  const sharedAuthCore: AtlasCodeOAuthCore = (
    dependencies.createSharedAuthSession ?? createAtlasCodeSharedAuthSession
  )({
    dataDir,
    ...authEnvironment,
    oauthEndpoints: resolveAtlasCodeOAuthEndpointConfig(process.env, authEnvironment),
  });
  ...
  const loadUpdateApplication =
    ...
    (async (currentVersion: string) => {
      const { AtlasCodeUpdateApplication } = await import('../update/application.js');
      return new AtlasCodeUpdateApplication({ currentVersion });
    });
  ...
  let restartRegion: AtlasCodeRegion | undefined;
    ...
    try {
      ...
      app = (dependencies.createApp ?? createTuiApp)({
        ...
        ...(presentationConfig.notifications
          ...
          : {}),
        ...(process.env[ATLASCODE_TUI_RESULT_PATH_ENV]
          ? { automationResultPath: process.env[ATLASCODE_TUI_RESULT_PATH_ENV] }
          : {}),
        ...
        auth: (dependencies.createAuthApplication ?? createDefaultAtlasCodeAuthApplication)({
          dataDir,
          ...authEnvironment,
          sharedAuthCore,
          ...(businessTelemetry ? { telemetry: businessTelemetry } : {}),
          telemetrySource: 'atlascode_tui',
        }),
        ...
        checkForUpdate: async () =>
          resolveAtlasCodeStartupUpdateNotice(await (await updateApplication()).inspect()),
        ...
        report: (error) => {
          try {
            process.stderr.write(
              `AtlasCode TUI stopped unexpectedly: ${tuiErrorDiagnostic(error)}. Restart AtlasCode; if it keeps happening, report it through an available support channel.\n`,
            );
          } catch {
      ...
      try {
        runtimeShutdownFailed = await lifecycle.shutdownTuiRuntime(runtime, {
          reportFailure: (step, error) => {
            ...
            try {
              process.stderr.write(
                `[atlascode] ${step} cleanup failed: ${tuiErrorDiagnostic(error)}\n`,
              );
            } catch {
              // The terminal may already be disconnected.
            }
...
export async function restartTuiProcess(
  ...
  if (
    await schedulePendingAtlasCodePrefixUpdate(
      process.argv[1],
      process.execPath,
      process.pid,
      args,
...
export function resolveRestartEnvironment(
  ...
  return region ? markLoginRestartHandoff({ ...environment, ATLASCODE_RUNTIME_REGION: region }) : environment;
}
...
export function resolveRestartArguments(
  ...
  if (!entryFile || !isExistingFile(entryFile)) {
    throw new Error('Unable to restart AtlasCode because its Node.js entry file is unavailable.');
  }
