...
export class AtlasCodeProviderApplication {
  constructor(private readonly port: AtlasCodeProviderRuntimePort) {}

  async snapshot(
    options: { readonly includeCodexOAuth?: boolean } = {},
  ): Promise<AtlasCodeProviderSnapshot> {
  ...
  setMiniMaxSource(source: AtlasCodeMiniMaxModelSource): Promise<AtlasCodeMiniMaxModelSource> {
    return this.port.setMiniMaxModelSource(source);
  }

  connectCodexOAuth(options?: AtlasCodeCodexOAuthLoginOptions): Promise<AtlasCodeCodexOAuthStartResult> {
  ...
  async create(input: AtlasCodeCreateProviderInput): Promise<void> {
    await this.port.createUserModelProvider(input);
  }

  saveCandidate(input: AtlasCodeSaveProviderCandidateInput): Promise<AtlasCodeSaveProviderCandidateResult> {
    return this.port.saveUserModelProviderCandidate(input);
  }

  async refreshModels(provider: AtlasCodeProviderView): Promise<number> {
    if (
  ...
  async update(input: AtlasCodeUpdateProviderInput): Promise<void> {
  ...
  test(providerId: string, modelId?: string): Promise<AtlasCodeProviderTestResult> {
    return modelId
...
function normalizeCodexOAuthProvider(status: AtlasCodeCodexOAuthStatus): AtlasCodeProviderView {
  return {
...
function normalizeCustomProvider(provider: AtlasCodeRuntimeProviderView): AtlasCodeProviderView {
