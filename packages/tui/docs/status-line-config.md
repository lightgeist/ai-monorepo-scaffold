...
The panel manages ordinary items only. For automation, edit the current profile's `config.yaml` (default in this distribution: `~/.atlascode/config.yaml`):
...
Restart atlascode after saving to enable the `[V]` line. `build-mode` is not in the ordinary selector. When enabled, the panel only explains how to edit configuration and restart; it does not switch the machine protocol or result channel.

## Configuration location

- File: `config.yaml` in the runtime data directory, normally `~/.atlascode/config.yaml`.
...
- Tips are enabled by default. Disable them in `~/.atlascode/config.yaml`:
