 ...
 *   - rewrittenInput (rm → atlascode-trash) survives the ask branch
...
function seedWindowsTrashRuntime(dataDir: string): void {
  ...
  writeFileSync(path.join(binDir, 'atlascode-trash.js'), TRASH_SCRIPT_WIN_CONTENT, 'utf8');
  writeFileSync(path.join(binDir, 'atlascode-trash.cmd'), TRASH_CMD_LAUNCHER_CONTENT, 'utf8');
}
...
describe('LocalPermissionFacade', () => {
  ...
  it('logs the command and detail for a top-level Bash safety decision', async () => {
    ...
    const { facade, dataDir } = freshFacade({ policyOwner: 'core' });
    try {
      const command = 'echo smoke > /tmp/atlascode-permission-smoke.txt';
      const result = await facade.checkPermission({
        toolName: 'bash',
    ...
    async (policyOwner) => {
      ...
      try {
        ...
        expect(result).toMatchObject({
          behavior: 'allow',
          rewrittenInput: { command: expect.stringContaining('atlascode-trash') },
        });
      } finally {
        cleanup(dataDir);
      }
  ...
  describe('POSIX host trash boundary', () => {
    ...
    it('routes an eligible rm through the textual rewrite in off mode', async () => {
      ...
      try {
        ...
        expect(result.behavior).toBe('allow');
        expect(result.rewrittenInput?.command).toContain('atlascode-trash -- ./old.txt');
        ...
        // runs in-cage, reaching atlascode-trash through the PATH shim.
  ...
  describe('Windows delete boundary', () => {
        ...
        for (const descriptorRedirect of ['2>&1', '1>&2', '*>&1']) {
          ...
          try {
            const r = await facade.checkPermission({
              toolName: 'bash',
              input: {
                command: String.raw`cmd /c "rmdir /S /Q \"X:\atlascode workspace\md2wechat-skill\"" ${descriptorRedirect}; if (Test-Path 'X:\atlascode workspace\md2wechat-skill') { Write-Host "STILL_EXISTS" } else { Write-Host "DELETED" }`,
              },
            });
            ...
            expect(r.reason).toContain('trusted atlascode-trash launcher');
    ...
    it.each([
      ...
      async (command) => {
        ...
        try {
          ...
          expect(r.rewrittenInput?.command).toBe('atlascode-trash --');
    ...
    it('fails closed for a proven native delete when the Windows trash runtime is unavailable', async () => {
      ...
      try {
        ...
        expect(r.behavior).toBe('deny');
        expect(r.reason).toMatch(/atlascode-trash|trash.*unavailable|安全删除/i);
        expect(r.reason).toContain('Deletion intent detected');
        ...
        expect(r.reason).toContain('Re-check atlascode-trash availability before retrying');
        expect(r.reason).toContain('use only the trusted local-runtime launcher');
        expect(r.reason).toContain('If atlascode-trash remains unavailable');
    ...
    it('rewrites an in-workspace rm to atlascode-trash even when permission mode is off', async () => {
      const workspaceDir = String.raw`C:\Users\16379\project`;
      const { facade, dataDir } = freshFacade({
        permissionMode: 'off',
        platform: 'win32',
      ...
      try {
        ...
        expect(r.behavior).toBe('allow');
        expect(r.rewrittenInput?.command).toMatch(/atlascode-trash/);
        ...
        const bareTrash = await facade.checkPermission({
          toolName: 'bash',
          agentName: 'atlascode',
          input: { command: String.raw`atlascode-trash -rf C:\Users\16379\project\mock` },
        });
        expect(bareTrash.behavior).toBe('deny');
        expect(bareTrash.reason).toContain('bare or non-canonical atlascode-trash is untrusted');
    ...
    it('does not trust an arbitrary same-named Windows trash launcher', async () => {
      ...
      try {
        ...
        const r = await facade.checkPermission({
          ...
          agentName: 'atlascode',
          input: {
            command: String.raw`C:\Users\16379\Downloads\atlascode-trash.cmd -rf C:\Users\16379\project\mock`,
          },
        });
        expect(r.behavior).toBe('deny');
        expect(r.reason).toContain('bare or non-canonical atlascode-trash is untrusted');
        expect(r.reason).toContain(path.win32.join(dataDir, 'bin', 'atlascode-trash.cmd'));
      } finally {
        cleanup(dataDir);
      }
    });

    it('does not trust a bare atlascode-trash.cmd resolved from the current directory', async () => {
      const workspaceDir = String.raw`C:\Users\16379\project`;
      const { facade, dataDir } = freshFacade({
        permissionMode: 'off',
        platform: 'win32',
      ...
      try {
        ...
        const r = await facade.checkPermission({
          toolName: 'bash',
          agentName: 'atlascode',
          input: { command: String.raw`atlascode-trash.cmd -rf C:\Users\16379\project\mock` },
        });
        expect(r.behavior).toBe('deny');
        expect(r.reason).toContain('bare or non-canonical atlascode-trash is untrusted');
        expect(r.reason).toContain(path.win32.join(dataDir, 'bin', 'atlascode-trash.cmd'));
    ...
    it.each([
      String.raw`cmd /c "atlascode-trash -- C:\Users\16379\project\mock"`,
      String.raw`powershell.exe -NoProfile -Command "atlascode-trash -- C:\Users\16379\project\mock"`,
      String.raw`cmd /c "atlascode-trash -rf .\mock"`,
    ])('does not trust bare atlascode-trash after wrapper expansion: %s', async (command) => {
      const workspaceDir = String.raw`C:\Users\16379\project`;
      const { facade, dataDir } = freshFacade({
        permissionMode: 'off',
        platform: 'win32',
    ...
    it('explains how to retry a direct compound atlascode-trash invocation safely', async () => {
    ...
    it('does not let a canonical launcher authorize a following bare trash command', async () => {
      ...
      try {
        ...
        const r = await facade.checkPermission({
          ...
          agentName: 'atlascode',
          input: {
            command: `"${launcherPath}" -- ${String.raw`C:\Users\16379\project\a`} && atlascode-trash.cmd -- ${String.raw`C:\Users\16379\project\b`}`,
          },
        });
        expect(r.behavior).toBe('deny');
        expect(r.reason).toContain('bare or non-canonical atlascode-trash is untrusted');
  ...
  describe('sandbox self-paths (dataDir auto-allow)', () => {
    // Reads of atlascode's own runtime data (skills, sessions, agent configs)
    ...
    it('read outside dataDir + outside workspace still ASKs (sandbox is targeted)', async () => {
      ...
      try {
        ...
        // The sandbox allow is for atlascode's own dataDir only. A random
  ...
  describe('rewrittenInput (rm → atlascode-trash)', () => {
    it('preserves rewrittenInput and exposes the complete production execution plan', async () => {
      ...
      try {
        ...
        // The rm → atlascode-trash rewrite is engine-side and exposed via
        ...
        expect(r.executionPlan).toMatchObject({
          ...
          effectiveInput: { command: expect.stringContaining('atlascode-trash'), timeout: 9 },
