...
const originalBuildEnv = process.env.ATLASCODE_BUILD_ENV;

afterEach(() => {
  vi.restoreAllMocks();
  if (originalBuildEnv === undefined) delete process.env.ATLASCODE_BUILD_ENV;
  else process.env.ATLASCODE_BUILD_ENV = originalBuildEnv;
});
