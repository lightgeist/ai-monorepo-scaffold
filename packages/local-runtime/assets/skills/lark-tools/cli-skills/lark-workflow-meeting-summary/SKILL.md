...
仅支持 **user 身份**。缺权限时按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 canonical 模板补：缺基础 scope（如 vc / drive 常用）先跑 `--recommend` 拿推荐全集；缺推荐之外的高敏感 scope（如 `vc:meeting.search:read`）跑 `--scope` 增量补。两者都通过 agent 自己 `--no-wait --json` + 后台 `--device-code`，用户只点授权链接。
