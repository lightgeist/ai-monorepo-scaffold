...
### Listen for messages and reply with the configured assistant
...
lark-cli event +subscribe \
  ...
  | while IFS= read -r line; do
      ...
      answer=$(codex exec --ephemeral --ignore-user-config --sandbox read-only --skip-git-repo-check \
        "Reply concisely: $content" < /dev/null 2>/dev/null)
