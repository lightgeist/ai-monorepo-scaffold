...
| Permission denied (99991679) | Missing `im:chat:update` permission | Follow `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 canonical template to top up the `im:chat:update` scope (agent runs `--no-wait --json` itself; user only clicks the auth link) |
