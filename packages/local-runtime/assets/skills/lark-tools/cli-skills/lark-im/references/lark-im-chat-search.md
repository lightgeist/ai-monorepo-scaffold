...
| Permission denied (99991679) with `--as user` | UAT is not authorized for `im:chat:read` | Follow `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 canonical template to top up the `im:chat:read` scope (agent runs `--no-wait --json` itself; user only clicks the auth link) |
