...
- **认证** — 列出所需 scope。缺权限时让 agent 按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 canonical 模板补：缺基础 scope 跑 `--recommend`，缺推荐之外的高敏感 scope 跑 `--scope <name>`，两者都通过 agent 自己 `--no-wait --json` + 后台 `--device-code`。文档里**指向 canonical 模板**，不要重复贴整条 `lark-cli auth login` 命令，也不要再教 `--domain`。
