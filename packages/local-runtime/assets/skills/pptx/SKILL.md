...
| PPTX preview selection edit context (`atlascode.ppt_artifact_edit_request.v1`) | Read `references/preview-selection-writeback.md`; use the targeted workflow there, not the template-wide editing workflow |
...
When creating a new PPTX from scratch for AtlasCode, also write an optional sidecar element map:

```text
slides/output/<descriptive-name>.atlascode-ppt-map.json
```
...
`references/element-map.md` and `scripts/atlascode-ppt-map.cjs` to record stable `elementId`, role,
...
7. For AtlasCode-generated decks, write `slides/output/<descriptive-name>.atlascode-ppt-map.json` as a sidecar using `references/element-map.md`.
...
6. For AtlasCode-generated decks, write `slides/output/<descriptive-name>.atlascode-ppt-map.json` as a sidecar using `references/element-map.md`.
...
- `references/element-map.md`: optional sidecar metadata for AtlasCode-generated decks — **must not replace the PPTX output contract**
