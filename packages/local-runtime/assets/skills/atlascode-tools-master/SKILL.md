---
name: atlascode-tools-master
description: >-
  You must load this skill before running any `atlascode-tools` Bash command. The `atlascode-tools` CLI is
...
requiresBeta: atlascodeTools
descriptions:
  zh-Hans:
    '调用任何 atlascode-tools Bash 命令前必须先加载本 skill。`atlascode-tools` CLI 已在 PATH 中，可通过 Bash
    直接调用。它既是 Connector 工具与 Bash
    脚本、管道、本地文件、循环或批处理组合使用的主要入口，也是图片/照片、视频、音频、音乐和文档等多模态生成与理解的主要入口；已连接插件或
    MCP 工具的普通单次调用直接使用模型工具列表。'
displayNames:
  zh-Hans: 'atlascode-tools Connector'
---

# atlascode-tools Master

Use the `atlascode-tools connector` subcommands through the `bash` tool. The launcher is already on the
...
```bash
atlascode-tools get_asset_url <node_id>
# Equivalent kebab-case spelling:
atlascode-tools get-asset-url <node_id>
```
...
```bash
atlascode-tools upload_temp_url <file_path>
# Equivalent kebab-case spelling:
atlascode-tools upload-temp-url <file_path>
# Optional MIME type override:
atlascode-tools upload-temp-url <file_path> --mime-type <type>
```
...
```json
{
  "command": "atlascode-tools connector call <tool_name> --args-file <path>",
  "timeout": 600
}
```

`timeout` is a top-level `bash` tool input field outside the command string. Never put `--timeout`,
`timeout=600`, or any other timeout argument in the command string: `atlascode-tools connector call` has
...
1. Discover candidate tools. List all tools only when the task is broad; otherwise narrow the list
   ...
   ```bash
   atlascode-tools connector tools
   atlascode-tools connector tools --keyword <text>
   ```

   Do not run `atlascode-tools list` or `atlascode-tools connector list`; those are not valid discovery
   commands.
...
2. Inspect the selected tool before calling it. Use the returned `input_schema` to construct the
   ...
   ```bash
   atlascode-tools connector tool <tool_name>
   ```
...
3. Call the tool with exactly one JSON object. Use inline JSON for small arguments and a file for
   ...
   ```bash
   atlascode-tools connector call <tool_name> --args '{"key":"value"}'
   atlascode-tools connector call <tool_name> --args-file <path>
   atlascode-tools connector call <tool_name> --args-file -
   ```

   `--args` and `--args-file` are mutually exclusive. Omitting both sends `{}`. Arrays, scalars, and
   `null` are not valid top-level arguments.

4. A successful `atlascode-tools connector call` directly writes the top-level JSON value described by
