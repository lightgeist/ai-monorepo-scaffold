...
# macOS / Linux / WSL
DOCX_ROOT="<skill_dir>"
CLI="dotnet run --project $DOCX_ROOT/scripts/dotnet/AtlasCodeDocx.Cli --"
```

```powershell
# Windows
$env:DOCX_ROOT = "<skill_dir>"
$CLI = "dotnet run --project $env:DOCX_ROOT\scripts\dotnet\AtlasCodeDocx.Cli --"
```
...
```bash
# macOS / Linux / WSL
test -d "$DOCX_ROOT" || { echo "FATAL: docx skill root missing" >&2; exit 1; }
...
```powershell
# Windows
if (-not (Test-Path $env:DOCX_ROOT)) { Write-Error "FATAL: docx skill root missing"; exit 1 }
...
```bash
$DOCX_ROOT/scripts/dotnet/AtlasCodeDocx.Core/Samples/*.cs
```
