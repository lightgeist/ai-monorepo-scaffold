...
if [[ "$LEVEL" == "full" ]]; then
  ...
  if [ ! -d "$DOTNET_DIR" ]; then
    ...
    if [ -f "$DOTNET_DIR/AtlasCodeDocx.Cli/bin/Debug/net10.0/AtlasCodeDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/AtlasCodeDocx.Cli/bin/Debug/net9.0/AtlasCodeDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/AtlasCodeDocx.Cli/bin/Debug/net8.0/AtlasCodeDocx.Cli.dll" ]; then
...
if [ "$STATUS" = "READY" ]; then
  ...
  if [ "$LOCALE_HEALED" -eq 1 ]; then
    ...
    echo "  To make UTF-8 sticky for the AtlasCode daemon and its workers:"
    if [ "$OS" = "macos" ]; then
    ...
    echo "  Note: editing ~/.zshrc does NOT help — AtlasCode subprocesses are non-interactive"
  ...
  if [ "$LOCALE_BROKEN" -eq 1 ]; then
    ...
    if [ "$OS" = "macos" ]; then
      echo "  macOS (persist for GUI apps + AtlasCode daemon; survives reboot once set in launchd):"
      echo "    launchctl setenv LANG en_US.UTF-8"
      echo "    launchctl setenv LC_ALL en_US.UTF-8"
      echo "  Then restart the daemon so it inherits the new env."
    elif [ "$OS" = "linux" ] || [ "$OS" = "wsl" ]; then
