...
import { getWorkspaceImagePreviewMimeType } from '@atlascode/shared/media-asset-meta';
import { getDataDir } from '@atlascode/config';
...
const RC_REQUEST_SOURCE_HEADER = 'X-AtlasCode-Request-Source';
const RC_REQUEST_SOURCE_VALUE = 'remote-control';
const RC_FILE_CONTENT_MAX_BYTES_HEADER = 'X-AtlasCode-Remote-Control-File-Content-Max-Bytes';
  ...
  if (method === 'GET' && tail === 'thumbnail') {
    ...
    return new Response(Uint8Array.from(thumbnail.bytes), {
      headers: {
        ...
        'X-AtlasCode-Thumbnail-Cache': thumbnail.cache,
      },
    });
  }
