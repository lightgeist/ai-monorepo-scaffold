import { getConfig, isManagedRuntime } from "@atlascode/config";
...
export function createLocalRuntimeHost(
  ...
  // (Electron, CLI, daemon).
  process.env.ATLASCODE_DATA_DIR = dataDir;
  ...
  const apiHostOptions = copyV2LayoutMigrationReceipt(options, {
    ...
    atlascodeCronAdapterProvider: options.atlascodeCronAdapterProvider,
...
function shouldUseRemoteSkillHub(
  ...
  if (process.env.__ATLASCODE_RUNTIME_MANAGED === "0") return false;
  return (
