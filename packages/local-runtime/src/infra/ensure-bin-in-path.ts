 ...
 * `atlascode-trash` and `atlascode`.
 *
 * `getShellEnv()` (third_party/pi-mono) spreads `process.env` and prepends
 * `~/.pi/agent/bin`, but does NOT add the atlascode data-dir bin. Rather than
...
export function ensureBinInProcessPath(dataDir: string): void {
  ...
  // Prepend so atlascode-trash takes priority over any stale copy elsewhere.
