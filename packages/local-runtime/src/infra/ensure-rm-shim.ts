...
import { resolveBashEnvPolicy, type BashEnvPolicy } from '@atlascode/agent-core/bash-subprocess-env';

/**
 * Delegates to the sibling `../bin/atlascode-trash` rather than an absolute path so
 * the shim keeps working if the dataDir is moved or mounted elsewhere.
 *
 * `exec` replaces the shim process, so the caller observes atlascode-trash's own
 * exit status and stderr directly — an `rm` failure must stay an `rm` failure.
 * Arguments are forwarded verbatim: atlascode-trash already accepts rm's surface
 * (`-f`, `-r`/`-R`, `-rf`, `--`) and mirrors `rm -f`'s empty-operand success.
 */
export const RM_SHIM_CONTENT = `#!/bin/bash
# Managed by AtlasCode. Do not edit.
#
...
trash_bin="$shim_dir/../bin/atlascode-trash"
if [ ! -x "$trash_bin" ]; then
      ...
      try {
        ...
        throw new Error(
          `The recoverable-delete rm shim at ${shimPath} is missing or not executable and could not be re-seeded. ` +
            ...
            'Check that the AtlasCode data directory is writable, then restart the app to re-seed the shim.',
          { cause },
        );
      }
    },
