...
import { isCanonicalSubagentRole } from '@atlascode/agent-tools/desktop/subagent-roles';
...
export function resolveFeatureAwareBuiltinSkillNames(
  ...
  if (capabilities.features.atlascode) {
    selected.add('atlascode');
    selected.add('create-agent');
    selected.add('atlascode-product');
  }
  if (
...
function isStandaloneSkillUsable(
  ...
  switch (skillName) {
    ...
    case 'lark-tools':
    case 'llm-call':
    case 'atlascode-tools-master':
    case 'atlascode-doctor':
