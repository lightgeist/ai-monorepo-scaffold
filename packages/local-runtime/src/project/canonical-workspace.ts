...
import { LEGACY_DATA_DIR_BASENAME, NEW_DATA_DIR_BASENAME } from '@atlascode/config/data-dir';
...
/**
 * Match the one-way read alias from the historical `.atlascode[-profile]` data
...
function parseAgentWorkspacePath(
  ...
  const match = dataDirPart?.match(/^\.(minimax|mavis|atlascode)(?:-(.+))?$/i);
...
/**
 * Parser-only: lexically parses absolute `<parent>/.{atlascode|minimax|mavis}[-profile]/sessions/<sessionId>/workspace`
...
function parseSessionDefaultWorkspacePath(
  ...
  const dataDirPart = parts.at(-4);
  const dataDirMatch = dataDirPart?.match(/^\.(minimax|mavis|atlascode)(?:-(.+))?$/i);
