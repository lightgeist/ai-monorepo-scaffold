...
import type { LocalAtlasCodeCronAdapter } from "@atlascode/agent-tools/desktop";
...
import type { GlobalEventPayloadMap } from "@atlascode/shared/global-events";
...
export interface LocalRuntimeApiHostOptions {
  ...
  /** Composition overrides injected by the Cron v2 owner for legacy Local AtlasCode commands. */
  atlascodeCronAdapterProvider?: () => LocalAtlasCodeCronAdapter | undefined;
  ...
  skillRegistryRoots?: import("@atlascode/skills").SkillSourceRoot[];
   ...
   * from `@atlascode/local-runtime` so the SDK does not enter the Electron main
