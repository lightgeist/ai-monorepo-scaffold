...
import { getConfig } from "@atlascode/config";
...
import {
  ...
  createLocalAtlasCodeAgentAdapter,
...
import { buildAtlasCodeSessionAdapter } from "../runtime/atlascode-tool-adapters.js";
...
const DEFAULT_LOCAL_AGENT_DISPLAY_NAME = "AtlasCode";
...
export class LocalRuntimeApiHost {
  ...
  private readonly atlascodeCronAdapterProvider: LocalRuntimeApiHostOptions["atlascodeCronAdapterProvider"];
  ...
  constructor(options: LocalRuntimeApiHostOptions = {}) {
    ...
    this.atlascodeCronAdapterProvider = options.atlascodeCronAdapterProvider;
  ...
  buildOwnerTurnToolSources(
    ...
    return buildLocalTurnToolSourcesForHost({
      ...input,
      host: this,
      atlascodeAgentAdapter: createLocalAtlasCodeAgentAdapter(this.agentRuntimePort),
      atlascodeCronAdapter: this.buildOwnerAtlasCodeCronAdapter(),
      atlascodeSessionAdapter: this.buildOwnerAtlasCodeSessionAdapter(),
    });
  }
  ...
  private buildOwnerAtlasCodeCronAdapter() {
    return this.atlascodeCronAdapterProvider?.();
  }
  /**
   * Mirrors the two conditions `buildLocalTurnToolSourcesForHost` uses to admit
   * `atlascodeCronAdapter` into a Turn. Kept in sync so reminder guidance never
  ...
  private isCronCapabilityAvailable(): boolean {
    ...
    return this.buildOwnerAtlasCodeCronAdapter() !== undefined;
  }
  private isUserMemoryCapabilityAvailable(): boolean {
    return this.lockOwner.ownerKind !== "tui";
  }
  private buildOwnerAtlasCodeSessionAdapter() {
    const conversation = this.requireRuntimeConversation(
      "AtlasCode Session adapter",
    );
    return buildAtlasCodeSessionAdapter({
