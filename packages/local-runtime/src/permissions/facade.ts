 ...
 *     (`bash(npm publish:*)`), rewrittenInput (`rm` → `atlascode-trash`), and
...
function isAtlasCodeTrashExecutable(command: string): boolean {
 ...
 * / `rd`, POSIX `unlink` / `shred`, or the recoverable `atlascode-trash`
...
function isDeleteLikeSegment(surface: string): boolean {
  return (
    ...
    isAtlasCodeTrashExecutable(surface)
  );
}
...
function hasUntrustedAtlasCodeTrashExecutable(command: string, launcherPath: string): boolean {
  const expectedLauncher = normalizeWindowsExecutablePath(launcherPath);
  return commandSurfaces(command).some((surface) => {
    if (!isAtlasCodeTrashExecutable(surface)) return false;
 ...
 * rewrite into `atlascode-trash <path>` further down the pipeline, or the exact
 * canonical `atlascode-trash.cmd` absolute path emitted by that rewrite.
 *
 * Without a per-segment audit, a compound command like
 *   "<dataDir>\bin\atlascode-trash.cmd" -- .\a && unlink .\b
...
function everyDeleteSegmentIsSafelyRewritable(
  ...
  return deleteSurfaces.every((surface) => {
    ...
    // the rm rewrite emits. Bare `atlascode-trash` / any other basename is
    // NOT accepted here (Windows CWD-first resolution risk).
    if (isAtlasCodeTrashExecutable(surface)) {
      const executable = readLeadingExecutable(surface)!;
...
export interface LocalPermissionFacadeDeps {
  ...
  /** Test-only trash health probe override; production invokes atlascode-trash.cmd. */
...
export class LocalPermissionFacade {
  ...
  async checkPermissionRaw(
    ...
    //   - rewrittenInput (rm → atlascode-trash)
  ...
  /**
   * Self-paths that atlascode auto-allows for READ-ONLY tools without
   * prompting. **Write-side tools always go through the normal review
   * flow even on these paths.**
   *
   * Rationale: atlascode's own runtime data (`cfg.dataDir`) and the user-wide
   ...
   * ask flow — the user sees a card for every atlascode-internal write,
  ...
  private resolveSandboxAllowPaths(toolName: string, workingDirectory: string): string[] {
    ...
    candidates.push(path.resolve(homedir(), '.atlascode'));
  ...
  private async evaluateWindowsHardSafety(
    ...
    const recoverableDeleteInstruction =
      ...
      'Retry only through a simple, top-level recoverable deletion that the runtime can route through its trusted atlascode-trash launcher.';
    const unavailableTrashInstruction =
      ...
      'Re-check atlascode-trash availability before retrying and use only the trusted local-runtime launcher. ' +
      'If atlascode-trash remains unavailable, tell the user that the target must be removed manually.';
    ...
    if (command) {
      ...
      if (permanentDeleteReason) {
        return {
          denial: {
            behavior: 'deny',
            reason: `Local hard safety policy blocked ${params.toolName}: ${permanentDeleteReason}. Use atlascode-trash for recoverable removal; permanent Windows delete commands are not allowed. ${recoverableDeleteInstruction}`,
      ...
      if (isDeleteLikeCommand(command)) {
        ...
        if (!trash.available) {
          return {
            denial: {
              behavior: 'deny',
              reason: `Local hard safety policy blocked deletion: the Windows atlascode-trash launcher is unavailable (${trash.reason ?? 'health check failed'}). The agent must not fall back to a permanent delete command. ${unavailableTrashInstruction}`,
              denySource: 'safety-immune',
            },
          };
        }
        const launcherPath = path.win32.join(dataDir, 'bin', 'atlascode-trash.cmd');
        if (hasUntrustedAtlasCodeTrashExecutable(command, launcherPath)) {
          return {
            denial: {
              behavior: 'deny',
              reason: `Local hard safety policy blocked deletion: bare or non-canonical atlascode-trash is untrusted because PATH/CWD may select a different executable. The target was not deleted. Retry with one top-level rm command; the runtime will use the trusted launcher at "${launcherPath}". Do not bypass this with a permanent delete command, wrapper, script, or filesystem API.`,
              denySource: 'safety-immune',
            },
          };
        }
        if (!everyDeleteSegmentIsSafelyRewritable(command, launcherPath, shellFamily)) {
          return {
            denial: {
              behavior: 'deny',
              reason: `Local hard safety policy blocked a wrapped or compound Windows delete command that cannot be safely rewritten. Use the trusted local-runtime atlascode-trash launcher for recoverable removal. ${recoverableDeleteInstruction}`,
              denySource: 'safety-immune',
            },
          };
        }
