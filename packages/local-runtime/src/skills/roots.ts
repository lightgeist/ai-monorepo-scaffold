...
import type { SkillSourceKind, SkillSourceRoot } from '@atlascode/skills';
...
function readExternalWorkspaceSkillRoots(
  ...
  return roots.flatMap((rootPath, index) => {
    ...
    return [
      externalSkillRoot(
        'workspace-minimax',
        join(rootPath, '.atlascode', 'skills'),
        'workspace',
        skillsConfig,
        nearestPriorityBonus,
      ),
      ...(existsSync(join(rootPath, '.minimax', 'skills')) ? [externalSkillRoot(
        'workspace-minimax',
        join(rootPath, '.minimax', 'skills'),
        'workspace',
        skillsConfig,
        nearestPriorityBonus - 0.01,
      )] : []),
      externalSkillRoot(
        'workspace-cc',
...
function externalSkillRoot(
  ...
  return {
    // Retain the stored source-policy key for compatibility; expose the current product name.
    id: `external:${source === 'workspace-minimax' ? 'workspace-atlascode' : source}:${rootPath}`,
    kind,
    rootPath,
