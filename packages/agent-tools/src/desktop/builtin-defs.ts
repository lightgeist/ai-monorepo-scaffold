...
import { createAtlasCodeOperationClassifier } from '../shared/atlascode-operation-classifier.js';
import { LOCAL_ATLASCODE_COMMANDS } from './local-atlascode-commands.js';
import { prepareAskUserArguments } from './prepare-ask-user-arguments.js';

import {
  LOCAL_ATLASCODE_AGENT_NAME_DESCRIPTION,
...
- Continue the child asynchronously with task_append using task_id; read
  task_output with the returned task_id. If the native atlascode tool is available,
...
still exists and is not archived. Use the native atlascode tool with command
...
const LocalAtlasCodeArgsSchema = Type.Object(
  {
    ...
    agent_name: Type.Optional(
      Type.String({
        description: `${LOCAL_ATLASCODE_AGENT_NAME_DESCRIPTION} Required for agent get/update/delete; optional filter for session list. For cron list, optional owner filter; for cron create/once, see session for owner selection.`,
      }),
    ),
    name: Type.Optional(
      Type.String({
    ...
    include_primary: Type.Optional(
      Type.Boolean({
        description: 'agent list: include the primary AtlasCode agent. Defaults to false.',
      }),
    ),
...
export const LocalAtlasCodeToolDef = {
  name: 'atlascode',
  executionMode: 'sequential',
  operationClassifier: createAtlasCodeOperationClassifier(LOCAL_ATLASCODE_COMMANDS),
  description: `Manage local desktop AtlasCode agents and their services. Use "<group> help" for command details and examples.
...
session — local desktop conversations
  ...
  - For the built-in atlascode agent, report when the user asks, returns after time away, or a meaningful cross-session change matters to them. On return, open with a brief status snapshot; surface other changes once at an appropriate moment.
  ...
  - For unfamiliar outcomes, call session messages with the target session_id and limit: 5 for the built-in atlascode agent, or limit: 3 for other agents.
...
mcp help — Show MCP command details.`,
    ...
    args: Type.Optional(LocalAtlasCodeArgsSchema),
  }),
} as const satisfies ToolDefinition;
export type LocalAtlasCodeToolInput = Static<typeof LocalAtlasCodeToolDef.schema>;
