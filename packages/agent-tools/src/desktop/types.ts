import type { RuntimeTool, ToolExecutionContext, ToolResult } from '@atlascode/agent-core/tools';
...
import type {
  ...
  TaskQuery,
} from '@atlascode/background-task';
...
export interface LocalAtlasCodeAgentDetail {
  name?: string;
  displayName?: string;
  agentRole?: string;
...
export interface LocalAtlasCodeAgentReadScope {
  canonicalName: string;
}

export interface LocalAtlasCodeAgentAdapter {
  listAgents(
    req: {
      limit?: number;
      offset?: number;
    ...
    signal?: AbortSignal,
  ): Promise<{ agents?: LocalAtlasCodeAgentDetail[]; baseResp?: Record<string, unknown> }>;
  createAgent(
    req: {
      name?: string;
      displayName?: string;
  ...
  getAgent(
    ...
    signal?: AbortSignal,
  ): Promise<{ agent?: LocalAtlasCodeAgentDetail; baseResp?: Record<string, unknown> }>;
  updateAgent(
    req: {
      name: string;
      displayName?: string;
    ...
    success?: boolean;
    agent?: LocalAtlasCodeAgentDetail;
  ...
  resolveAgentReadScope(
    ...
    signal?: AbortSignal,
  ): Promise<LocalAtlasCodeAgentReadScope>;
...
export type LocalAtlasCodeMcpTransport = 'stdio' | 'http' | 'streamable-http' | 'sse';

export interface LocalAtlasCodeMcpCreateRequest {
  name: string;
  transport: LocalAtlasCodeMcpTransport;
  command?: string;
  url?: string;
  args?: string[];
...
export interface LocalAtlasCodeMcpUpdateRequest {
  name: string;
  transport?: LocalAtlasCodeMcpTransport;
  command?: string;
  url?: string;
...
/**
 * MCP settings seam for the model-visible `atlascode` tool. Implementations own
 * persistence and redaction; model-visible return values must never contain
 * environment-variable or header values.
 */
export interface LocalAtlasCodeMcpAdapter {
  ...
  createServer(req: LocalAtlasCodeMcpCreateRequest, signal?: AbortSignal): Promise<{ server: unknown }>;
  updateServer(req: LocalAtlasCodeMcpUpdateRequest, signal?: AbortSignal): Promise<{ server: unknown }>;
  deleteServer(req: { name: string }, signal?: AbortSignal): Promise<{ success: boolean }>;
}

export type LocalAtlasCodeCommandInput = LocalAtlasCodeToolInput;

export type LocalAtlasCodeCronSessionTarget =
  | { readonly mode: 'new' }
  | { readonly mode: 'sessionId'; readonly sessionId: string };

export type LocalAtlasCodeCronCreationSessionTarget =
  | { readonly mode: 'new' }
  | { readonly mode: 'sessionId'; readonly sessionId?: string };

export interface LocalAtlasCodeCronTask {
  cronId?: string;
  cronName?: string;
  agentName?: string;
  schedule?: string;
  ...
  enabled?: boolean;
  prompt?: string;
  session?: LocalAtlasCodeCronSessionTarget;
 ...
 */
export interface LocalAtlasCodeCronRun {
  runId: string;
  sessionId?: string;
  createdAt: number;
  deliveredAt?: number;
 ...
 */
export interface LocalAtlasCodeCronLegacySession {
  runId?: never;
  triggerSource?: never;
  sessionId?: string;
  createdAt?: number | string;
...
export type LocalAtlasCodeCronHistoryItem = LocalAtlasCodeCronRun | LocalAtlasCodeCronLegacySession;

export type LocalAtlasCodeModelResolution =
  | { readonly kind: 'resolved'; readonly model: string }
  | { readonly kind: 'ambiguous'; readonly candidates: readonly string[] }
  | { readonly kind: 'not_found' };

export interface LocalAtlasCodeCronAdapter {
  resolveModel?(
    req: { model: string; sessionId?: string },
    signal?: AbortSignal,
  ): Promise<LocalAtlasCodeModelResolution>;
  listCrons(
    req: { agentName?: string; cursor?: string; limit?: number },
    signal?: AbortSignal,
  ): Promise<{
    tasks: LocalAtlasCodeCronTask[];
    count?: number;
    hasMore?: boolean;
    nextCursor?: string;
  }>;
  getCron(req: { cronId: string }, signal?: AbortSignal): Promise<{ task?: LocalAtlasCodeCronTask }>;
  createCron(
    req: {
      agentName: string;
      cronName: string;
      ...
      timezone?: string;
      activeHours?: { start?: string; end?: string };
      session: LocalAtlasCodeCronCreationSessionTarget;
      enabled?: boolean;
      project?: string;
      model?: string;
    },
    signal?: AbortSignal,
  ): Promise<{ task?: LocalAtlasCodeCronTask }>;
  createSelfReminder(
    req: {
      agentName: string;
      sessionId: string;
  ...
  triggerCron(
    ...
    triggeredAt?: string;
    createdAt?: number;
    status?: LocalAtlasCodeCronRun['status'];
    triggerSource?: LocalAtlasCodeCronRun['triggerSource'];
    errorCode?: string;
    reason?: string;
  }>;
  listCronSessions(
    ...
    sessions: LocalAtlasCodeCronHistoryItem[];
    total?: number;
    hasMore?: boolean;
    nextCursor?: string;
  }>;
...
export interface LocalAtlasCodeSessionInfo {
  sessionId?: string;
  agentName?: string;
...
export interface LocalAtlasCodeSessionAdapter {
  listSessions(
    req: {
      agentName?: string;
    ...
    signal?: AbortSignal,
  ): Promise<{ sessions?: LocalAtlasCodeSessionInfo[]; hasMore?: boolean; nextCursor?: string }>;
  getSession(
    req: { sessionId: string; source?: 'local' | 'cloud' },
    signal?: AbortSignal,
  ): Promise<{ session?: LocalAtlasCodeSessionInfo }>;
  updateSession(
    req: { sessionId: string; title?: string; archived?: boolean },
    signal?: AbortSignal,
  ): Promise<{ session?: LocalAtlasCodeSessionInfo; success?: boolean }>;
