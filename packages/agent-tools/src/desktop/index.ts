import { toRuntimeTool } from '@atlascode/agent-core/tools';
...
import { LocalAtlasCodeTool } from './local-atlascode.js';
...
export * from './local-atlascode.js';
export * from './local-atlascode-commands.js';
export * from './local-atlascode-cron-adapter.js';
...
export function buildLocalToolRegistry(deps: LocalToolRegistryDeps): Map<string, LocalRuntimeTool> {
  ...
  if (deps.atlascodeAgentAdapter) {
    tools.push(
      toRuntimeTool(
        new LocalAtlasCodeTool(
          deps.atlascodeAgentAdapter,
          deps.atlascodeCronAdapter,
          deps.atlascodeSessionAdapter,
          deps.atlascodeMcpAdapter,
        ),
      ),
    );
  }
