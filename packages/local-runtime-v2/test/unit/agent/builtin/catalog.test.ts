...
describe("BuiltinAgentCatalog recoverable deletion guidance", () => {
  it("teaches AtlasCode to use the runtime-owned recoverable deletion path", async () => {
    ...
    for (const appMode of ["coding", "work"] as const) {
      ...
      expect(rendered.corePrompt).toContain("/bin/atlascode-trash");
      expect(rendered.corePrompt).toMatch(
        /never guess it or use\s+bare `atlascode-trash`/u,
      );
      expect(rendered.corePrompt).toMatch(
...
describe("BuiltinAgentCatalog Explore composition", () => {
  it("keeps Explore Bash boundaries in V1 and V2 assets and task reuse in V1 delegation", async () => {
    ...
    const [v1ExplorePrompt, v2ExplorePrompt, delegation] = await Promise.all([
      ...
      readFile(
        join(assetsDir, "atlascode", "features", "delegation.md.hbs"),
        "utf8",
      ),
    ]);
...
describe("BuiltinAgentCatalog composition", () => {
  ...
  it("avoids duplicating the shared AtlasCode mode layer while retaining Windows and worker bases", async () => {
    const root = await mkdtemp(
      join(tmpdir(), "local-runtime-v2-agent-shared-base-"),
    );
    fixtureDirectories.push(root);
    await mkdir(join(root, "atlascode", "modes", "coding", "online"), {
      recursive: true,
    });
    await mkdir(join(root, "worker"), { recursive: true });
    await mkdir(join(root, "_default"), { recursive: true });
    await writeFile(join(root, "builtin-agents.json"), '["atlascode", "worker"]');
    await writeFile(
      join(root, "atlascode", "modes", "coding", "online", "SYSTEM.md.hbs"),
      "## Harness\nAtlasCode system",
    );
    await writeFile(
      join(root, "atlascode", "modes", "coding", "online", "PERSONA.md"),
      "AtlasCode persona",
    );
    ...
    const atlascode = await catalog.render({ ...common, agentName: "atlascode" });
    expect(atlascode.corePrompt).toBe("## Harness\nAtlasCode system\n\nwindows base");
    expect(atlascode.corePrompt.match(/^## Harness$/gmu)).toHaveLength(1);
    await expect(
...
describe("BuiltinAgentCatalog localized assets", () => {
  it("uses English built-in personas while keeping greeting assets localized", async () => {
    ...
    expect(canonicalBuiltinName(" MAIN ")).toBe("atlascode");
...
describe("BuiltinAgentCatalog validation and fallback rendering", () => {
  ...
  it("renders primary internal-channel fallback and resolves capability intersections", async () => {
    ...
    await mkdir(join(root, "_default"), { recursive: true });
    await writeFile(join(root, "builtin-agents.json"), '["atlascode"]');
    await writeFile(
      join(root, "atlascode", "modes", "coding", "online", "SYSTEM.md.hbs"),
      "online",
    );
    ...
    await writeFile(
      join(root, "atlascode", "modes", "coding", "online", "PERSONA-zh.md"),
      "马维斯",
    );
    await writeFile(
      join(root, "_default", "prompt-base-all.md"),
    ...
    ).resolves.toMatchObject({ corePrompt: "online", persona: "AtlasCode" });

    const resolved = resolveCanonicalCapabilities(
      {
        persona: { enabled: true },
        tools: ["read", "write"] as never,
        features: { atlascode: true, delegation: true, webSearch: true },
      },
    ...
    expect(resolved).toMatchObject({
      ...
      tools: ["read", "write"],
      features: { atlascode: true, delegation: false, webSearch: true },
    });
    expect(
    ...
    expect(legacyNamesFor("atlascode")).toEqual(["main", "mavis"]);
...
describe("BuiltinAgentCatalog TodoWrite lifecycle guidance", () => {
  it("leaves TodoWrite lifecycle guidance to the tool description", async () => {
    ...
    const [atlascode, work, worker, withoutTodoWrite] = await Promise.all([
      catalog.render({
        ...common,
        agentName: "atlascode",
        capabilities: resolveAgentCapabilities(),
    ...
    for (const rendered of [atlascode, work, worker, withoutTodoWrite]) {
...
describe("BuiltinAgentCatalog session surfaces", () => {
  ...
  it("uses the complete coding template for CLI while preserving capability gates", async () => {
    ...
    expect(cliZh.corePrompt).not.toContain("You are AtlasCode");
  ...
  it("keeps legacy role names out of model-visible AtlasCode onboarding assets", async () => {
    const catalog = new BuiltinAgentCatalog();
    ...
    const systemPrompt = await readFile(
      join(root, "atlascode", "system-prompt.md.hbs"),
      "utf8",
    );
  ...
  it("keeps product knowledge in the AtlasCode Skill catalog instead of duplicating it in mode prompts", async () => {
    const catalog = new BuiltinAgentCatalog();
    const root = await catalog.resolveAssetsDir();
    const common = {
      agentName: "atlascode",
    ...
    const [coding, work, productSkill] = await Promise.all([
      ...
      catalog.render({ ...common, appMode: "work", promptMode: "work" }),
      readFile(
        join(root, "atlascode", "skills", "atlascode-product", "SKILL.md"),
        "utf8",
      ),
    ]);

    for (const rendered of [coding, work]) {
      expect(rendered.corePrompt).not.toContain("atlascode-product");
      expect(rendered.corePrompt).not.toContain(
    ...
    expect(productSkill).not.toMatch(
      /filecdn\.atlascode\.chat|file\.cdn\.atlascode\.io/i,
    );
    expect(productSkill).not.toMatch(
    ...
    expect(productSkill).not.toMatch(
      /国内 Changelog[^\n]*https:\/\/agent\.atlascode\.cn\/docs\/changelog(?!\.md)/i,
    );
    expect(productSkill).not.toMatch(
      /海外 Changelog[^\n]*https:\/\/agent\.atlascode\.io\/docs\/changelog(?!\.md)/i,
    );
...
describe("BuiltinAgentCatalog persona assets", () => {
  it("keeps identity and Core Judgment in the complete coding template", async () => {
    const catalog = new BuiltinAgentCatalog();
    const root = await catalog.resolveAssetsDir();
    const persona = await readFile(join(root, "atlascode", "PERSONA.md"), "utf8");
    ...
    expect(persona).toContain("display_name: AtlasCode");
    expect(persona).toContain("AtlasCode");
    expect(persona).not.toContain("You are AtlasCode");
...
describe("BuiltinAgentCatalog prompt modes", () => {
  ...
  it("reads V2 without V1 feature files and rejects a missing selected mode", async () => {
    ...
    const catalog = new BuiltinAgentCatalog({ assetsDir: root });
    await rm(join(root, "atlascode/features"), { recursive: true });
    const input = {
      ...common,
      promptMode: "work" as const,
      capabilities: resolveAgentCapabilities(),
