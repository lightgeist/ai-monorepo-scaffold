...
import { formatDefaultAgentAvatarMarker } from "@atlascode/shared/agent-avatar";
import { serializeAgentReference } from "@atlascode/shared/agent-mention";
...
describe("Local Agent reference resolver integration", () => {
  it("authorizes a freshly persisted coder for a trusted interactive AtlasCode turn", async () => {
...
function registerConfigDocumentPublicationTests(): void {
  ...
  it("publishes a Custom Agent as canonical agent.md without recreating legacy assets", async () => {
    ...
    await repository.insert({
      ...
      agentRole: "orchestrator",
      creationSource: "builtin",
      defaultWorkspaceDir: "/workspace/atlascode",
      createdAtMs: 20,
      updatedAtMs: 20,
    });
    ...
    await expect(repository.list()).resolves.toMatchObject([
      { name: "atlascode", agentRole: "orchestrator" },
...
function registerBuiltinCanonicalPublicationTests(): void {
  it.each(["tui", "coding", "work"] as const)(
    ...
    async (promptMode) => {
      ...
      const canonical = await repository.getBuiltinCanonicalConfig("atlascode");
      ...
      expect(profile.corePrompt).toContain(
        promptMode === "tui"
          ? "You are a coding agent running in the AtlasCode terminal"
          : "You run inside AtlasCode",
      );
      ...
      expect(frozen.corePrompt).not.toContain(
        "user's active AtlasCode terminal conversation",
      );
      ...
      expect(await repository.getBuiltinCanonicalConfig("atlascode")).toEqual(
        canonical,
      );
    },
  );

  it("rebuilds a legacy partial-policy Builtin with the resolver-provided M3 thinking group and removes stale AtlasCode/main files", async () => {
    const { repository, dataDir } = await createRepository();
    const atlascodeBuiltin = join(
      dataDir,
      "agents",
      ".builtin",
      "atlascode",
    ...
    await mkdir(join(dataDir, "agents", ".builtin", "atlascode"), {
      recursive: true,
    });
    ...
    await writeFile(atlascodeBuiltin, "stale atlascode config");
    ...
    await expect(readFile(atlascodeBuiltin, "utf8")).resolves.toContain(
      "name: atlascode",
    );
    ...
    await expect(
      readFile(join(dataDir, "agents", "atlascode", "agent.md"), "utf8"),
    ...
    expect(rebuilt).toMatchObject({
      ...
      model: "minimax/MiniMax-M3",
      effort: "on",
      xAtlasCode: { contextWindow: 64_000, maxOutputTokens: 2_048 },
  ...
  it("persists only model-group changes through a Builtin raw Config PUT", async () => {
    ...
    expect(updated).toMatchObject({
      ...
      configured: {
        model: "openai/gpt-6",
        effort: "low",
        atlascode: { contextWindow: 65_536, maxOutputTokens: 8_192 },
  ...
  it("creates a UI definition without a route name using a generated durable owner", async () => {
    ...
    const created = await application.createDefinition({
      ...
      initialDefinition: {
        ...
        description: "",
        model: "minimax/MiniMax-M3",
        atlascode: { displayName: "测试删除", avatar: PNG_DATA_URL },
        systemPrompt,
      },
    });
    ...
    await expect(document).resolves.toMatchObject({
      ...
      configured: {
        ...
        description: "测试删除",
        model: "minimax/MiniMax-M3",
        atlascode: { displayName: "测试删除", avatar: "./avatar.png" },
        systemPrompt,
      },
    });
    await expect(
...
describe("DrizzleAgentRepository legacy canonical materialization safety", () => {
  it("fills only a missing canonical display name from the V2 target identity", async () => {
    ...
    const original = [
      ...
      "  unknownAtlasCodeField: keep-atlascode-field",
      "---",
      "",
      "Keep system prompt.",
      "",
    ...
    expect(reconciled).toContain("unknownAtlasCodeField: keep-atlascode-field");
    ...
    await expect(repository.getCanonicalConfig(name)).resolves.toMatchObject({
      xAtlasCode: { displayName: "New V2 Value" },
    });
  });
  ...
  it("discovers file-only Custom candidates without borrowing reserved or Builtin names", async () => {
    ...
    for (const name of ["file-only", "atlascode", "builtin-owned"]) {
      const agentDir = repository.getAgentDir(name);
      await mkdir(agentDir, { recursive: true });
    ...
    expect(candidates.map((candidate) => candidate.name)).not.toEqual(
      expect.arrayContaining(["atlascode", "builtin-owned"]),
    );
    await expect(
      repository.materializeLegacyCustomAgent("atlascode"),
...
describe("DrizzleAgentRepository primary family provenance recovery", () => {
  ...
  it("normalizes mixed historical primary rows without adding or deleting either owner", async () => {
    ...
    expect(
      database.rawDb
        .prepare(
          "SELECT agent_name, agent_role, creation_source, main_session_id, created_at, updated_at FROM agents WHERE agent_name IN ('main', 'atlascode') ORDER BY agent_name",
        )
        .all(),
    ).toEqual([
      {
      ...
      {
        agent_name: "atlascode",
        agent_role: 1,
        creation_source: "builtin",
        main_session_id: "atlascode-root",
        created_at: 20,
        updated_at: 21,
      },
...
function registerCanonicalSourceOwnershipTests(): void {
  it("never reconciles file-only main or atlascode definitions into Custom runtime rows", async () => {
    const { repository, database } = await createRepository();
    const files = new Map([
      ["main", "---\nname: main\ndescription: Main\n---\n\nmain prompt\n"],
    ...
    expect(
      database.rawDb
        .prepare(
          "SELECT agent_name FROM agents WHERE agent_name IN ('main', 'atlascode')",
        )
        .all(),
    ).toEqual([]);
    for (const [name, contents] of files) {
  ...
  it("keeps trusted Builtins ahead of same-named Custom root files", async () => {
    ...
    await repository.writeCanonicalConfig("worker", {
      ...
      xAtlasCode: { displayName: "Untrusted Custom" },
...
function registerCanonicalSourceCutoverTests(): void {
  ...
  it("stops consuming legacy Custom assets after the canonical cutover", async () => {
    ...
    expect(newConfig.xAtlasCode?.defaultWorkspaceDir).toBeUndefined();
  });
}
  ...
  it.each(["on", "off"] as const)(
    ...
    async (effort) => {
      ...
      const markdown = [
        ...
        "  unknownAtlasCodeField: UNKNOWN_ATLASCODE_SENTINEL",
        "---",
        "",
        systemPrompt.trim(),
        "",
      ...
      await expect(repository.getCanonicalConfig(owner)).resolves.toMatchObject(
        {
          ...
          diagnostics: [
            ...
            { code: "unsupported_agent_field", field: "unknownTopLevel" },
            {
              ...
              field: "x-atlascode.unknownAtlasCodeField",
            },
          ],
        },
      );
      ...
      expect(live.corePrompt).not.toContain("UNKNOWN_ATLASCODE_SENTINEL");
...
describe("DrizzleAgentRepository canonical Custom profile editing", () => {
  it("edits Custom identity metadata through the service without rewriting prompt, unknown YAML, or SQLite config", async () => {
    ...
    expect(config.xAtlasCode?.displayName).toBeUndefined();
    expect(config.xAtlasCode?.avatar).toMatch(/^\.\/avatar-[a-f0-9]{16}\.png$/u);
    const updated = await readFile(agentFile, "utf8");
    expect(updated).toContain("unknownTopLevel: retained");
    expect(updated).toContain("unknownAtlasCode: retained");
