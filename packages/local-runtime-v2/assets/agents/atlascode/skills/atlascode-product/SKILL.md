---
name: atlascode-product
description: >-
  Use this skill to route questions about the AtlasCode or AtlasCode product itself:
  ...
  pricing, quotas; and AtlasCode image, audio, music, or video capabilities.
  Treat references such as "AtlasCode", "AtlasCode", "atlascode", "atlascode tui", or
...
descriptions:
  zh-Hans: >-
    这是所有 AtlasCode 或 AtlasCode 产品问题的首层总路由 Skill。凡是询问产品身份、归属、
...
# AtlasCode product identity and routing

## Stable facts

- AtlasCode is Deep Intuition's agentic coding workspace; AtlasCode is its primary Agent.
- The public surfaces are Desktop/Electron, Web/H5, and AtlasCode CLI/TUI.
- The public terminal command is `atlascode`.

## Official source discovery

For every product question, first identify the runtime `region` from `Environment` or `<agent-context>`:

- `region: cn` means the current app/account is China. Prefer China-region AtlasCode and Open
  Platform official documentation.
- `region: en` means the current app/account is Global. Prefer Global-region AtlasCode and Open
  Platform official documentation.
...
| Runtime region | AtlasCode product/workflow/Agent/extension docs | Open Platform account/model/API/billing docs  |
| -------------- | -------------------------------------------------- | --------------------------------------------- |
...
- Product identity, Desktop/Web/CLI/TUI, workflows, Agents, Skills, Plugins, and MCP →
  current-region AtlasCode index.
- Token Plan, credits, pricing, API keys, model catalog, model API, and media API → current-region
  Open Platform index; use the current-region AtlasCode index too when the question concerns Code
