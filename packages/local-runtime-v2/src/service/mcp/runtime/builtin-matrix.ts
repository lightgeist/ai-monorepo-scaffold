...
import { resolveV2DirectoryContract } from '@atlascode/shared/local-runtime-paths';
...
// web_search is the only Matrix tool that atlascode-tools does NOT provide, so when
// atlascode-tools owns the media/generation Matrix tools the built-in Matrix MCP is
...
// Same stdio child, two layouts: packaged atlascode resolves next to package/cli.js,
...
export function buildBuiltinMatrixServerConfig(
  ...
  return {
    ...
    env: {
      ...runtimeEnv,
      ATLASCODE_MATRIX_WORKSPACE_ROOT: workspaceRoot,
      ...(context?.routingContext?.bedrockLane
        ? { ATLASCODE_MATRIX_BEDROCK_LANE: context.routingContext.bedrockLane }
        : {}),
    ...
    metadata: {
      atlascodeBuiltinMcpServer: BUILTIN_MATRIX_SERVER_NAME,
      atlascodeBuiltinMcpVersion: BUILTIN_MATRIX_MCP_VERSION,
...
export function isBuiltinMatrixConfig(server: string, config: LocalMcpServerConfig): boolean {
  return (
    ...
    config.builtin === true &&
    config.metadata?.['atlascodeBuiltinMcpServer'] === BUILTIN_MATRIX_SERVER_NAME &&
    config.metadata?.['atlascodeBuiltinMcpVersion'] === BUILTIN_MATRIX_MCP_VERSION
  );
}
...
export function isRetiredLegacyMatrixMcpServerConfig(
  ...
  return (
    config.metadata?.['atlascodeBuiltinMcpServer'] === BUILTIN_MATRIX_SERVER_NAME &&
    config.metadata?.['atlascodeBuiltinMcpVersion'] === LEGACY_MATRIX_MCP_VERSION
  );
}
  ...
  return {
    ...
    env: {
      ...
      ...(context?.routingContext?.bedrockLane
        ...
        : {}),
      ...resolveMatrixExtraInputRootsEnv(context),
      ATLASCODE_MATRIX_ACCESS_TOKEN: accessToken,
...
export function resolveMatrixMcpStdioEntrypoint(baseUrl = import.meta.url): string | undefined {
  const override = process.env.ATLASCODE_MATRIX_MCP_STDIO_ENTRYPOINT?.trim();
...
function resolveMatrixExtraInputRootsEnv(context?: LocalMcpRuntimeContext): Record<string, string> {
  ...
  if (!dataDir) return {};
  return { ATLASCODE_MATRIX_EXTRA_INPUT_ROOTS: resolveV2DirectoryContract(dataDir).assets };
}

function resolveMatrixWorkspaceRoot(context?: LocalMcpRuntimeContext): string {
  return (
    context?.workspaceRoot?.trim() ||
    process.env.ATLASCODE_MATRIX_WORKSPACE_ROOT?.trim() ||
...
function buildBuiltinMatrixRuntimeEnv(): Record<string, string> {
  ...
  return {
    ...
    ATLASCODE_RUNTIME_REGION: getRuntimeRegion(),
    ...(buildEnv ? { ATLASCODE_BUILD_ENV: buildEnv } : {}),
  };
}
...
function getBuiltinMatrixTools(
  ...
  // atlascode-tools provides every Matrix media/generation tool except web_search,
