...
export interface CanonicalAgentConfig {
  ...
  readonly xAtlasCode?: CanonicalAgentAtlasCodeConfig;
...
export interface CanonicalAgentAtlasCodeConfig {
...
const KNOWN_FIELDS = new Set([
  ...
  'skills',
  'x-atlascode',
  'x-mavis', // Historical canonical agent metadata; never emitted by new writes.
]);
const BUILTIN_KNOWN_FIELDS = new Set([...KNOWN_FIELDS, 'features']);
const BUILTIN_FEATURE_FIELDS = new Set(['atlascode', 'mavis', 'delegation', 'webSearch']);
const KNOWN_ATLASCODE_FIELDS = new Set([
  'displayName',
  'avatar',
  'contextWindow',
  'maxOutputTokens',
...
export async function readCanonicalAgentConfig(input: {
  ...
  if (parsed.xAtlasCode?.avatar) {
    await validateAgentAvatar(input.agentDir, parsed.xAtlasCode.avatar, input.trustedRoot);
  }
  return parsed;
}
...
export async function readBuiltinCanonicalAgentConfig(input: {
  ...
  if (parsed.config.xAtlasCode?.avatar) {
    await validateAgentAvatar(input.agentDir, parsed.config.xAtlasCode.avatar, input.trustedRoot);
  }
  return parsed;
}
...
function canonicalConfigFromSource(
  ...
  const xAtlasCode = parseAtlasCode((source['x-atlascode'] ?? source['x-mavis']));
  return {
    name,
    description,
    ...(model ? { model } : {}),
...
function diagnosticsForSource(
  ...
  const atlascodeSource = asPlainObject((source['x-atlascode'] ?? source['x-mavis']));
  if (atlascodeSource) {
    diagnostics.push(...unsupportedFieldDiagnostics(atlascodeSource, KNOWN_ATLASCODE_FIELDS, 'x-atlascode.'));
  }
...
function serializeAgentConfig(
  ...
  if (features) {
    frontmatter.features = {
      atlascode: features.atlascode,
      delegation: features.delegation,
      webSearch: features.webSearch,
    };
  }
  const xAtlasCode = serializeAtlasCode(config.xAtlasCode);
  if (xAtlasCode) frontmatter['x-atlascode'] = xAtlasCode;
...
function agentDirectoryRootKind(root: string): '.atlascode' | '.minimax' | '.mavis' | 'other' {
  const name = basename(root).toLowerCase();
  if (name === '.atlascode' || name.startsWith('.atlascode-')) return '.atlascode';
...
function parseAtlasCode(value: unknown): CanonicalAgentAtlasCodeConfig | undefined {
  if (value === undefined) return undefined;
  const source = asPlainObject(value);
  if (!source) throw invalid('x-atlascode', 'x-atlascode must be a mapping.');
  const defaultWorkspaceDir = canonicalWorkspace(source);
  const result = atlascodeFields(source, defaultWorkspaceDir);
...
function serializeAtlasCode(
  value: CanonicalAgentAtlasCodeConfig | undefined,
...
function requiredBuiltinFeaturePolicy(source: Record<string, unknown>): BuiltinAgentFeaturePolicy {
  ...
  return Object.freeze({
    atlascode: requiredBoolean(features, features.atlascode === undefined && features.mavis !== undefined ? 'mavis' : 'atlascode'),
...
function avatarInvalid(message: string): AgentConfigError {
  return new AgentConfigError('AGENT_CONFIG_AVATAR_INVALID', 'x-atlascode.avatar', message);
}
