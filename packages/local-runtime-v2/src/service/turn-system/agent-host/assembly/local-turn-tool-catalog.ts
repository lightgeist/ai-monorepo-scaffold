import type { RuntimeTool, RuntimeToolSource } from '@atlascode/agent-core/tools';
import {
...
import { AGENT_BUILTIN_MCP_TOOL_IDS, AGENT_BUILTIN_TOOL_IDS } from '@atlascode/config';
...
function isProfileToolAllowed(
  ...
  if (isBuiltinSubagentTaskChild(profile) && tool.def.name === 'atlascode') return false;
...
function isAuthoritativeMainProfile(profile: LocalTurnAgentProfileFacts | undefined): boolean {
  return (
    ...
    profile.trustedBuiltin === true &&
    profile.canonicalRole === 'atlascode'
  );
}
...
/**
 * SessionSend is a sub-command of the composite `atlascode` Tool, so it cannot be
 * removed from the catalog on its own. A builtin SubAgent running as a task
 * child therefore loses the whole `atlascode` Tool. The Agent profile resolver
 * already forces `features.atlascode` off for this surface; repeating the rule on
...
function isFeatureEnabled(ceiling: LocalAgentCapabilityCeiling, toolName: string): boolean {
  if (toolName === 'atlascode') return ceiling.features.atlascode;
...
export function resolveLocalMcpDisclosureOptions(
  ...
  return {
    enabled: resolveOption(env.ATLASCODE_MCP_TOOL_SEARCH_ENABLED, raw?.enabled, true, parseBoolean),
    modelWhitelist: resolveModelWhitelist(env.ATLASCODE_MCP_TOOL_SEARCH_WHITELIST, raw),
    thresholdPct: resolveOption(
      env.ATLASCODE_MCP_TOOL_SEARCH_THRESHOLD_PCT,
      raw?.thresholdPct,
      0.15,
      parseNumber,
    ),
    minDeferCount: resolveOption(
      env.ATLASCODE_MCP_TOOL_SEARCH_MIN_DEFER_COUNT,
      raw?.minDeferCount,
      1,
      parseNumber,
    ),
    topKDefault: resolveOption(
      env.ATLASCODE_MCP_TOOL_SEARCH_TOP_K_DEFAULT,
      raw?.topKDefault,
      5,
      parseNumber,
    ),
    topKMax: resolveOption(env.ATLASCODE_MCP_TOOL_SEARCH_TOP_K_MAX, raw?.topKMax, 20, parseNumber),
    systemHint: resolveOption(
      env.ATLASCODE_MCP_TOOL_SEARCH_SYSTEM_HINT,
      raw?.systemHint,
      true,
      parseBoolean,
    ),
    maxSchemaTextLen: resolveOption(
      env.ATLASCODE_MCP_TOOL_SEARCH_MAX_SCHEMA_TEXT_LEN,
