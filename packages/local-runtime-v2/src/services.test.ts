...
import { createAgentRuntime, type AgentExtension } from "@atlascode/agent-runtime";
import { baseCtx } from "../../agent-runtime/src/testing/fixtures.js";
import type { RuntimeTool } from "@atlascode/agent-core/tools";
import { LocalSkillToolDef } from "@atlascode/agent-tools/desktop";
import type {
  DatabaseLike,
...
vi.mock("@atlascode/config", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@atlascode/config")>()),
...
describe("runtime services composition", () => {
  it.each([false, true])(
    ...
    async (internal) => {
      vi.stubEnv("__ATLASCODE_BUILD_INTERNAL", String(internal));
      vi.stubEnv("__ATLASCODE_BUILD_INSIDE", "false");
      const services = await createRuntimeServices({
        db: {} as AppDb,
        dataDir: "/data",
        scheduler: {} as SchedulerClient,
...
describe("runtime services CLI composition", () => {
  ...
  it("captures TUI requests even though the host composes no transport", async () => {
    ...
    const globalFetch = vi.fn(async () => new Response("{}"));
    const originalFetch = globalThis.fetch;
    const originalOptIn = process.env.ATLASCODE_TUI_LLM_CONTEXT_INSPECTOR;
    globalThis.fetch = globalFetch as unknown as typeof fetch;
    process.env.ATLASCODE_TUI_LLM_CONTEXT_INSPECTOR = "1";
    ...
    try {
      ...
      if (originalOptIn === undefined)
        delete process.env.ATLASCODE_TUI_LLM_CONTEXT_INSPECTOR;
      else process.env.ATLASCODE_TUI_LLM_CONTEXT_INSPECTOR = originalOptIn;
    }
  });
});
  ...
  it("selects task-child prompts only for hidden task-purpose branches and preserves frozen read refs", async () => {
    mocked.agentService.renderProfile.mockImplementation(
      async (input: {
        ...
        ({
          requestRef: "main",
          resourceReadRef: "atlascode",
          exactOwnerName: input.exactOwnerName,
          canonicalViewName: "atlascode",
          resolvedAgentName: "atlascode",
      ...
      cases.map(() => [
        expect.objectContaining({
          ...
          capabilities: expect.objectContaining({
            features: expect.objectContaining({ atlascode: false }),
          }),
        }),
      ]),
    );
...
describe("runtime services lifecycle", () => {
  ...
  it("accepts v1 events through the injected write port once and detaches on close", async () => {
    ...
    await services.ready();
    publish?.({ type: "agent.created", payload: { agentName: "atlascode" } });
...
async function assertWorkspaceBashFileSourceResolution(): Promise<void> {
  ...
  expect(insidePatch?.content?.at(-1)).toEqual({
    ...
    text: expect.stringContaining(
      "Citation candidate: [demo.docx](#atlascode-source=file:L3dvcmtzcGFjZS9wcm9qZWN0L2RlbW8uZG9jeA)",
    ),
  });
...
describe("runtime services primary family execution identity", () => {
  ...
  function primaryFamilyViews(canonicalDisplayName: string): void {
    mocked.agentService.getPersistedOwnerForFrozenTask.mockImplementation(
      async (name) =>
        name === "atlascode"
          ? view("atlascode", canonicalDisplayName)
          : name === "main"
  ...
  it("keeps the main storage owner while sourcing identity from canonical atlascode", async () => {
    primaryFamilyViews("AtlasCode Canonical");
    const info = vi.fn();
    ...
    await expect(
      ...
      agentName: "main",
      executionOwnerName: "atlascode",
      resourceAgentName: "atlascode",
      displayName: "AtlasCode Canonical",
      metadata: { requestRef: "atlascode", canonicalViewName: "atlascode" },
    });
    await expect(
      sources.agents.getExecutionSnapshot("atlascode"),
    ).resolves.toMatchObject({
      agentName: "atlascode",
      executionOwnerName: "atlascode",
      resourceAgentName: "atlascode",
    ...
    expect(info).toHaveBeenCalledWith(
      { storage_owner: "main", execution_owner: "atlascode", outcome: "canonical" },
  ...
  it("renders the Turn profile from the resolved execution owner, not the storage owner", async () => {
    ...
    ).resolves.toMatchObject({ memoryReadAgentNames: ["atlascode", "main"] });
