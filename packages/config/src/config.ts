...
function isLegacyRuntimeEnvAllowed(): boolean {
  return process.env.__ATLASCODE_ALLOW_LEGACY_RUNTIME_ENV === "1";
}

// ---------------------------------------------------------------------------
// Runtime environment types & helpers (ATLASCODE_* plus Electron cold-start fallbacks)
// ---------------------------------------------------------------------------
...
// bootstrap has copied the build-time NEXT_PUBLIC_* values into ATLASCODE_*.
...
const ELECTRON_BUILD_ENV_TO_RUNTIME: Readonly<Record<string, AtlasCodeBuildEnv>> = {
  dev: "test",
  test: "test",
  staging: "staging",
  prod: "prod",
...
function isValidRuntimeRegion(value: string | undefined): value is AtlasCodeRegion {
  return value === "cn" || value === "en";
}

function isValidRuntimeBuildEnv(
  value: string | undefined,
): value is AtlasCodeBuildEnv {
  return (
    value === "test" ||
    value === "prod" ||
    value === "dev" ||
...
export function getRuntimeRegion(): AtlasCodeRegion {
  const val = process.env.ATLASCODE_RUNTIME_REGION;
...
export function getRuntimeBuildEnv(): AtlasCodeBuildEnv {
  const val = process.env.ATLASCODE_BUILD_ENV;
 ...
 *
 * Detection priority:
 *   1. Explicit __ATLASCODE_RUNTIME_MANAGED env var ('1'/'0') — for overrides
 *      - '1': force-enable (e.g. `__ATLASCODE_RUNTIME_MANAGED=1 pnpm dev` to test C-end locally)
 ...
 *   3. Auto-detect production build (ATLASCODE_BUILD_ENV=prod)
...
export function isManagedRuntime(): boolean {
  ...
  const explicit = process.env.__ATLASCODE_RUNTIME_MANAGED;
...
export type PresetKey = `${AtlasCodeRegion}-${AtlasCodeBuildEnv}`;
...
export interface BetaConfig {
   ...
   * the AtlasCode Browser Bridge extension (logged-in sites, persistent cookies,
  ...
  threadGoal: boolean;
  /**
   * Bundled atlascode-tools CLI and Electron Device Flow integration. Packaged
...
export interface BetaFeatureDef {
  ...
  defaultBuildEnvironments?: readonly AtlasCodeBuildEnv[];
...
export interface BetaFeatureResolutionContext {
  ...
  defaultBuildEnvironment?: AtlasCodeBuildEnv;
  internalBuild?: boolean;
  insideBuild?: boolean;
  platform?: NodeJS.Platform;
  atlascodeToolsDevMode?: "published";
}
...
function getBetaDefaultBuildEnvironment(): AtlasCodeBuildEnv {
...
export function resolveBetaFeature(
  ...
  const defaultBuildEnvironment =
    ...
    getBetaDefaultBuildEnvironment();
  if (
    feature === "atlascodeTools" &&
    defaultBuildEnvironment === "dev" &&
    context.atlascodeToolsDevMode === "published" &&
...
export function isInternalBuild(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.__ATLASCODE_BUILD_INTERNAL === "true";
}

function isInsideBuild(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.__ATLASCODE_BUILD_INSIDE === "true";
}
...
export interface CliSpawnConfig {
  /**
   * Expose `atlascode session spawn` CLI command.
   ...
   * AtlasCode itself and its agents do NOT need this — team plans spawn worker sessions
...
export interface MemoryConfig {
   ...
   * writes through AtlasCode APIs. Existing files are preserved and read/search
...
// ---------------------------------------------------------------------------
export const BRAND = {
  /** Data directory basename (e.g. '.atlascode'). */
  APP_DIR: ".atlascode",
...
export const RUNTIME_ENV_PREFIX = "__ATLASCODE_RUNTIME";
...
const REPO_NAMES: ReadonlySet<string> = new Set(["agent-archon", "atlascode"]);
...
function getExplicitPublicDataDirEnv(): string | undefined {
  return (
    process.env.ATLASCODE_DATA_DIR?.trim() || process.env.ATLASCODE_RUNTIME_DATA_DIR?.trim()
  );
}
 ...
 * Any branch in the atlascode repo gets a hash-based offset.
 * Default port 5321 is reserved for non-git / non-atlascode environments.
 */
...
export function getDataDir(): string {
  ...
  // 2. Git auto-detection (any branch in atlascode repo) → profile-scoped dir
  ...
  // 5. resolveDataDir() → non-git / non-atlascode default data directory
...
// ---------------------------------------------------------------------------
// Default provider presets per environment combo (ATLASCODE_RUNTIME_REGION x ATLASCODE_BUILD_ENV)
// ---------------------------------------------------------------------------
...
> = {
  ...
  opencode: {
    ...
    keepAlive: {
      ...
      agents: ["atlascode", "main", "verifier", "code-reviewer"],
    },
    spawnConcurrency: 3,
  },
  contextManagement: {
...
function parseBetaConfig(raw: Record<string, unknown>): BetaConfig {
  ...
  const context: BetaFeatureResolutionContext = {
    ...
    atlascodeToolsDevMode:
      process.env.ATLASCODE_DEV_ATLASCODE_TOOLS_MODE === "published"
        ? "published"
        : undefined,
  };
 ...
 * 1. Explicit profile-scoped port (__ATLASCODE_RUNTIME_PROFILE + __ATLASCODE_RUNTIME_PORT both set)
 * 2. Git auto-detection (hash-based offset from branch name in atlascode repo)
 * 3. __ATLASCODE_RUNTIME_PORT env var (only when git detection is not active)
 * 4. DEFAULT_PORT (5321)
 *
