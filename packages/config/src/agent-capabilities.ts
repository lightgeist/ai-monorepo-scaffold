...
const FEATURE_KEYS = new Set(['atlascode', 'delegation', 'webSearch']);
...
export function parseAgentCapabilityConfig(raw: unknown, path = 'agent'): AgentCapabilityConfig {
  ...
  if (obj.features !== undefined) {
    ...
    result.features = {
      ...(typeof features.atlascode === 'boolean' ? { atlascode: features.atlascode } : {}),
...
export function resolveAgentCapabilities(
  ...
  return {
    ...
    features: {
      atlascode: override.features?.atlascode ?? defaults.features?.atlascode ?? true,
