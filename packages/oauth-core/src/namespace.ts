...
import { ATLASCODE_OAUTH_CLIENT_ID, type AuthNamespaceInput } from './contracts.js';
...
export function createAuthNamespace(input: AuthNamespaceInput): AuthNamespace {
  ...
  const namespaceHome = join(authHome, input.buildEnv, input.region, ATLASCODE_OAUTH_CLIENT_ID);
  const service = `com.deepintuition.atlascode.oauth.${input.buildEnv}.${input.region}`;
  const account = createHash('sha256')
    .update(`${authHome}\0${ATLASCODE_OAUTH_CLIENT_ID}`)
    .digest('base64url');
