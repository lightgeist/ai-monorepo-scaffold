...
import {
  ATLASCODE_OAUTH_AUDIENCE,
  ATLASCODE_OAUTH_CLIENT_ID,
...
export class AuthRequiredError extends Error {
  ...
  constructor(options?: ErrorOptions) {
    super('AtlasCode authentication is required.', options);
...
export class AuthLoginCancelledError extends Error {
  ...
  constructor() {
    super('AtlasCode OAuth device authorization was cancelled.');
...
export class AuthScopeUpgradeRequiredError extends Error {
  ...
  constructor(readonly missingScopes: string[]) {
    super('The current AtlasCode authorization does not include the required scope.');
...
export class AuthResourceContractMismatchError extends Error {
  ...
  constructor() {
    super('The stored credential does not match the AtlasCode resource contract.');
...
export class AuthDomainConflictError extends Error {
  ...
  constructor(
    ...
    super(
      `AtlasCode is signed in to ${active.buildEnv}/${active.region}; sign out before using ${requested.buildEnv}/${requested.region}.`,
    );
    this.name = 'AuthDomainConflictError';
  }
}

export interface AtlasCodeOAuthCoreOptions {
...
export class AtlasCodeOAuthCore {
  ...
  constructor(private readonly options: AtlasCodeOAuthCoreOptions) {
  ...
  async getStatus(): Promise<AuthStatusSnapshot> {
    ...
    if (state.status === 'authenticated' || state.status === 'expired') {
      ...
      try {
        assertCredentialContract(credential, [...ATLASCODE_OAUTH_SCOPES]);
      } catch (error) {
  ...
  async handleUnauthorized(context: UnauthorizedContext): Promise<'retry' | 'logout'> {
    ...
    try {
      return await this.lock.withLock(async () => {
        const current = await this.readUsableCredential([...ATLASCODE_OAUTH_SCOPES]);
  ...
  private async recoverInterruptedCredentialCommit(observedState: AuthState): Promise<AuthState> {
    ...
    assertCredentialContract(observedCredential, [...ATLASCODE_OAUTH_SCOPES]);
    return this.lock.withLock(async () => {
      const state = await this.stateStore.read();
      const credential = await this.options.credentialStore.get(
        this.options.namespace.credentialKey,
...
function baseState(
  ...
  return {
    ...
    clientId: ATLASCODE_OAUTH_CLIENT_ID,
    scopes: [...ATLASCODE_OAUTH_SCOPES],
    audience: ATLASCODE_OAUTH_AUDIENCE,
...
function credentialFromGrant(
  ...
  assertCredentialContract(credential, [...ATLASCODE_OAUTH_SCOPES]);
  if (grant.audience !== ATLASCODE_OAUTH_AUDIENCE) throw new AuthResourceContractMismatchError();
  return credential;
}

function validateRequestedScopes(requiredScopes: string[]): void {
  const unsupported = requiredScopes.filter(
    (scope) => !ATLASCODE_OAUTH_SCOPES.includes(scope as 'agent.default'),
  );
...
function assertCredentialContract(credential: StoredCredential, requiredScopes: string[]): void {
  if (
    credential.clientId !== ATLASCODE_OAUTH_CLIENT_ID ||
    credential.audience !== ATLASCODE_OAUTH_AUDIENCE ||
