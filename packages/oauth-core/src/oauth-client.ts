...
import { ATLASCODE_OAUTH_AUDIENCE, ATLASCODE_OAUTH_CLIENT_ID, ATLASCODE_OAUTH_SCOPES } from './contracts.js';
...
export class HttpOAuthClient implements OAuthClient {
  ...
  async startDeviceAuthorization(options: OAuthRequestOptions = {}): Promise<DeviceAuthorization> {
    ...
    const body = await this.postForm(
      ...
      {
        client_id: ATLASCODE_OAUTH_CLIENT_ID,
        scope: ATLASCODE_OAUTH_SCOPES.join(' '),
...
function parseTokenGrant(
  ...
  if (
    ...
    !expiresInSec ||
    !scopes.includes(ATLASCODE_OAUTH_SCOPES[0])
  ) {
