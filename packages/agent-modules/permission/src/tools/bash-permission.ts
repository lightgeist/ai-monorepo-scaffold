...
export function parseRmTargetsWithMetadata(
  ...
  for (let i = rmIdx + 1; i < tokens.length; i++) {
    ...
    if (tokenMeta) {
      ...
      // drive/UNC-shaped target before building the atlascode-trash rewrite.
      if (
...
/**
 * Build a atlascode-trash command that replaces an `rm` invocation.
 *
 ...
 *     8. rm rewrite           → allow (rewrites to atlascode-trash even with
...
function evaluateSingleSubcommand(
  ...
  //
  // EXCEPT: rm → atlascode-trash rewrite (step 8 below) is a transformation,
  ...
  if (mode === 'bypass') {
    ...
    // want; deny with a hint that atlascode-trash is the recoverable path.
    //
      ...
      return logLayer('bypass-delete-sniff-deny', {
        ...
        reason: {
          ...
          description:
            ...
            'Use `atlascode-trash <path>` for recoverable removal, or drop bypass for this call.',
