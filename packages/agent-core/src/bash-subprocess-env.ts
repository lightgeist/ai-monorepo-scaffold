 ...
 *
 * Layer A — AtlasCode runtime boundary strip. ALWAYS on, no escape hatch.
 ...
 *   parent session against the runtime API, or a nested `atlascode` CLI inside
...
export const BASH_SUBPROCESS_SCRUB: readonly string[] = [
  ...
  // AtlasCode-side additions — runtime-only integration tokens, no subprocess use
  'FIGMA_API_TOKEN',
  'FIGMA_TOKEN',
  'E2E_AUTH_TOKEN',
];
 ...
 * `ATLASCODE_BASH_ENV_SANITIZE` env switch > auto (CI markers → `scrub`,
...
export function resolveBashEnvPolicy(
  ...
  if (mode === undefined) {
    const explicit = envSource.ATLASCODE_BASH_ENV_SANITIZE?.trim().toLowerCase();
...
export function sanitizeBashSubprocessEnv(
  ...
  // Layer A — AtlasCode boundary strip. Always on; allowlist deliberately ignored.
