...
describe("Agent", () => {
	...
	it.each(["steering", "follow-up"] as const)(
		"should stop after a turn before consuming a queued %s message",
		async (queueKind) => {
			let responseCount = 0;
			let queued = false;
			const agent = new Agent({
			...
			agent.subscribe((event) => {
				if (event.type !== "turn_end" || queued) return;
				queued = true;
				const message = {
					role: "user" as const,
					content: [{ type: "text" as const, text: `Queued ${queueKind}` }],
					timestamp: Date.now(),
				};
				if (queueKind === "steering") agent.steer(message);
	...
	it("steerBatch consumes the whole snapshot in one slot and leaves subsequent steering separate", async () => {
		...
		agent.state.messages = [createAssistantMessage("previous answer")];
		const snapshot = ["A", "machine", "B"].map((content, timestamp) => ({ role: "user" as const, content, timestamp }));
		agent.steerBatch(snapshot);
		agent.steer({ role: "user", content: "C after snapshot", timestamp: 3 });
		await agent.continue();
		expect(contexts).toHaveLength(2);
		for (const member of snapshot) expect(contexts[0]).toContain(member.content);
		expect(contexts[0]).not.toContain("C after snapshot");
		expect(contexts[1]).toContain("C after snapshot");
		expect(agent.state.messages.map((message) => message.role)).toEqual(["assistant", "user", "user", "user", "assistant", "user", "assistant"]);
	});
