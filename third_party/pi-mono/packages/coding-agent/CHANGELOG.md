...
- Updated `minimatch` and `undici` to patched releases used by the MiniMax packaged CLI dependency closure.

### Fixed

- Distinguished an unavailable Photon image-processing runtime from images that cannot be resized below the inline size limit, so read-tool and CLI file inputs report the actual failure.
- Fixed built-in tool tests to tolerate terminal wrapping of long resource paths and preserve spawn-error coverage after constructor-time shell detection.
- Fixed the different-file mutation queue regression to use deterministic callback coordination instead of a scheduler-sensitive overlap delay.
- Fixed reftable watcher tests to wait for the refresh to start while preserving the exact-one debounce assertion under concurrent test load.
- Fixed coding-agent asset copying on Windows when nested npm scripts cannot resolve pnpm-provided `shx` binaries.
