...
import { type BashOperations, createBashTool, createBashToolDefinition, createLocalBashOperations } from "../src/core/tools/bash.ts";
...
describe("Coding Agent Tools", () => {
	...
	describe("read tool", () => {
		it("should read file contents that fit within limits", async () => {
			...
			expect(result.details?.textFile).toEqual({
				path: testFile,
				content,
				numLines: 3,
				startLine: 1,
				totalLines: 3,
				truncatedByOutputLimit: false,
				wholeFileAutoTruncated: false,
			});
		});
	...
	describe("bash tool", () => {
		it("persists valid UTF-8 when stdout and stderr chunks interleave", async () => {
			const accumulator = new OutputAccumulator({ maxBytes: 3, maxLines: 100 });
			accumulator.append(Buffer.from([0xe6]), "stdout");
			accumulator.append(Buffer.from("X"), "stderr");
			accumulator.append(Buffer.from([0xb1, 0x89]), "stdout");
			accumulator.finish();
			const snapshot = accumulator.snapshot({ persistIfTruncated: true });
			await accumulator.closeTempFile();

			expect(snapshot.content).toBe("汉");
			expect(snapshot.fullOutputPath).toBeDefined();
			expect(readFileSync(snapshot.fullOutputPath!, "utf8")).toBe("X汉");
		...
		it("describes the command as the current shell rather than always calling it Bash", () => {
			...
			const commandDescription = (definition.parameters as { properties?: { command?: { description?: string } } }).properties
				?.command?.description;
			expect(commandDescription).toContain("current shell");
			expect(commandDescription).toContain("default shell is PowerShell");
		});
		...
		it("should execute simple commands", async () => {
			...
			expect(result.details?.processOutput).toEqual({
				stdout: "test output\n",
				stderr: "",
				exitCode: 0,
				interrupted: false,
		...
		it("preserves stdout and stderr as separate source-owned process facts", async () => {
			const result = await bashTool.execute("test-call-split-streams", {
				command: "printf 'from-stdout'; printf 'from-stderr' >&2",
			});

			expect(result.details?.processOutput).toMatchObject({
				stdout: "from-stdout",
				stderr: "from-stderr",
				exitCode: 0,
				interrupted: false,
		...
		it("should handle process spawn errors", async () => {
			...
			vi.spyOn(shellModule, "getShellConfig").mockReturnValueOnce(invalidShell).mockReturnValueOnce(invalidShell);
