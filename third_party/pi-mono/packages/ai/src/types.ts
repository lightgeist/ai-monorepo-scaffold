...
export interface StreamOptions {
	...
	/**
	 * Provider 在把原始异常转换为最终错误文本前同步调用。宿主可借此采集异常对象，
	 * 回调不得改变请求结果，provider 会消化回调自身抛出的异常。
	 */
	onProviderError?: (error: unknown, model: Model<Api>) => void;
	/**
	 * Optional synchronous observer for provider-native events after transport
	 * parsing and before Pi projects them into AssistantMessage events.
	 *
	 * Providers isolate observer failures. Hosts must copy anything they retain;
	 * an event may be mutated later while its streamed deltas are assembled.
	 */
	onProviderStreamEvent?: (event: unknown, model: Model<Api>) => void;
	/**
	...
	headers?: Record<string, string>;
	/**
	 * Optional fetch implementation for providers/SDKs that support custom HTTP
	 * transports. Hosts can use this to route requests through their own proxy
	 * and trust-store integration.
	 */
