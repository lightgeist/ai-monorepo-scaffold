...
const DEFAULT_SSE_HEADER_TIMEOUT_MS = 30_000;
...
function resolveCodexTransport(options?: OpenAICodexResponsesOptions): NonNullable<StreamOptions["transport"]> {
	const requestedTransport = options?.transport ?? "auto";
	return requestedTransport === "auto" && options?.fetch ? "sse" : requestedTransport;
}
