...
import { notifyProviderError, withProviderEventObserver } from "../utils/provider-error.ts";
...
export const DEFAULT_THINKING_MODEL_IDS = [
	"claude-opus-5",
	"claude-sonnet-5",
	"claude-fable-5",
	"claude-mythos-5",
...
export function isDefaultThinkingModelId(value: unknown): boolean {
	if (typeof value !== "string") return false;
	const normalizedId = value.trim().toLowerCase();
	// BYOK owns the model's context configuration. Ignore only the exact [1m]
	// decoration for default-thinking classification; preserve the wire model id.
	const baseId = normalizedId.replace(/\[1m\]$/u, "");
	return (DEFAULT_THINKING_MODEL_IDS as readonly string[]).includes(baseId);
}
...
async function* iterateSseMessages(
	...
	let buffer = "";
	const onAbort = () => {
		// Settle a pending read even if the transport does not propagate abort to its body.
		// The transport's cancellation cleanup must not delay the terminal stream event.
...
async function* iterateAnthropicEvents(
	...
	// Structural-integrity counters only: event counts and elapsed timings.
	// Never record SSE payload bodies, prompts, reasoning or credentials here.
	...
	if (signal?.aborted) {
		// Cancellation is handled by the existing abort path; never report it as a provider error.
		return;
	}

	if (!sawMessageStart) {
		// A stream that never opened a message is a transport/gateway failure, not a valid
		// empty answer. Without this guard the caller would observe stopReason "stop" with
		// empty content and zero usage, i.e. a silent fake success.
		const diagnostics =
			`rawEventCount=${rawEventCount}; messageEventCount=${messageEventCount}; ` +
			`firstEventAtMs=${firstEventAtMs ?? "none"}; lastEventAtMs=${lastEventAtMs ?? "none"}; ` +
			`eofAtMs=${Date.now() - startedAtMs}`;
		throw new Error(
			sawMessageEnd
				? `Anthropic stream ended without message_start (${diagnostics})`
				: `Anthropic stream ended without any message events (${diagnostics})`,
		);
	}
...
export const streamAnthropic: StreamFunction<"anthropic-messages", AnthropicOptions> = (
	...
	(async () => {
		...
		try {
			...
			const providerEvents = iterateAnthropicEvents(response, options?.signal);
			const observedEvents = withProviderEventObserver(providerEvents, options?.onProviderStreamEvent, model);
...
export const streamSimpleAnthropic: StreamFunction<"anthropic-messages", SimpleStreamOptions> = (
	...
	// Current Claude default-thinking models do not need an explicit `thinking`
	// parameter. Keep the agent's reasoning effort on output_config only.
	if (isDefaultThinkingModelId(model.id)) {
...
function buildParams(
	...
	if (model.reasoning) {
		if (options?.thinkingEnabled) {
			...
			if (isDefaultThinkingModelId(model.id)) {
				// Thinking is already enabled by the model default; only effort
				// belongs on the wire. `thinkingEnabled` stays an internal control
				// so an explicit off selection can still use thinking.type=disabled.
				if (options.effort) {
