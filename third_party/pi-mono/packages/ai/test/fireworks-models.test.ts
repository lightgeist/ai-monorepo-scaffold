...
async function captureAnthropicRequest(
	...
	context: Context,
	options?: { sessionId?: string; cacheRetention?: string; fetch?: typeof globalThis.fetch },
...
describe("Fireworks Anthropic session affinity and tool compat", () => {
	it("uses the custom fetch implementation for Anthropic-compatible requests", async () => {
		...
		await captureAnthropicRequest(model, createContext(), { fetch: customFetch });
