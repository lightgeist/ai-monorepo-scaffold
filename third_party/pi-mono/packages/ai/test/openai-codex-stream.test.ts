...
describe("openai-codex streaming", () => {
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		...
		const streamResult = streamOpenAICodexResponses(model, context, {
			apiKey: token,
			transport: "sse",
			onProviderStreamEvent: (event) => providerEvents.push(event),
		});
		...
		expect(providerEvents[2]).toMatchObject({
			type: "response.output_text.delta",
			delta: "Hello",
		});
	});

	it("uses custom fetch for the Codex SSE request", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
		const encoder = new TextEncoder();
		const modelFetch = vi.fn(async (input: string | URL, init?: RequestInit) => {
			const url = typeof input === "string" ? input : input.toString();
		...
		const streamResult = streamOpenAICodexResponses(model, context, {
			apiKey: token,
			fetch: modelFetch as unknown as typeof globalThis.fetch,
			transport: "sse",
		});
		...
		expect(globalFetch.mock.calls.map((call) => String(call[0]))).not.toContain(
			"https://chatgpt.com/backend-api/codex/responses",
		);
	});

	it("uses SSE by default when a custom fetch implementation is provided", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
		const encoder = new TextEncoder();
		...
		class UnexpectedWebSocket {
			constructor() {
				throw new Error("custom fetch calls should not use the websocket transport by default");
			}
		}
		vi.stubGlobal("fetch", globalFetch);
	...
	it("allows SSE response headers that arrive after 10 seconds", async () => {
		vi.useFakeTimers();
		const token = mockToken();
	...
	it("aborts SSE fetch when response headers do not arrive within 30 seconds", async () => {
		vi.useFakeTimers();
		const token = mockToken();
		let requestSignal: AbortSignal | undefined;
