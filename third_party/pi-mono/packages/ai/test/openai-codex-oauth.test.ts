...
describe("OpenAI Codex OAuth", () => {
	...
	it("uses the injected fetch for browser authorization code exchange", async () => {
		const accessToken = createAccessToken("account-browser");
		let authorizationUrl = "";
		const injectedFetch = vi.fn(async (input: unknown, init?: RequestInit): Promise<Response> => {
		...
		await expect(
			loginOpenAICodex({
				...
				onManualCodeInput: async () => {
					...
					if (!state) throw new Error("Authorization state was not provided");
					return `http://localhost:1455/auth/callback?code=oauth-code&state=${state}`;
				},
	...
	it("refreshes tokens through the injected fetch implementation", async () => {
		...
		await expect(refreshOpenAICodexToken("refresh-token", injectedFetch as typeof fetch)).resolves.toMatchObject({
			access: accessToken,
