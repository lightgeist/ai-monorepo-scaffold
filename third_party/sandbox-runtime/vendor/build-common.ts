...
export function setup(opts: {
  ...
  srcDirName: string
  /** Permit an explicit SRT_BUILD_ARCH cross-build on another host platform. */
  allowCrossPlatform?: boolean
}): { SRC: string; OUT: string; arch: string } {
  if (
    process.platform !== opts.requirePlatform &&
    !(opts.allowCrossPlatform && process.env.SRT_BUILD_ARCH)
  ) {
    console.error(
  ...
  const arch = process.env.SRT_BUILD_ARCH ?? nodeArchToDir[process.arch]
  if (!arch || !Object.values(nodeArchToDir).includes(arch)) {
