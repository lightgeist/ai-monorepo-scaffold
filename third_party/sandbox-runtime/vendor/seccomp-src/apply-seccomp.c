...
static void install_observe_filter(int sp_fd) {
    if (sp_fd < 0) return;
    int debug = getenv("SRT_OBSERVE_DEBUG") != NULL;
    ...
    if (!(syscall(SYS_seccomp, SECCOMP_SET_MODE_FILTER,
          ...
          errno == EFAULT)) {
        if (debug) dprintf(2, "apply-seccomp: observe feature probe failed: %s\n",
                           strerror(errno));
    ...
    if (nfd < 0) {
        ...
        if (debug) dprintf(2, "apply-seccomp: observe listener failed: %s\n",
                           strerror(errno));
        (void)!write(sp_fd, "E", 1);
        close(sp_fd);
        return;
                ...
                if (oc && out_sock >= 0 &&
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        ssize_t l = read_remote_cstr(req->pid,
                                      ...
                                      pbuf, OBS_PATH_MAX);
                        if (l <= 0) {
                            if (debug && !reported_read_error) {
                                dprintf(2,
                                    "apply-seccomp: observe path read failed for pid %d: %s\n",
                                    (int)req->pid, strerror(errno));
                                reported_read_error = 1;
                            }
                            continue;
                        }
                        if (pbuf[0] == '/') {
...
/* Clear effective, permitted and inheritable capabilities in the workload
 * process after the namespace/mount setup is complete. Clearing only the
 * ambient set is insufficient when apply-seccomp itself starts with an
 * effective capability set (as it does on a privileged runner): exec would
 * otherwise preserve those capabilities for an unprivileged file. */
static void drop_all_capabilities(void) {
    ...
    if (syscall(SYS_capset, &header, data) < 0) {
        die("apply-seccomp: capset(drop all)");
    }
}
