...
import { connectViaParentProxy, stripHopByHop } from "./parent-proxy.js";
...
export type TerminateTarget = {
  ...
  /** Parent HTTP(S) proxy selected for this HTTPS destination, if any. */
  parentProxyUrl?: URL;
  /**
...
async function forwardUpstream(
  ...
  // https.request does not resolve HTTP(S)_PROXY itself. Use a one-shot
  // Agent whose connection factory first opens a CONNECT tunnel through the
  // selected parent proxy, then performs the ordinary verified TLS handshake
  // inside it. This keeps TLS termination usable on corporate networks while
  // preserving hostname/SNI verification on the final upstream.
  const parentAgent = target.parentProxyUrl
    ? new HttpsAgent({ keepAlive: false })
    : undefined;
  if (parentAgent && target.parentProxyUrl) {
    const parentProxyUrl = target.parentProxyUrl;
    parentAgent.createConnection = (options, callback) => {
      if (!callback) {
        throw new Error("parent proxy TLS connection requires a callback");
      }
      void connectViaParentProxy(
        parentProxyUrl,
        target.hostname,
        target.port,
      ).then(
        (tunnel) => {
          ...
          callback(null, upstreamTls);
        },
        (error) => callback(error as Error, null as unknown as Duplex),
      );
      return undefined;
    };
  }
