...
/** Test-only invariant probe; never exposes command text or ids. */
export function __getCommandTextMapSize(): number {
  return commandTextsById.size;
}
...
async function filterNetworkRequest(
  ...
  // MCode allow-all mode still traverses the proxy so explicit deny rules
  // remain authoritative and the policy can be swapped live.
  if (config.network.allowAll) {
    logForDebugging(`Allowed by allowAll after deny rules: ${host}:${port}`);
    return true;
  }
...
async function initialize(
  ...
  // MCODE-008: network.disabled=true skips the entire network subsystem —
  // no mux proxy, no Linux socat bridge, no Java agent jar resolution. A
  // network-infrastructure failure therefore can never block the
  // filesystem sandbox, and wrap-time needsNetworkProxy stays false for
  // every command (see hasNetworkConfig in wrapWithSandbox /
  // wrapWithSandboxArgv), so nothing downstream ever waits on the
  // never-created initializationPromise.
  if (runtimeConfig.network.disabled === true) {
    logForDebugging("Network subsystem disabled (network.disabled=true)");
    return;
  }
...
function getCredentialDenyReadPaths(
  ...
  const files = credentials?.files ?? [];
  return [
    ...new Set(files.filter((f) => f.mode === "deny").map((f) => f.path)),
  ];
}
...
export type WrapWithSandboxOptions = {
   ...
   */
  commandText?: string;
  /** Sanitized per-invocation environment used for child-env composition. */
  baseEnv?: NodeJS.ProcessEnv;
  /** Consumer-owned per-session temp directory. Required for new filesystem-enabled callers. */
...
async function wrapWithSandbox(
  ...
  if (!fsDisabled && options && !options.sandboxTempDir) {
    throw new Error(
      "sandboxTempDir is required in wrap options when filesystem policy is enabled",
    );
  }
  ...
  // Always include compatibility system write paths (for example /dev/null)
  //
  ...
  if (!fsDisabled) {
    ...
    writeConfig = {
      ...
      unlinkAllowOnly:
        (customConfig?.filesystem?.unlinkAllowOnly ??
        config?.filesystem.unlinkAllowOnly)
          ? stripWriteGlobs(
              customConfig?.filesystem?.unlinkAllowOnly ??
                config?.filesystem.unlinkAllowOnly ??
                [],
  ...
  //
  // MCODE-008: network.disabled=true turns network enforcement off entirely,
  // regardless of allowedDomains — no restriction, no proxy.
  const networkDisabled =
    customConfig?.network?.disabled ?? config?.network?.disabled ?? false;
  ...
  switch (platform) {
    case "macos":
      ...
      return wrapCommandWithSandboxMacOS({
        ...
        allowSecurityServer:
          customConfig?.allowSecurityServer ??
          config?.allowSecurityServer ??
          true,
        binShell,
      });
...
async function wrapWithSandboxArgv(
  ...
  if (platform === "windows") {
    const baseEnv = options?.baseEnv ?? process.env;
    // MCODE-008: same disabled short-circuit as wrapWithSandbox, so the
    // Windows path can never fork semantically from macOS/Linux.
    const networkDisabled =
      customConfig?.network?.disabled ?? config?.network?.disabled ?? false;
    const hasNetworkConfig =
      !networkDisabled &&
  ...
  return { argv: [shell, "-c", wrapped], env: options?.baseEnv ?? process.env };
}
