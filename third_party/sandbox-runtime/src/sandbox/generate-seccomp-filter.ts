...
const GLOBAL_PACKAGE_PATH_PARTS = [
  "@minimax",
  "mcode-sandbox-runtime",
] as const;

/**
 * Get paths to check for the globally installed internal package.
...
export function getGlobalNpmPaths(): string[] {
  ...
  try {
    ...
    if (npmRoot) {
      paths.push(join(npmRoot, ...GLOBAL_PACKAGE_PATH_PARTS));
    }
  } catch {
  ...
  paths.push(
    ...
    join("/usr", "lib", "node_modules", ...GLOBAL_PACKAGE_PATH_PARTS),
    join("/usr", "local", "lib", "node_modules", ...GLOBAL_PACKAGE_PATH_PARTS),
    ...
    join(
      ...
      "lib",
      "node_modules",
      ...GLOBAL_PACKAGE_PATH_PARTS,
    ),
    // User-local npm global
    join(home, ".npm", "lib", "node_modules", ...GLOBAL_PACKAGE_PATH_PARTS),
    join(
      home,
      ".npm-global",
      "lib",
