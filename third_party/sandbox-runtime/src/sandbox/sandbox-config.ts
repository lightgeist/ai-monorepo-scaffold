 ...
 */
const NetworkConfigObjectSchema = z.object({
  disabled: z
    .boolean()
    .optional()
    .describe(
      "Disable all network policy enforcement. When true, the network " +
        "subsystem is skipped entirely: no local proxy is started, no " +
        "proxy/CA-trust/Java-agent env vars are injected into sandboxed " +
        "children, and the sandbox profile allows every network operation " +
        '(macOS: "(allow network*)" plus "(allow system-socket)"). ' +
        "allowedDomains/deniedDomains and the other network fields are " +
        "ignored. Filesystem and credential-env restrictions still apply. " +
        "Incompatible with tlsTerminate, mitmProxy, parentProxy, " +
  ...
  allowAll: z
    ...
    .optional()
    .describe(
      "Allow every destination not matched by deniedDomains. Deny rules are always evaluated first.",
    ),
  allowUnixSockets: z
    .array(z.string())
    .optional()
...
// MCODE-008: `disabled` bypasses the proxy entirely, so any field that only
// has an effect through the proxy pipeline is a contradiction — reject it
// loudly instead of silently ignoring it. Kept as a separate chained schema
// so the object literal above stays byte-identical to upstream ordering.
export const NetworkConfigSchema = NetworkConfigObjectSchema.superRefine(
  (network, ctx) => {
    if (network.disabled !== true) return;
    for (const key of [
      "tlsTerminate",
      "mitmProxy",
      "parentProxy",
      ...
      if (network[key] !== undefined) {
        ctx.addIssue({
          ...
          path: [key],
          message:
            `network.${key} has no effect when network.disabled is true ` +
            `— the network subsystem (proxy, TLS termination, env ` +
            `injection) is skipped entirely. Remove one of the two.`,
        });
      }
    }
  },
...
export const FilesystemConfigSchema = z.object({
  ...
  unlinkAllowOnly: z
    ...
    .optional()
    .describe(
      "macOS only: canonical roots within which file-write-unlink is allowed. When set, unlink is denied globally before these roots are re-allowed.",
    ),
  denyWrite: z
    .array(filesystemPathSchema)
...
export const SandboxRuntimeConfigSchema = z
  .object({
    ...
    allowSecurityServer: z
      .boolean()
      .optional()
      .describe(
        "Allow macOS SecurityServer Mach lookup. Defaults to true for upstream compatibility; set false to remove the rule from the Seatbelt profile.",
      ),
