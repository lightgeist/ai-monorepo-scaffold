...
export function isSymlinkOutsideBoundary(
  ...
  // /tmp/tool-session -> /private/tmp/tool-session is OK
  ...
  // e.g., /tmp/tool-session -> /tmp means the symlink points to a broader scope
  if (normalizedOriginal.startsWith(normalizedResolved + "/")) {
    return true;
  }

  // Also check the canonical form of the original path for macOS
  // e.g., /tmp/tool-session should also be checked as /private/tmp/tool-session
  ...
  // Any other resolution (e.g., /tmp/tool-session -> /Users/example) is outside expected bounds
...
export function generateProxyEnvVars(
  ...
  // The consumer owns a private per-session temp directory and grants it in
  // allowWrite. Never fall back to an ambient or shared product directory.
  if (!skipTmpdir && sandboxTempDir) {
    envVars.push(`TMPDIR=${sandboxTempDir}`);
  }
...
export function buildPosixGitSafeDirEnv(opts: {
  ...
  const baseEnv: Record<string, string | undefined> = {
    GIT_CONFIG_COUNT: (opts.baseEnv ?? process.env).GIT_CONFIG_COUNT,
  };
