...
export interface MacOSSandboxParams {
  ...
  /** Sanitized environment inherited by the child and used for composition. */
  baseEnv?: Readonly<Record<string, string | undefined>>;
  /** Consumer-owned per-session temp directory used as child TMPDIR. */
  ...
  /** Allow SecurityServer Mach lookup. Defaults to true for compatibility. */
  allowSecurityServer?: boolean;
  binShell?: string;
}
...
function lateReadDenyFilters(resolved: ResolvedReadConfig): {
  ...
  const literalAllowDirs = resolved.allows
    .filter((a) => !a.glob)
    .map((a) => a.path);
...
function generateReadDenyUnlinkRules(
  ...
  const literalWriteRoots = writeRoots
    .filter((w) => !w.glob)
    .map((w) => w.path);
  const strictlyBelowWriteRoot = (p: string): boolean =>
    literalWriteRoots.some((w) =>
      isStrictlyUnder({ path: p, glob: false }, w),
...
function generateReadRules(
  ...
  const denyFilters = new Set(
    resolved.denies.map((d) => denyPathFilter(d.path)),
  );
...
function generateWriteRules(
  ...
  // MCode operation-scoped delete policy. The global unlink deny must be
  // emitted after both the read-side write-root allow and the move-blocking
  // rules above. Only unlink is re-opened: file-write-create remains subject
  // to the mandatory injection protections. Explicit caller denyWrite paths
  // are carved out of each re-allow root; mandatory deny patterns are not,
  // so deleting third-party trees containing .vscode/.mcp.json still works.
  if (config.unlinkAllowOnly !== undefined) {
    rules.push(
      ...renderRule(
        "deny",
        ["file-write-unlink"],
    ...
    const callerDenyFilters = (config.denyWithinAllow ?? []).map((p) =>
      denyPathFilter(normalizePathForSandbox(p)),
    );
    const unlinkAllowFilters = new Set(
      config.unlinkAllowOnly.map((p) =>
        carveFilter(pathFilter(normalizePathForSandbox(p)), callerDenyFilters),
      ),
    );
    rules.push(
      ...renderRule("allow", ["file-write-unlink"], unlinkAllowFilters, logTag),
    );
  }
...
function generateSandboxProfile({
  ...
  if (!needsNetworkRestriction) {
    ...
    // MCODE-008: network* covers bind/inbound/outbound but NOT the
    // socket(AF_UNIX) creation syscall — that needs system-socket (see the
    // allowAllUnixSockets note below). Allow it here so Unix-socket
    // clients (ssh-agent, docker.sock, Gradle) work in the unrestricted
    // branch too.
...
export function wrapCommandWithSandboxMacOS(
  ...
  const javaToolOptions = buildJavaToolOptions({
    ...
    unsetEnvVars,
    inherited: baseEnv.JAVA_TOOL_OPTIONS,
  });
