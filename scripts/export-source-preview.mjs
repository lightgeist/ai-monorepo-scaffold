...
try {
  ...
  await measure("archive", () =>
    git(["archive", "--format=tar.gz", "--prefix=atlascode/", revision], {
      stdio: ["ignore", fd, "pipe"],
    }),
  );
  snapshot = mkdtempSync(path.join(tmpdir(), "atlascode-source-export-"));
  extractor = await measure("extract", () =>
    extractSourceArchive(
      destination,
      snapshot,
      process.env.ATLASCODE_SOURCE_EXTRACTOR,
    ),
  );
  const snapshotRoot = path.join(snapshot, "atlascode");
  ...
  if (process.env.ATLASCODE_VERIFY_REPORT_DIR) {
    mkdirSync(process.env.ATLASCODE_VERIFY_REPORT_DIR, { recursive: true });
    writeFileSync(
      path.join(process.env.ATLASCODE_VERIFY_REPORT_DIR, "export.json"),
