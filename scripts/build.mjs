...
import { copyAtlasCodeToolsArtifact } from './lib/atlascode-tools-artifact.mjs';
...
const version = packages.get("@atlascode/atlascode").manifest.version;
const result = await build({
  absWorkingDir: root,
  entryPoints: {
    cli: "packages/tui/src/index.ts",
    "image-preview-worker": "packages/tui/src/host/image-preview-worker.ts",
    'atlascode-tools': 'packages/tui/src/cli/atlascode-tools-entry.ts',
...
await copyAtlasCodeToolsArtifact(root, outdir);
cpSync(path.join(root, 'packages/tui/src/cli/atlascode-tools-launchers'), path.join(outdir, 'internal-bin'), { recursive: true });
for (const name of ['internal-bin/atlascode-tools', 'atlascode-tools.js', 'matrix-mcp-stdio.js'])
...
console.log(
  `Built AtlasCode ${version} from ${Object.keys(result.metafile.inputs).length} source files.`,
);

writeFileSync(
  path.join(outdir, "package.json"),
  JSON.stringify(
    { name: "@deepintuition/atlascode", version, type: "module", private: true },
    null,
    2,
  ) + "\n",
);
