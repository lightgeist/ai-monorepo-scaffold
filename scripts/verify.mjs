...
const temporary = values.list
  ? undefined
  : mkdtempSync(path.join(tmpdir(), "atlascode-verify-"));
const preview = path.join(temporary ?? tmpdir(), "atlascode-source.tar.gz");
...
const report = {
  ...
  revision:
    revision.status === 0
      ...
      : profile === "archive" &&
          /^[a-f0-9]{40}$/u.test(process.env.ATLASCODE_VERIFY_REVISION ?? "")
        ? process.env.ATLASCODE_VERIFY_REVISION
        : null,
...
const reportDir = process.env.ATLASCODE_VERIFY_REPORT_DIR;
function saveReport() {
