...
function verificationFixture(t) {
  ...
  writeFileSync(manager, `
    ...
    const report = JSON.parse(fs.readFileSync(path.join(process.env.ATLASCODE_VERIFY_REPORT_DIR, 'verification.json')));
  ...
  function run(args = [], env = {}) {
    return spawnSync(process.execPath, [verifier, ...args], {
      ...
      env: {
        ...
        ATLASCODE_VERIFY_REPORT_DIR: reportDir,
...
test('manual source candidates pin every checkout and receipt to the selected revision', () => {
  ...
  assert.equal(workflow.env.ATLASCODE_CANDIDATE_REVISION, '${{ inputs.revision || github.sha }}');
  assert.equal(workflow.concurrency['cancel-in-progress'], false);
  assert.match(workflow.concurrency.group, /inputs\.revision \|\| github\.sha/);
  const revision = '${{ env.ATLASCODE_CANDIDATE_REVISION }}';
  ...
  assert.equal(verify.env.ATLASCODE_VERIFY_REVISION, revision);
...
test('native and Node source extractors produce identical file content', async t => {
  const f = archiveFixture(t, [
    { path: 'atlascode/README.md', content: 'Source preview\n' },
    { path: 'atlascode/nested/file.txt', content: 'portable contents\n' },
  ]);
  for (const extractor of ['native', 'node']) {
    ...
    assert.equal(readFileSync(path.join(output, 'atlascode/nested/file.txt'), 'utf8'), 'portable contents\n');
  }
});

test('Windows extraction ignores a shadow tar executable on PATH', { skip: process.platform !== 'win32' }, t => {
  const f = archiveFixture(t, [{ path: 'atlascode/README.md', content: 'Windows archive\n' }]);
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(readFileSync(path.join(output, 'atlascode/README.md'), 'utf8'), 'Windows archive\n');
});

test('source archive rejects traversal, links, Git history and duplicate entries before writing files', async t => {
  const good = { path: 'atlascode/good.txt', content: 'safe' };
  for (const bad of [
    { path: 'atlascode/../../outside.txt', content: 'unsafe' },
    { path: 'atlascode/link', type: 'SymbolicLink', linkpath: '../../outside' },
    { path: 'atlascode/link', type: 'Link', linkpath: '../../outside' },
    { path: 'atlascode/.git/config', content: 'history' },
    { path: 'atlascode/file:stream', content: 'stream' },
    good,
  ]) {
...
test('candidate rejects mismatched receipts and requires three successful same-revision reports', t => {
  const f = archiveFixture(t, [{ path: 'atlascode/README.md', content: 'source' }]);
